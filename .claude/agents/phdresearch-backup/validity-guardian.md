...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/validity" --key "threat_assessment"
    npx tsx src/god-agent/universal/cli.ts learn "$VALIDITY_ASSESSMENT" -d "research/validity" -t "threat_assessment" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/validity" --key "threat_assessment" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/validity-threat-assessment.json \
  -d "research/validity" \
  -t "threat_assessment" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/validity" --key "methods_text" --value "..."
npx tsx src/god-agent/universal/cli.ts learn "Validity procedures text for Methods section" \
  -d "research/validity" \
  -t "methods_text" \
  -c "fact"
```
