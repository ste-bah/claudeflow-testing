...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/constructs" --key "definitions"
    npx tsx src/god-agent/universal/cli.ts learn "$CONSTRUCTS_JSON" -d "research/constructs" -t "definitions" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/context/topic"
npx tsx src/god-agent/universal/cli.ts query -d "research/context" -t "topic" --json

# LEGACY: npx claude-flow memory retrieve --key "research/methodology/approaches"
npx tsx src/god-agent/universal/cli.ts query -d "research/methodology" -t "approaches" --json
```
...
# LEGACY: npx claude-flow memory store --namespace "research/constructs" --key "definitions" --value '{...}'
cat > /tmp/phd-construct-definitions.json << 'EOF'
{
  "primary_constructs": [],
  "variable_types": {},
  "measurement_methods": {},
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-construct-definitions.json \
  -d "research/constructs" \
  -t "definitions" \
  -c "fact"
rm -f /tmp/phd-construct-definitions.json

# For All Future Agents
# LEGACY: npx claude-flow memory store --namespace "research/constructs" --key "glossary" --value '{...}'
cat > /tmp/phd-glossary.json << 'EOF'
{
  "construct1": "operational_definition",
  "construct2": "operational_definition"
}
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-glossary.json \
  -d "research/constructs" \
  -t "glossary" \
  -c "fact"
rm -f /tmp/phd-glossary.json

# For Methodology Agents
# LEGACY: npx claude-flow memory store --namespace "research/constructs" --key "measurement_requirements" --value '{...}'
cat > /tmp/phd-measurement-requirements.json << 'EOF'
{
  "validity_thresholds": {},
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-measurement-requirements.json \
  -d "research/constructs" \
  -t "measurement_requirements" \
  -c "fact"
rm -f /tmp/phd-measurement-requirements.json
```
