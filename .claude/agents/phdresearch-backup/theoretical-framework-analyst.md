...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/literature/literature-map"
    npx tsx src/god-agent/universal/cli.ts query -d "research/literature" -t "literature-map" --json
  post: |
    echo "✅ Theoretical frameworks identified and mapped"
    # LEGACY: npx claude-flow memory store --namespace "research/theory" --key "theoretical-frameworks"
    npx tsx src/god-agent/universal/cli.ts learn "$THEORETICAL_FRAMEWORKS_JSON" -d "research/theory" -t "theoretical-frameworks" -c "fact"
---
...
## MEMORY RETRIEVAL
```bash
# LEGACY: npx claude-flow memory retrieve --key "research/literature/theoretical-clusters"
npx tsx src/god-agent/universal/cli.ts query -d "research/literature" -t "theoretical-clusters" --json

# LEGACY: npx claude-flow memory retrieve --key "research/quality/source-tiers"
npx tsx src/god-agent/universal/cli.ts query -d "research/quality" -t "source-tiers" --json

# LEGACY: npx claude-flow memory retrieve --key "research/citations/citation-database"
npx tsx src/god-agent/universal/cli.ts query -d "research/citations" -t "citation-database" --json
...
# LEGACY: npx claude-flow memory store --namespace "research/theory" --key "theoretical-frameworks" --value '{...}'
cat > /tmp/phd-theoretical-frameworks.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-theoretical-frameworks.json \
  -d "research/theory" \
  -t "theoretical-frameworks" \
  -c "fact"
rm -f /tmp/phd-theoretical-frameworks.json

# For Gap Hunter
# LEGACY: npx claude-flow memory store --namespace "research/theory" --key "theoretical-gaps" --value '{...}'
cat > /tmp/phd-theoretical-gaps.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-theoretical-gaps.json \
  -d "research/theory" \
  -t "theoretical-gaps" \
  -c "fact"
rm -f /tmp/phd-theoretical-gaps.json

# For All Synthesis Agents
# LEGACY: npx claude-flow memory store --namespace "research/theory" --key "theory-synthesis" --value '{...}'
cat > /tmp/phd-theory-synthesis.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-theory-synthesis.json \
  -d "research/theory" \
  -t "theory-synthesis" \
  -c "fact"
rm -f /tmp/phd-theory-synthesis.json
```
