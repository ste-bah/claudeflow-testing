...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/theory" --key "framework"
    npx tsx src/god-agent/universal/cli.ts learn "$FRAMEWORK_JSON" -d "research/theory" -t "framework" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/synthesis/thematic_framework"
npx tsx src/god-agent/universal/cli.ts query -d "research/synthesis" -t "thematic_framework" --json
...
```bash
# For Hypothesis-Generator
# LEGACY: npx claude-flow memory store --namespace "research/theory" --key "framework" --value '{...}'
cat > /tmp/phd-theory-framework.json << 'EOF'
{
  "constructs": [
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-theory-framework.json \
  -d "research/theory" \
  -t "framework" \
  -c "fact"
rm -f /tmp/phd-theory-framework.json

# For All Future Agents
# LEGACY: npx claude-flow memory store --namespace "research/theory" --key "testable_framework" --value '{...}'
cat > /tmp/phd-testable-framework.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-testable-framework.json \
  -d "research/theory" \
  -t "testable_framework" \
  -c "fact"
rm -f /tmp/phd-testable-framework.json
```
