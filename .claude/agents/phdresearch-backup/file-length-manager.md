...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/paper-complete"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "paper-complete" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/file-structure"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "file-structure" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/section-lengths"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "section-lengths" --json
```
...
# Store results
# LEGACY: npx claude-flow@alpha memory store --key "phd/file-lengths" --content "..."
find docs/phdresearch -name '*.md' -exec wc -l {} \; > /tmp/phd-file-lengths.txt
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-file-lengths.txt \
  -d "phd" \
  -t "file-lengths" \
  -c "fact"
...
**10. Update memory**
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/file-splits" --content '{...}'
cat > /tmp/phd-file-splits.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-file-splits.json \
  -d "phd" \
  -t "file-splits" \
  -c "fact"
rm -f /tmp/phd-file-splits.json
```
...
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/file-length-status" --content '{...}'
cat > /tmp/phd-file-length-status.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-file-length-status.json \
  -d "phd" \
  -t "file-length-status" \
  -c "fact"
rm -f /tmp/phd-file-length-status.json

# Store split metadata for each split file
# LEGACY: npx claude-flow@alpha memory store --key "phd/split-metadata/literature-review" --content '{...}'
cat > /tmp/phd-split-metadata.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-split-metadata.json \
  -d "phd/split-metadata" \
  -t "literature-review" \
  -c "fact"
rm -f /tmp/phd-split-metadata.json

# XP reward (bigger reward for complex splits)
# LEGACY: npx claude-flow@alpha hooks xp-reward --agent "file-length-manager" --xp 60 --reason "..."
