...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/risks/fmea_analysis"
    npx tsx src/god-agent/universal/cli.ts query -d "research/risks" -t "fmea_analysis" --json
  post: |
    echo "✅ Study quality assessment complete"
    # LEGACY: npx claude-flow memory store --namespace "research/quality" --key "assessment"
    npx tsx src/god-agent/universal/cli.ts learn "$QUALITY_ASSESSMENT" -d "research/quality" -t "assessment" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/quality" --key "assessment" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/quality-assessment.json \
  -d "research/quality" \
  -t "assessment" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/quality" --key "evidence_weights" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/evidence-weights.json \
  -d "research/quality" \
  -t "evidence_weights" \
  -c "fact"
