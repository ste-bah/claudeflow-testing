...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/methods/measurement_specs"
    npx tsx src/god-agent/universal/cli.ts query -d "research/methods" -t "measurement_specs" --json
  post: |
    echo "✅ Instruments developed and stored"
    # LEGACY: npx claude-flow memory store --namespace "research/instruments" --key "validated_measures"
    npx tsx src/god-agent/universal/cli.ts learn "$INSTRUMENTS_JSON" -d "research/instruments" -t "validated_measures" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/sampling/recruitment_plan"
npx tsx src/god-agent/universal/cli.ts query -d "research/sampling" -t "recruitment_plan" --json
...
```bash
# LEGACY: npx claude-flow memory store --namespace "research/instruments" --key "validated_measures" --value '{...}'
cat > /tmp/phd-validated-measures.json << 'EOF'
{
  "instruments": [
    {
      "construct": "Psychological Safety",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-validated-measures.json \
  -d "research/instruments" \
  -t "validated_measures" \
  -c "fact"
rm -f /tmp/phd-validated-measures.json
```
