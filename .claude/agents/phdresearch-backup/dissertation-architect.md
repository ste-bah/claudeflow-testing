...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store "dissertation-structure" '{"locked": true}' --namespace "research/structure"
    npx tsx src/god-agent/universal/cli.ts learn '{"locked": true}' -d "research/structure" -t "dissertation-structure" -c "fact"
---
...
1. Retrieve this structure: `npx tsx src/god-agent/universal/cli.ts query -d "research/structure" -t "chapters" --json`
...
# LEGACY: npx claude-flow memory store "chapters" '{...}' --namespace "research/structure"
cat > /tmp/phd-chapters.json << 'EOF'
{
  "locked": true,
  "totalChapters": "N",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-chapters.json \
  -d "research/structure" \
  -t "chapters" \
  -c "fact"
rm -f /tmp/phd-chapters.json

# Store chapter titles for quick reference
# LEGACY: npx claude-flow memory store "chapter-titles" '{...}' --namespace "research/structure"
cat > /tmp/phd-chapter-titles.json << 'EOF'
{
  "1": "Introduction",
  "2": "Literature Review"
}
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-chapter-titles.json \
  -d "research/structure" \
  -t "chapter-titles" \
  -c "fact"
rm -f /tmp/phd-chapter-titles.json

# Store cross-reference rules
# LEGACY: npx claude-flow memory store "reference-rules" '{...}' --namespace "research/structure"
cat > /tmp/phd-reference-rules.json << 'EOF'
{
  "maxValidChapter": "N",
  "rule": "References to Chapter X where X > N are INVALID"
}
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-reference-rules.json \
  -d "research/structure" \
  -t "reference-rules" \
  -c "fact"
rm -f /tmp/phd-reference-rules.json
```
