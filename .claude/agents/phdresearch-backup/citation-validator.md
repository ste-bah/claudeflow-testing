...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/literature-review"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "literature-review" --json
...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/bibliography"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "bibliography" --json
```
...
**Store results**:
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/citation-link-verification" --content "..."
npx tsx src/god-agent/universal/cli.ts learn "$(cat link-check-results.txt)" \
  -d "phd" \
...
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/citation-validation" --content '{...}'
cat > /tmp/phd-citation-validation.json << 'EOF'
{
  "total_citations": 87,
  "total_references": 85,
  "citations_valid": 82,
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-citation-validation.json \
  -d "phd" \
  -t "citation-validation" \
  -c "fact"
rm -f /tmp/phd-citation-validation.json

# Store validated reference list
# LEGACY: npx claude-flow@alpha memory store --key "phd/validated-references" --content "[...]"
npx tsx src/god-agent/universal/cli.ts learn "[Complete, validated reference list in APA 7th format]" \
  -d "phd" \
  -t "validated-references" \
  -c "fact"

# XP reward (Note: hooks system still uses claude-flow for now)
# LEGACY: npx claude-flow@alpha hooks xp-reward --agent "citation-validator" --xp 50 --reason "..."
