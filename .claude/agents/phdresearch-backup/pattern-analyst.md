...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/synthesis/evidence"
    npx tsx src/god-agent/universal/cli.ts query -d "research/synthesis" -t "evidence" --json
  post: |
    echo "✅ Pattern analysis complete"
    # LEGACY: npx claude-flow memory store --namespace "research/patterns" --key "analysis"
    npx tsx src/god-agent/universal/cli.ts learn "$PATTERN_ANALYSIS" -d "research/patterns" -t "analysis" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/patterns" --key "analysis" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/pattern-analysis.json \
  -d "research/patterns" \
  -t "analysis" \
  -c "fact"
