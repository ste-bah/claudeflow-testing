...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/execution/research-plan"
    npx tsx src/god-agent/universal/cli.ts query -d "research/execution" -t "research-plan" --json
  post: |
    echo "✅ Context tiers established: hot/warm/cold"
    # LEGACY: npx claude-flow memory store --namespace "research/organization" --key "context-tiers"
    npx tsx src/god-agent/universal/cli.ts learn "$CONTEXT_TIERS_JSON" -d "research/organization" -t "context-tiers" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/meta/self-ask-questions"
npx tsx src/god-agent/universal/cli.ts query -d "research/meta" -t "self-ask-questions" --json

# LEGACY: npx claude-flow memory retrieve --key "research/meta/principles"
npx tsx src/god-agent/universal/cli.ts query -d "research/meta" -t "principles" --json

# LEGACY: npx claude-flow memory retrieve --key "research/session/config"
npx tsx src/god-agent/universal/cli.ts query -d "research/session" -t "config" --json
```
...
# LEGACY: npx claude-flow memory retrieve --key "research/context/hot-tier"
npx tsx src/god-agent/universal/cli.ts query -d "research/context" -t "hot-tier" --json
...
# LEGACY: npx claude-flow memory retrieve --key "research/context/warm-tier/{source-id}"
npx tsx src/god-agent/universal/cli.ts query -d "research/context" -t "warm-tier-{source-id}" --json
...
# LEGACY: npx claude-flow memory search --namespace "research/context/cold-tier" --query "{keywords}"
npx tsx src/god-agent/universal/cli.ts query -d "research/context" -t "cold-tier" --json
...
# LEGACY: npx claude-flow memory store --namespace "research/organization" --key "context-tiers" --value '{...}'
cat > /tmp/phd-context-tiers.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-context-tiers.json \
  -d "research/organization" \
  -t "context-tiers" \
  -c "fact"
rm -f /tmp/phd-context-tiers.json

# For Citation Extractor
# LEGACY: npx claude-flow memory store --namespace "research/organization" --key "extraction-priority" --value '{...}'
cat > /tmp/phd-extraction-priority.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-extraction-priority.json \
  -d "research/organization" \
  -t "extraction-priority" \
  -c "fact"
rm -f /tmp/phd-extraction-priority.json

# For All Agents
# LEGACY: npx claude-flow memory store --namespace "research/organization" --key "tier-criteria" --value '{...}'
cat > /tmp/phd-tier-criteria.json << 'EOF'
{
  "hot_criteria": {},
  "warm_criteria": {},
  "cold_criteria": {},
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-tier-criteria.json \
  -d "research/organization" \
  -t "tier-criteria" \
  -c "fact"
rm -f /tmp/phd-tier-criteria.json
```
