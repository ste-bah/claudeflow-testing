...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/sources/bibliography"
    npx tsx src/god-agent/universal/cli.ts query -d "research/sources" -t "bibliography" --json
  post: |
    echo "✅ APA 7th formatting applied and verified"
    # LEGACY: npx claude-flow memory store --namespace "research/formatting" --key "apa_compliance"
    npx tsx src/god-agent/universal/cli.ts learn "$APA_COMPLIANCE_JSON" -d "research/formatting" -t "apa_compliance" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/content/sections"
npx tsx src/god-agent/universal/cli.ts query -d "research/content" -t "sections" --json

# LEGACY: npx claude-flow memory retrieve --key "research/tables_figures/list"
npx tsx src/god-agent/universal/cli.ts query -d "research/tables_figures" -t "list" --json
```
...
```bash
# For Abstract Writer
# LEGACY: npx claude-flow memory store --namespace "research/formatting" --key "apa_compliance" --value '{...}'
cat > /tmp/phd-apa-compliance.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-apa-compliance.json \
  -d "research/formatting" \
  -t "apa_compliance" \
  -c "fact"
rm -f /tmp/phd-apa-compliance.json

# For Manuscript Writers
# LEGACY: npx claude-flow memory store --namespace "research/formatting" --key "manuscript_structure" --value '{...}'
cat > /tmp/phd-manuscript-structure.json << 'EOF'
{
  "heading_levels": 5,
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-manuscript-structure.json \
  -d "research/formatting" \
  -t "manuscript_structure" \
  -c "fact"
rm -f /tmp/phd-manuscript-structure.json
```
