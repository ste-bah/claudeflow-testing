...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/models/structural_models"
    npx tsx src/god-agent/universal/cli.ts query -d "research/models" -t "structural_models" --json
  post: |
    echo "✅ Research opportunities identified and stored"
    # LEGACY: npx claude-flow memory store --namespace "research/opportunities" --key "gaps_and_questions"
    npx tsx src/god-agent/universal/cli.ts learn "$OPPORTUNITIES_JSON" -d "research/opportunities" -t "gaps_and_questions" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/theory/framework"
npx tsx src/god-agent/universal/cli.ts query -d "research/theory" -t "framework" --json

# LEGACY: npx claude-flow memory retrieve --key "research/hypotheses/testable_predictions"
npx tsx src/god-agent/universal/cli.ts query -d "research/hypotheses" -t "testable_predictions" --json

# LEGACY: npx claude-flow memory retrieve --key "research/analysis/gaps"
npx tsx src/god-agent/universal/cli.ts query -d "research/analysis" -t "gaps" --json
```
...
```bash
# For Method-Designer
# LEGACY: npx claude-flow memory store --namespace "research/opportunities" --key "gaps_and_questions" --value '{...}'
cat > /tmp/phd-gaps-questions.json << 'EOF'
{
  "theoretical_gaps": [
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-gaps-questions.json \
  -d "research/opportunities" \
  -t "gaps_and_questions" \
  -c "fact"
rm -f /tmp/phd-gaps-questions.json

# For All Future Agents
# LEGACY: npx claude-flow memory store --namespace "research/opportunities" --key "research_roadmap" --value '{...}'
cat > /tmp/phd-research-roadmap.json << 'EOF'
{
  "phase1_quick_wins": [],
  "phase2_core": [],
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-research-roadmap.json \
  -d "research/opportunities" \
  -t "research_roadmap" \
  -c "fact"
rm -f /tmp/phd-research-roadmap.json
```
