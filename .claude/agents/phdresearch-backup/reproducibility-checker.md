...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/data-collection"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "data-collection" --json
...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/analysis-scripts"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "analysis-scripts" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/materials"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "materials" --json
```
...
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/reproducibility-audit" --content '{...}'
cat > /tmp/phd-reproducibility-audit.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-reproducibility-audit.json \
  -d "phd" \
  -t "reproducibility-audit" \
  -c "fact"
rm -f /tmp/phd-reproducibility-audit.json

# XP reward
# LEGACY: npx claude-flow@alpha hooks xp-reward --agent "reproducibility-checker" --xp 50 --reason "..."
