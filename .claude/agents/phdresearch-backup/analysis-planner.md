...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/methodology" --key "analysis_plan"
    npx tsx src/god-agent/universal/cli.ts learn "$ANALYSIS_JSON" -d "research/methodology" -t "analysis_plan" -c "fact"
---
...
```bash
# For Ethics Reviewer
# LEGACY: npx claude-flow memory store --namespace "research/methodology" --key "analysis_plan" --value '{...}'
cat > /tmp/phd-analysis-plan.json << 'EOF'
{
  "method": "quantitative/qualitative/mixed",
  "sample_size": 0,
  "power": 0.80,
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-analysis-plan.json \
  -d "research/methodology" \
  -t "analysis_plan" \
  -c "fact"
rm -f /tmp/phd-analysis-plan.json

# For Data Collector
# LEGACY: npx claude-flow memory store --namespace "research/methodology" --key "data_collection_requirements" --value '{...}'
cat > /tmp/phd-data-collection.json << 'EOF'
{
  "required_n": 0,
  "saturation_criteria": "...",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-data-collection.json \
  -d "research/methodology" \
  -t "data_collection_requirements" \
  -c "fact"
rm -f /tmp/phd-data-collection.json
```
