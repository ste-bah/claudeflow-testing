...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/meta/context"
    npx tsx src/god-agent/universal/cli.ts query -d "research/meta" -t "context" --json
  post: |
    echo "✅ Principles established and stored"
    # LEGACY: npx claude-flow memory store --namespace "research/meta" --key "principles"
    npx tsx src/god-agent/universal/cli.ts learn "$PRINCIPLES" -d "research/meta" -t "principles" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/meta" --key "principles" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/meta-principles.json \
  -d "research/meta" \
  -t "principles" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/meta" --key "quality_standards" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/quality-standards.json \
  -d "research/meta" \
  -t "quality_standards" \
  -c "fact"
