...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/contradictions/analysis"
    npx tsx src/god-agent/universal/cli.ts query -d "research/contradictions" -t "analysis" --json
  post: |
    echo "✅ Risk analysis complete with mitigation strategies"
    # LEGACY: npx claude-flow memory store --namespace "research/risks" --key "fmea_analysis"
    npx tsx src/god-agent/universal/cli.ts learn "$RISK_ANALYSIS" -d "research/risks" -t "fmea_analysis" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/risks" --key "fmea_analysis" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/risk-fmea-analysis.json \
  -d "research/risks" \
  -t "fmea_analysis" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/risks" --key "monitoring_plan" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/risk-monitoring-plan.json \
  -d "research/risks" \
  -t "monitoring_plan" \
  -c "fact"
