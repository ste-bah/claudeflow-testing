...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/analysis/patterns"
    npx tsx src/god-agent/universal/cli.ts query -d "research/analysis" -t "patterns" --json
  post: |
    echo "✅ Themes synthesized and stored"
    # LEGACY: npx claude-flow memory store --namespace "research/synthesis" --key "themes"
    npx tsx src/god-agent/universal/cli.ts learn "$THEMES_JSON" -d "research/synthesis" -t "themes" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/analysis/findings"
npx tsx src/god-agent/universal/cli.ts query -d "research/analysis" -t "findings" --json
```
...
```bash
# For Theory-Builder
# LEGACY: npx claude-flow memory store --namespace "research/synthesis" --key "themes" --value '{...}'
cat > /tmp/phd-themes.json << 'EOF'
{
  "themes": [
    {
      "id": 1,
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-themes.json \
  -d "research/synthesis" \
  -t "themes" \
  -c "fact"
rm -f /tmp/phd-themes.json

# For All Future Agents
# LEGACY: npx claude-flow memory store --namespace "research/synthesis" --key "thematic_framework" --value '{...}'
cat > /tmp/phd-thematic-framework.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-thematic-framework.json \
  -d "research/synthesis" \
  -t "thematic_framework" \
  -c "fact"
rm -f /tmp/phd-thematic-framework.json
```
