...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/ethics" --key "review_complete"
    npx tsx src/god-agent/universal/cli.ts learn "$ETHICS_REVIEW" -d "research/ethics" -t "review_complete" -c "fact"
---
...
```bash
# For Data Collector
# LEGACY: npx claude-flow memory store --namespace "research/ethics" --key "review_complete" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/ethics-review-complete.json \
  -d "research/ethics" \
  -t "review_complete" \
  -c "fact"
...
# For All Agents
# LEGACY: npx claude-flow memory store --namespace "research/ethics" --key "compliance_requirements" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/ethics-compliance-requirements.json \
  -d "research/ethics" \
