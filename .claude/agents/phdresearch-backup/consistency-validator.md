...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/qa" --key "consistency-report"
    npx tsx src/god-agent/universal/cli.ts learn "$CONSISTENCY_REPORT" -d "research/qa" -t "consistency-report" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/files/manifest"
npx tsx src/god-agent/universal/cli.ts query -d "research/files" -t "manifest" --json
```
...
```bash
# Retrieve structure
# LEGACY: STRUCTURE=$(npx claude-flow memory retrieve --key "research/structure/chapters")
STRUCTURE=$(npx tsx src/god-agent/universal/cli.ts query -d "research/structure" -t "chapters" --json)
...
# LEGACY: npx claude-flow memory store "consistency-report" '{...}' --namespace "research/qa"
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/consistency-report.json \
  -d "research/qa" \
