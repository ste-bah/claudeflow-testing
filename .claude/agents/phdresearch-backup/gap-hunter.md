...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/gaps" --key "comprehensive_analysis"
    npx tsx src/god-agent/universal/cli.ts learn "$GAP_ANALYSIS" -d "research/gaps" -t "comprehensive_analysis" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/gaps" --key "comprehensive_analysis" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/gap-comprehensive-analysis.json \
  -d "research/gaps" \
...
# LEGACY: npx claude-flow memory store --namespace "research/gaps" --key "research_agenda" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/gap-research-agenda.json \
  -d "research/gaps" \
  -t "research_agenda" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/gaps" --key "focus_areas" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/gap-focus-areas.json \
  -d "research/gaps" \
  -t "focus_areas" \
  -c "fact"
