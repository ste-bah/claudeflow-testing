...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/methodology/analysis_plan"
    npx tsx src/god-agent/universal/cli.ts query -d "research/methodology" -t "analysis_plan" --json
  post: |
    echo "✅ Methods section complete with full replicability"
    # LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "methodology"
    npx tsx src/god-agent/universal/cli.ts learn "$METHODOLOGY_JSON" -d "research/manuscript" -t "methodology" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/ethics/review_complete"
npx tsx src/god-agent/universal/cli.ts query -d "research/ethics" -t "review_complete" --json
...
# LEGACY: npx claude-flow memory retrieve --key "research/design/type"
npx tsx src/god-agent/universal/cli.ts query -d "research/design" -t "type" --json
```
...
```bash
# For Results Writer
# LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "methodology" --value '{...}'
cat > /tmp/phd-methodology.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-methodology.json \
  -d "research/manuscript" \
  -t "methodology" \
  -c "fact"
rm -f /tmp/phd-methodology.json
```
