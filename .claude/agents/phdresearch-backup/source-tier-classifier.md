...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/quality" --key "source-tiers"
    npx tsx src/god-agent/universal/cli.ts learn "$SOURCE_TIERS" -d "research/quality" -t "source-tiers" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/quality" --key "source-tiers" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/source-tiers.json \
  -d "research/quality" \
  -t "source-tiers" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/quality" --key "tier-citation-strategy" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/tier-citation-strategy.json \
  -d "research/quality" \
...
# LEGACY: npx claude-flow memory store --namespace "research/quality" --key "theory-quality-filter" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/theory-quality-filter.json \
  -d "research/quality" \
