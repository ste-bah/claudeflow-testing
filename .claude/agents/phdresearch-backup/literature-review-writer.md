...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/literature/comprehensive_synthesis"
    npx tsx src/god-agent/universal/cli.ts query -d "research/literature" -t "comprehensive_synthesis" --json
  post: |
    echo "✅ Literature Review section complete"
    # LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "literature_review"
    npx tsx src/god-agent/universal/cli.ts learn "$LIT_REVIEW_JSON" -d "research/manuscript" -t "literature_review" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/literature/themes"
npx tsx src/god-agent/universal/cli.ts query -d "research/literature" -t "themes" --json
...
# LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "literature_review" --value '{...}'
cat > /tmp/phd-literature-review.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-literature-review.json \
  -d "research/manuscript" \
  -t "literature_review" \
  -c "fact"
rm -f /tmp/phd-literature-review.json
```
