...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/gap-analysis"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "gap-analysis" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/results-section"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "results-section" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/discussion-section"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "discussion-section" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/theoretical-framework"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "theoretical-framework" --json

# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/objectives"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "objectives" --json
```
...
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/conclusion-section" --content '{...}'
cat > /tmp/phd-conclusion-section.json << 'EOF'
{
  "core_contributions": [
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-conclusion-section.json \
  -d "phd" \
  -t "conclusion-section" \
  -c "fact"
rm -f /tmp/phd-conclusion-section.json

# Mark research paper complete
# LEGACY: npx claude-flow@alpha memory store --key "phd/paper-complete" --content '{...}'
cat > /tmp/phd-paper-complete.json << 'EOF'
{
  "sections_complete": [
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-paper-complete.json \
  -d "phd" \
  -t "paper-complete" \
  -c "fact"
rm -f /tmp/phd-paper-complete.json

# XP reward (Note: hooks system still uses claude-flow for now)
# LEGACY: npx claude-flow@alpha hooks xp-reward --agent "conclusion-writer" --xp 50 --reason "..."
