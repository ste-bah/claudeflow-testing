...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/structure/chapters"
    npx tsx src/god-agent/universal/cli.ts query -d "research/structure" -t "chapters" --json
    # LEGACY: npx claude-flow memory retrieve --key "research/literature/synthesized"
    npx tsx src/god-agent/universal/cli.ts query -d "research/literature" -t "synthesized" --json
  post: |
    echo "✅ Introduction section complete"
    # LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "introduction"
    npx tsx src/god-agent/universal/cli.ts learn "$INTRODUCTION_JSON" -d "research/manuscript" -t "introduction" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/gaps/identified"
npx tsx src/god-agent/universal/cli.ts query -d "research/gaps" -t "identified" --json

# LEGACY: npx claude-flow memory retrieve --key "research/theoretical_framework/complete"
npx tsx src/god-agent/universal/cli.ts query -d "research/theoretical_framework" -t "complete" --json

# LEGACY: npx claude-flow memory retrieve --key "research/questions/refined"
npx tsx src/god-agent/universal/cli.ts query -d "research/questions" -t "refined" --json
```
...
# LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "introduction" --value '{...}'
cat > /tmp/phd-introduction.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-introduction.json \
  -d "research/manuscript" \
  -t "introduction" \
  -c "fact"
rm -f /tmp/phd-introduction.json

# For Methodology Writer
# LEGACY: npx claude-flow memory store --namespace "research/manuscript" --key "research_questions_final" --value '{...}'
cat > /tmp/phd-rqs-final.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-rqs-final.json \
  -d "research/manuscript" \
  -t "research_questions_final" \
  -c "fact"
rm -f /tmp/phd-rqs-final.json
```
