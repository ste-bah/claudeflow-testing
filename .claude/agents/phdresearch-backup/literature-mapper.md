...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/literature" --key "literature-map"
    npx tsx src/god-agent/universal/cli.ts learn "$LITERATURE_MAP" -d "research/literature" -t "literature-map" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/meta/ambiguities"
npx tsx src/god-agent/universal/cli.ts query -d "research/meta" -t "ambiguities" --json
...
# LEGACY: npx claude-flow memory store --namespace "research/literature" --key "literature-map" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/literature-map.json \
...
# LEGACY: npx claude-flow memory store --namespace "research/literature" --key "citation-network" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/citation-network.json \
  -d "research/literature" \
  -t "citation-network" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/literature" --key "theoretical-clusters" --value '{...}'
cat > /tmp/theoretical-clusters.json << 'EOF'
{
  "clusters": []
}
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/theoretical-clusters.json \
  -d "research/literature" \
...
# For All Agents
# LEGACY: npx claude-flow memory store --namespace "research/literature" --key "search-metadata" --value '{...}'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/search-metadata.json \
  -d "research/literature" \
  -t "search-metadata" \
  -c "fact"
