...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/models" --key "structural_models"
    npx tsx src/god-agent/universal/cli.ts learn "$MODELS_JSON" -d "research/models" -t "structural_models" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/synthesis/themes"
npx tsx src/god-agent/universal/cli.ts query -d "research/synthesis" -t "themes" --json
```
...
# LEGACY: npx claude-flow memory store --namespace "research/models" --key "structural_models" --value '{...}'
cat > /tmp/phd-structural-models.json << 'EOF'
{
  "primary_model": {
    "name": "...",
    "constructs": [],
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-structural-models.json \
  -d "research/models" \
  -t "structural_models" \
  -c "fact"
rm -f /tmp/phd-structural-models.json

# For Method-Designer
# LEGACY: npx claude-flow memory store --namespace "research/models" --key "estimation_requirements" --value '{...}'
cat > /tmp/phd-estimation-requirements.json << 'EOF'
{
  "estimator": "MLR",
  "missing_data": "FIML",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-estimation-requirements.json \
  -d "research/models" \
  -t "estimation_requirements" \
  -c "fact"
rm -f /tmp/phd-estimation-requirements.json
```
