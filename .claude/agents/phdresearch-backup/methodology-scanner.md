...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/theory/theoretical-frameworks"
    npx tsx src/god-agent/universal/cli.ts query -d "research/theory" -t "theoretical-frameworks" --json
  post: |
    echo "✅ Methodologies scanned and categorized - gaps identified"
    # LEGACY: npx claude-flow memory store --namespace "research/methods" --key "methodology-scan"
    npx tsx src/god-agent/universal/cli.ts learn "$METHODOLOGY_SCAN_JSON" -d "research/methods" -t "methodology-scan" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/literature/methodological-clusters"
npx tsx src/god-agent/universal/cli.ts query -d "research/literature" -t "methodological-clusters" --json
...
```bash
# For Gap Hunter
# LEGACY: npx claude-flow memory store --namespace "research/methods" --key "methodology-scan" --value '{...}'
cat > /tmp/phd-methodology-scan.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-methodology-scan.json \
  -d "research/methods" \
  -t "methodology-scan" \
  -c "fact"
rm -f /tmp/phd-methodology-scan.json

# For Discussion Writer
# LEGACY: npx claude-flow memory store --namespace "research/methods" --key "method-limitations" --value '{...}'
cat > /tmp/phd-method-limitations.json << 'EOF'
{
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-method-limitations.json \
  -d "research/methods" \
  -t "method-limitations" \
  -c "fact"
rm -f /tmp/phd-method-limitations.json
```
