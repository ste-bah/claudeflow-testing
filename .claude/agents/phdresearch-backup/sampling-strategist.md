...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/methods/research_design"
    npx tsx src/god-agent/universal/cli.ts query -d "research/methods" -t "research_design" --json
  post: |
    echo "✅ Sampling strategy created and stored"
    # LEGACY: npx claude-flow memory store --namespace "research/sampling" --key "recruitment_plan"
    npx tsx src/god-agent/universal/cli.ts learn "$SAMPLING_JSON" -d "research/sampling" -t "recruitment_plan" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/sampling" --key "recruitment_plan" --value '{...}'
cat > /tmp/phd-recruitment-plan.json << 'EOF'
{
  "studies": [
    {
      "id": "Study1",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-recruitment-plan.json \
  -d "research/sampling" \
  -t "recruitment_plan" \
  -c "fact"
rm -f /tmp/phd-recruitment-plan.json
```
