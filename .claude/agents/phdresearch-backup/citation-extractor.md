...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/synthesis/systematic-review"
    npx tsx src/god-agent/universal/cli.ts query -d "research/synthesis" -t "systematic-review" --json
  post: |
    echo "✅ Citations extracted - full APA format with URLs and page numbers"
    # LEGACY: npx claude-flow memory store --namespace "research/citations" --key "citation-database"
    npx tsx src/god-agent/universal/cli.ts learn "$CITATION_DATABASE" -d "research/citations" -t "citation-database" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/organization/context-tiers"
npx tsx src/god-agent/universal/cli.ts query -d "research/organization" -t "context-tiers" --json
...
# LEGACY: npx claude-flow memory store --namespace "research/citations" --key "citation-database" --value '...'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/citation-database.json \
...
# LEGACY: npx claude-flow memory store --namespace "research/citations" --key "theory-citations" --value '...'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/theory-citations.json \
  -d "research/citations" \
  -t "theory-citations" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/citations" --key "method-citations" --value '...'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/method-citations.json \
  -d "research/citations" \
  -t "method-citations" \
  -c "fact"
...
# LEGACY: npx claude-flow memory store --namespace "research/citations" --key "citation-quality" --value '...'
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/citation-quality.json \
  -d "research/citations" \
  -t "citation-quality" \
  -c "fact"
