...
# LEGACY: npx claude-flow@alpha memory retrieve --key "phd/literature-synthesis"
npx tsx src/god-agent/universal/cli.ts query -d "phd" -t "literature-synthesis" --json
...
```bash
# LEGACY: npx claude-flow@alpha memory store --key "phd/discussion-section" --content '{...}'
cat > /tmp/phd-discussion-section.json << 'EOF'
{
  "key_interpretations": [
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-discussion-section.json \
  -d "phd" \
  -t "discussion-section" \
  -c "fact"
rm -f /tmp/phd-discussion-section.json

# XP reward (Note: hooks system still uses claude-flow for now)
# LEGACY: npx claude-flow@alpha hooks xp-reward --agent "discussion-writer" --xp 50 --reason "..."
