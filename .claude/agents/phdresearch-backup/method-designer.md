...
hooks:
  pre: |
    ...
    # LEGACY: npx claude-flow memory retrieve --key "research/opportunities/gaps_and_questions"
    npx tsx src/god-agent/universal/cli.ts query -d "research/opportunities" -t "gaps_and_questions" --json
  post: |
    echo "✅ Research methods designed and stored"
    # LEGACY: npx claude-flow memory store --namespace "research/methods" --key "research_design"
    npx tsx src/god-agent/universal/cli.ts learn "$METHODS_JSON" -d "research/methods" -t "research_design" -c "fact"
---
...
# LEGACY: npx claude-flow memory store --namespace "research/methods" --key "research_design" --value '{...}'
cat > /tmp/phd-research-design.json << 'EOF'
{
  "studies": [
    {
      "id": "Study1",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-research-design.json \
  -d "research/methods" \
  -t "research_design" \
  -c "fact"
rm -f /tmp/phd-research-design.json

# For Instrument-Developer
# LEGACY: npx claude-flow memory store --namespace "research/methods" --key "measurement_specs" --value '{...}'
cat > /tmp/phd-measurement-specs.json << 'EOF'
{
  "constructs": [],
  "instruments": [],
  "adaptations_needed": []
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-measurement-specs.json \
  -d "research/methods" \
  -t "measurement_specs" \
  -c "fact"
rm -f /tmp/phd-measurement-specs.json
```
