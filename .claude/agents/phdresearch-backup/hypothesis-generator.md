...
hooks:
  ...
  post: |
    ...
    # LEGACY: npx claude-flow memory store --namespace "research/hypotheses" --key "testable_predictions"
    npx tsx src/god-agent/universal/cli.ts learn "$HYPOTHESES_JSON" -d "research/hypotheses" -t "testable_predictions" -c "fact"
---
...
# LEGACY: npx claude-flow memory retrieve --key "research/theory/testable_framework"
npx tsx src/god-agent/universal/cli.ts query -d "research/theory" -t "testable_framework" --json
...
```bash
# For Model-Architect
# LEGACY: npx claude-flow memory store --namespace "research/hypotheses" --key "testable_predictions" --value '{...}'
cat > /tmp/phd-testable-predictions.json << 'EOF'
{
  "hypotheses": [
    {
      "id": "H1",
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-testable-predictions.json \
  -d "research/hypotheses" \
  -t "testable_predictions" \
  -c "fact"
rm -f /tmp/phd-testable-predictions.json

# For Method-Designer
# LEGACY: npx claude-flow memory store --namespace "research/hypotheses" --key "measurement_plan" --value '{...}'
cat > /tmp/phd-measurement-plan.json << 'EOF'
{
  "instruments": [],
  "data_collection": {},
  "sample_specs": {}
...
EOF
npx tsx src/god-agent/universal/cli.ts learn --file /tmp/phd-measurement-plan.json \
  -d "research/hypotheses" \
  -t "measurement_plan" \
  -c "fact"
rm -f /tmp/phd-measurement-plan.json
```
