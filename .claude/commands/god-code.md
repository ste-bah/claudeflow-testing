...
3. **Initialize or restore batch state:**
   ...
   **If `-resume` flag AND batch state exists:**
   ```bash
   npx claude-flow@alpha memory retrieve --key "coding/batch/status"
   # Restore: TASK_FILES, COMPLETED_TASKS, CURRENT_INDEX
   ```

   **Otherwise, initialize fresh batch:**
   ```bash
   npx claude-flow@alpha memory store "coding/batch/status" '{
     "mode": "batch",
     "totalTasks": '${#TASK_FILES[@]}',
     "completedCount": 0,
     "currentIndex": 0,
   ...
   npx claude-flow@alpha memory store "coding/batch/task-list" '"'${TASK_FILES[*]}'"' --namespace "coding"
   npx claude-flow@alpha memory store "coding/batch/completed" '[]' --namespace "coding"
   ```
...
4. **Execute batch loop:**
   ...
   FOR each TASK_FILE in TASK_FILES (starting from CURRENT_INDEX):
     ...
     a. Update current task in memory:
        npx claude-flow@alpha memory store "coding/batch/current-task" '"'$TASK_FILE'"' --namespace "coding"

     b. Read task content:
        TASK_CONTENT=$(cat "$TASK_FILE")

     c. Store task as pipeline input:
        npx claude-flow@alpha memory store "coding/input/task" '"Implement: '$TASK_FILE' - '$TASK_CONTENT'"' --namespace "coding"
...
5. **On batch completion:**
   ...
   npx claude-flow@alpha memory store "coding/batch/status" '{
     "mode": "batch",
     "totalTasks": '${#TASK_FILES[@]}',
     "completedCount": '${#TASK_FILES[@]}',
     "currentIndex": '${#TASK_FILES[@]}',
...
npx claude-flow@alpha memory store "coding/input/task" '"$ARGUMENTS"' --namespace "coding"

# Initialize project context (if available)
npx claude-flow@alpha memory store "coding/context/project" '{"cwd": "'$(pwd)'", "initialized": true}' --namespace "coding"

# Initialize pipeline tracking
npx claude-flow@alpha memory store "coding/pipeline/status" '{"task": "$ARGUMENTS", "trajectoryId": "[trajectoryId]", "startTime": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "status": "running", "currentPhase": 1}' --namespace "coding"
```
...
## MEMORY RETRIEVAL
npx claude-flow memory retrieve --key "coding/understanding/prioritized-requirements"
...
npx claude-flow memory retrieve --key "coding/understanding/scope"
npx claude-flow memory retrieve --key "coding/understanding/context"
npx claude-flow memory retrieve --key "coding/understanding/feasibility"
...
## MEMORY RETRIEVAL
npx claude-flow memory retrieve --key "coding/exploration/technologies"
npx claude-flow memory retrieve --key "coding/understanding/context"
...
## MEMORY RETRIEVAL
npx claude-flow memory retrieve --key "coding/exploration/codebase-analysis"
npx claude-flow memory retrieve --key "coding/understanding/requirements"
...
## MEMORY RETRIEVAL
npx claude-flow memory retrieve --key "coding/architecture/design"
...
## MEMORY RETRIEVAL
npx claude-flow memory retrieve --key "coding/architecture/interfaces"
npx claude-flow memory retrieve --key "coding/architecture/data-models"
...
```bash
npx claude-flow@alpha memory store "coding/pipeline/result" '{"success": true, "phases_completed": 7, "totalAgents": 47, "trajectoryId": "[trajectoryId]", "endTime": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' --namespace "coding"
```
...
npx claude-flow@alpha memory store "coding/pipeline/feedback-status" '{"trajectoryId": "[trajectoryId]", "feedbackSubmitted": true, "quality": [calculated_quality], "timestamp": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'", "verified": true}' --namespace "coding"
```
...
BATCH_STATUS=$(npx claude-flow@alpha memory retrieve --key "coding/batch/status" 2>/dev/null)
COMPLETED_LIST=$(npx claude-flow@alpha memory retrieve --key "coding/batch/completed" 2>/dev/null)
...
```bash
npx claude-flow@alpha memory store "coding/batch/completed" '[...previous, "'$CURRENT_TASK'"]' --namespace "coding"

npx claude-flow@alpha memory store "coding/batch/status" '{
  "mode": "batch",
  "totalTasks": [TOTAL],
  "completedCount": [NEW_COUNT],
  "currentIndex": [NEW_INDEX],
  "startTime": "[original]",
  "status": "running",
  "lastCompletedTask": "'$CURRENT_TASK'",
  "lastCompletedTime": "'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"
}' --namespace "coding"
```
...
npx claude-flow@alpha memory store "coding/input/task" '""' --namespace "coding"
npx claude-flow@alpha memory store "coding/understanding/task-analysis" '{}' --namespace "coding"
