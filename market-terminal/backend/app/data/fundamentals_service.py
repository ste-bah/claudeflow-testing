...
def _fetch_yf_financials_direct(sym: str) -> dict[str, Any] | None:
    """Direct yfinance quarterly financial statements, bypassing YFinanceClient circuit.

    Mirrors the structure returned by YFinanceClient.get_financials() so it can
    be passed directly to _adapt_yfinance_financials().
    """
    try:
        ...
        return {
            "income_statement": _df_to_records(raw.get("income")),
            "balance_sheet": _df_to_records(raw.get("balance")),
            "cash_flow": _df_to_records(raw.get("cashflow")),
            "annual_income_statement": _df_to_records(raw.get("annual_income")),
            "annual_balance_sheet": _df_to_records(raw.get("annual_balance")),
            "annual_cash_flow": _df_to_records(raw.get("annual_cashflow")),
        }
    except Exception:
        return None


def _fetch_fast_info(sym: str) -> dict[str, Any] | None:
    """Direct yfinance fast_info fetch, bypassing the YFinanceClient circuit breaker.

    Returns a minimal dict with current_price and market_cap, or None on failure.
    Used as a last-resort fallback when YFinanceClient circuit is open.
    """
    try:
        import yfinance as yf
        fi = yf.Ticker(sym).fast_info
        price = fi.last_price if fi.last_price is not None else fi.previous_close
        mc = fi.market_cap
...
def _build_ttm(
    ...
    # FCF and EPS TTM require all 4 quarters to be valid.
    # A partial sum (e.g. 2 quarters) is misleading and blocks the Finnhub fallback.
    fcf_vals = [q.get("free_cash_flow") for q in recent_quarters]
    fcf = sum(fcf_vals) if len(fcf_vals) == 4 and all(v is not None for v in fcf_vals) else None

    eps_vals = [q.get("eps_diluted") for q in recent_quarters]
    eps = sum(eps_vals) if len(eps_vals) == 4 and all(v is not None for v in eps_vals) else None
    ...
    # Finnhub fallbacks for balance-derived ratios
    if debt_to_equity is None and market_data and market_data.get("debt_to_equity") is not None:
        debt_to_equity = market_data.get("debt_to_equity")
    if return_on_equity is None and market_data and market_data.get("roe_pct") is not None:
        return_on_equity = round(market_data["roe_pct"] / 100, 6)
    ...
    if (eps is None or eps == 0) and yf_data and yf_data.get("eps"):
        eps = yf_data.get("eps")
    if (eps is None or eps == 0) and market_data and market_data.get("eps"):
        eps = market_data.get("eps")

    # Shares fallback: compute from market_cap / price when other sources fail
    ...
    # FCF fallback: Finnhub cash_flow_per_share_ttm × shares
    if fcf is None and market_data and market_data.get("cash_flow_per_share") and shares is not None and shares > 0:
        fcf = int(market_data["cash_flow_per_share"] * shares)
...
async def get_fundamentals_data(symbol: str) -> dict[str, Any]:
    ...
    # Direct fast_info fallback when YFinanceClient circuit is open
    if yf_data is None:
        try:
            loop = asyncio.get_event_loop()
            fi_result = await loop.run_in_executor(None, _fetch_fast_info, symbol)
            if fi_result:
                yf_data = fi_result
                logger.info("fast_info fallback succeeded for %s: %s", symbol, fi_result)
        except Exception:
            logger.warning("fast_info fallback failed for %s", symbol, exc_info=True)
    ...
    if needs_financials:
        ...
        # If YFinanceClient circuit is open, try direct yfinance bypass
        if yf_fins is None:
            try:
                loop = asyncio.get_event_loop()
                yf_fins = await loop.run_in_executor(None, _fetch_yf_financials_direct, symbol)
                if yf_fins:
                    logger.info("Direct yfinance financials bypass succeeded for %s", symbol)
            except Exception:
                logger.warning("Direct yfinance financials bypass failed for %s", symbol, exc_info=True)
        ...
        if yf_fins:
            ...
            if (not has_valid_income) and id_yf:
                ...
                # EDGAR may return many sparse rows (e.g. quarters=12) with mostly None EPS/revenue.
                # Count only quarters with BOTH valid eps_diluted AND revenue for a fair comparison.
                valid_edgar = sum(
                    1 for r in (income_data or [])
                    if r.get("eps_diluted") is not None and r.get("revenue") is not None
                )
                valid_yf = sum(
                    1 for r in id_yf
                    if r.get("eps_diluted") is not None and r.get("revenue") is not None
                )
                if valid_yf > valid_edgar:
                    # YoY growth (revenue_growth_yoy, eps_growth_yoy) requires index i+4.
                    # Prefer YFinance data when it provides more dense quarterly history.
                    logger.info(
                        "Replacing sparse EDGAR income (%d valid / %d total) with YFinance (%d valid) for growth data",
                        valid_edgar, len(income_data or []), valid_yf,
                    )
                    income_data = id_yf
