 ...
 */
import type { FundamentalsTtm, FundamentalsQuarter, ShortInterest, AnalystRatings } from '../types/fundamentals';
import {
  isDisplayable,
  formatCurrency,
  formatPercent,
...
/** Phase 3: Short Interest Grid Panel */
function ShortInterestWidget({ data }: { readonly data: ShortInterest | null }) {
  if (!data) return null;
  return (
    <div className="pt-2 border-t border-terminal-border/30">
      <h4 className="text-text-primary text-xs font-semibold mb-2">Short Interest</h4>
      <div className="grid grid-cols-2 gap-2">
        <MetricCell label="Shares Short" value={data.sharesShort?.toLocaleString() ?? '--'} />
        <MetricCell label="Short Ratio" value={formatRatio(data.shortRatio)} />
        <MetricCell label="% of Float" value={formatPercent(data.percentOfFloat)} />
        <MetricCell label="Settled" value={data.settlementDate ?? '--'} />
      </div>
    </div>
  );
}

/** Phase 3: Analyst Ratings Consensus Bars */
function AnalystRatingsWidget({ data }: { readonly data: AnalystRatings | null }) {
  if (!data || data.totalAnalysts === 0) return null;

  const buyPct = (data.buy / data.totalAnalysts) * 100;
  const holdPct = (data.hold / data.totalAnalysts) * 100;
  const sellPct = (data.sell / data.totalAnalysts) * 100;

  return (
    <div className="pt-2 border-t border-terminal-border/30">
      <h4 className="text-text-primary text-xs font-semibold mb-2">
        Analyst Consensus: <span className="capitalize">{data.consensus.replace('_', ' ')}</span>
      </h4>

      <div className="w-full h-3 bg-terminal-container rounded overflow-hidden flex mb-2">
        {buyPct > 0 && <div style={{ width: `${buyPct}%` }} className="bg-accent-green h-full" />}
        {holdPct > 0 && <div style={{ width: `${holdPct}%` }} className="bg-text-muted h-full" />}
        {sellPct > 0 && <div style={{ width: `${sellPct}%` }} className="bg-accent-red h-full" />}
      </div>
      ...
      <div className="grid grid-cols-3 gap-2 mt-2 pt-2 border-t border-terminal-border/10">
        <MetricCell label="Low" value={formatCurrency(data.priceTargetLow)} />
        <MetricCell label="Average" value={formatCurrency(data.priceTargetMean)} />
        <MetricCell label="High" value={formatCurrency(data.priceTargetHigh)} />
      </div>
    </div>
  );
}
...
export default function Fundamentals({ symbol }: { symbol: string }) {
  const { data, shortInterest, analystRatings, loading, error } = useFundamentals(symbol);
      ...
      <div className="flex-1 overflow-y-auto space-y-3">
        {loading ? (
          ...
          <>
            ...
            <ShortInterestWidget data={shortInterest} />
            <AnalystRatingsWidget data={analystRatings} />
