 ...
 */
export class ClaudeCodeStepExecutor implements IStepExecutor {
  ...
  async execute(
    ...
    try {
      ...
      const duration = Date.now() - startTime;
      const err = error as Error & { timedOut?: boolean; partialOutput?: string };
   ...
   */
  private runClaude(prompt: string, timeoutMs: number): Promise<string> {
    return new Promise((resolve, reject) => {
      ...
      child.on('error', (err) => {
        ...
        if ((err as NodeJS.ErrnoException).code === 'ENOENT') {
          reject(new Error(
            `Claude Code CLI not found at "${this.claudeBinary}". ` +
      ...
      child.on('close', (code) => {
        ...
        if (killed) {
          const err = new Error(`Timed out after ${timeoutMs}ms`) as Error & {
            timedOut: boolean;
   ...
   */
  private assessQuality(output: string, _agentKey: string): number {
    ...
    if (errorPattern.test(output)) return 0.3;

    // Substantive output is generally higher quality
    if (output.length > 2000) return 0.8;
    if (output.length > 500) return 0.7;
