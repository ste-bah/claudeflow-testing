...
// ─────────────────────────────────────────────────────────────────────────────
const PHASE_QUALITY_THRESHOLDS = {
 ...
 */
export async function init(task, _bundle) {
    ...
    try {
        ...
        console.error(`[RLM] Init failed (non-fatal): ${err.message}`);
    }
 ...
 */
export async function next(sessionId, _bundle) {
    ...
    if (batchResponse.status !== 'complete' && batchResponse.batch[0]) {
        ...
        let sessionData = {};
        try {
            sessionData = JSON.parse(await fsP.readFile(`.god-agent/coding-sessions/${sessionId}.json`, 'utf-8'));
        }
        ...
        const phaseIndex = sessionData.currentPhaseIndex ?? 0;
        const batchIndex = sessionData.currentBatchIndex ?? 0;
        ...
        try {
            ...
            if (await rlm.load(rlmPath)) {
                ...
                const matchingNs = rlm.getNamespaces().filter(ns => namespaces.some(pattern => pattern.endsWith('/')
                    ? ns.startsWith(pattern)
                    : ns === pattern));
                if (matchingNs.length > 0) {
                    const phaseContext = await rlm.retrieve(prompt.substring(0, 500), matchingNs, 50000);
            ...
            console.error(`[RLM] Context injection failed (non-fatal): ${err.message}`);
        }
                ...
                if (candidates.length > 0) {
                    ...
                    if (vetted.length > 0) {
                        ...
                        for (let idx = 0; idx < toInject.length; idx++) {
                            ...
                            const quality = ep.metadata?.quality;
        ...
        try {
            ...
            console.error(`[LEANN] Semantic context injection failed (non-fatal): ${err.message}`);
        }
        ...
        try {
            ...
            if (failures.length > 0) {
                ...
                for (let i = 0; i < recentFailures.length; i++) {
                    ...
                    const pipelineId = f.context?.find((c) => c.startsWith('pipeline:'))?.replace('pipeline:', '');
        ...
        try {
            ...
            const currentBatchAgents = batches?.[phaseIndex]?.[batchIndex];
        ...
        if (/^phase-\d+-reviewer$/.test(agentKey)) {
            try {
                ...
                const { Verdict, VerdictConfidence, InvestigationTier, PHASE_NAMES } = await import('../core/pipeline/sherlock-phase-reviewer-types.js');
                const { RLMContextStore } = await import('../core/pipeline/rlm-context-store.js');
                const rlm = new RLMContextStore();
                const rlmPath = `.god-agent/rlm-context/${sessionId}.json`;
                if (await rlm.load(rlmPath)) {
                    const phaseOutputNs = rlm.getNamespaces().filter(ns => ns.startsWith(`coding/phase${phaseNum}/`));
        ...
        try {
            ...
            console.error(`[PROMPT-BUILDER] Fallback to basic context (${err.message})`);
        }
 ...
 */
export async function complete(sessionId, agentKey, options, _bundle) {
    ...
    const sessionData = JSON.parse(await fsP.readFile(`.god-agent/coding-sessions/${sessionId}.json`, 'utf-8'));
    ...
    try {
        ...
        try {
            ...
            console.error(`[FEEDBACK] Failed (non-fatal): ${fbErr.message}`);
        }
        ...
        console.error(`[TRAJECTORY] Creation failed (non-fatal): ${trajErr.message}`);
    }
    ...
    try {
        const updatedSession = JSON.parse(await fsP.readFile(sessionFilePath, 'utf-8'));
        if (!updatedSession.xp)
            updatedSession.xp = { total: 0, breakdown: [] };
    ...
    if (assessment.score > 0.50 && output.length > 100) {
        try {
            ...
            const ucmClient = getUCMClient();
            if (await ucmClient.isHealthy()) {
                const sessionPrompt = sessionData.batches?.[sessionData.currentPhaseIndex]?.[sessionData.currentBatchIndex]?.[0]?.prompt || agentKey;
    ...
    if (IMPLEMENTATION_AGENTS.includes(agentKey)) {
        try {
            ...
            console.error(`[LEANN] Indexing failed (non-fatal): ${err.message}`);
        }
    }
    ...
    try {
        ...
        console.error(`[RLM] Storage failed (non-fatal): ${err.message}`);
    }
    ...
    try {
        ...
        try {
            const batches = sessionData.batches;
            const phaseIdx = sessionData.currentPhaseIndex ?? 0;
            const batchIdx = sessionData.currentBatchIndex ?? 0;
            const currentBatchAgents = batches?.[phaseIdx]?.[batchIdx];
    ...
    try {
        ...
        console.error(`[CHECKPOINT] Creation failed (non-fatal): ${err.message}`);
    }
    ...
    if (phaseGate && assessment.score < phaseGate.threshold) {
        console.error(`[QUALITY-GATE] WARNING: ${agentKey} scored ${assessment.score.toFixed(2)} below ` +
    ...
    try {
        ...
        await fsP.writeFile(`.god-agent/progress/${sessionId}.json`, JSON.stringify({
            completed: agentKey, phase, timestamp: Date.now(),
            summary: outputSummary,
        }, null, 2));
        console.error(`[PROGRESS] ${agentKey} completed — files: ${outputSummary.filesCreated.length} created, ${outputSummary.filesModified.length} modified`);
    }
    catch (err) {
 ...
 */
export async function completeAndNext(sessionId, agentKey, options) {
...
function calculateXP(quality, _tier, session, agentKey) {
    const rewards = {};
    ...
    if (quality > 0.80)
        rewards.quality_bonus = 50;
    // First-Time Success: 75 XP (agent not previously completed in this session)
    const completedKeys = (session.completedAgents || []).map((a) => a.agentKey || a);
    ...
    if (quality > 0.80)
        rewards.pattern_contribution = 30;
 ...
 */
export async function resume(sessionId, _bundle) {
...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    switch (command) {
        case 'init':
            if (!arg1)
                fail('Usage: coding-pipeline-cli.ts init "<task>"');
            init(arg1)
        ...
        case 'next':
            if (!arg1)
                fail('Usage: coding-pipeline-cli.ts next <sessionId>');
            next(arg1)
                .then(data => console.log(JSON.stringify(data, null, 2)))
                .catch(e => { console.error('Error:', e); process.exit(1); });
            break;
        case 'complete': {
            if (!arg1 || !arg2)
                fail('Usage: coding-pipeline-cli.ts complete <sessionId> <agentKey> [--file <path>]');
        ...
        case 'complete-and-next': {
            if (!arg1 || !arg2)
                fail('Usage: coding-pipeline-cli.ts complete-and-next <sessionId> <agentKey> [--file <path>]');
            const file = parseFileFlag();
        ...
        case 'resume':
            if (!arg1)
                fail('Usage: coding-pipeline-cli.ts resume <sessionId>');
            resume(arg1)
                .then(data => console.log(JSON.stringify(data, null, 2)))
                .catch(e => { console.error('Error:', e); process.exit(1); });
            break;
        case 'status':
            if (!arg1)
                fail('Usage: coding-pipeline-cli.ts status <sessionId>');
...
//# sourceMappingURL=coding-pipeline-cli.js.map