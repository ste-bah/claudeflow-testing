...
export class PaperCombiner {
     ...
     */
    static INLINE_APPENDIX_THRESHOLD = 5000;
    /**
     * Maximum anchor length per spec
     */
    static MAX_ANCHOR_LENGTH = 50;
    /**
     * Default appendix structure per SPEC-FUNC-001 Section 2.6.5
     */
    static DEFAULT_APPENDICES = [
     ...
     */
    async combine(chapters, metadata) {
     ...
     */
    generateMetadata(slug, chapters) {
     ...
     */
    async handleAppendices(referencesChapterContent, appendices, outputDir) {
        // Calculate total appendix words
        const totalWords = appendices.reduce((sum, app) => sum + (app.wordCount || this.countWords(app.content || '')), 0);
     ...
     */
    generateTitlePage(metadata, chapters) {
        const totalWords = chapters.reduce((sum, c) => sum + c.wordCount, 0);
        const totalCitations = this.countUniqueCitations(chapters);
        const formattedDate = new Date(metadata.generatedDate).toLocaleDateString('en-GB', {
            day: 'numeric',
            month: 'long',
            year: 'numeric'
        });
...
**Chapters**: ${chapters.length}`;
     ...
     */
    validateCrossReferences(content, chapters) {
     ...
     */
    inferTitleFromChapters(chapters) {
     ...
     */
    static getDefaultAppendices() {
        return [...PaperCombiner.DEFAULT_APPENDICES];
    }
}
//# sourceMappingURL=paper-combiner.js.map