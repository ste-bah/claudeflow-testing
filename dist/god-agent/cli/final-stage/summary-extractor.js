...
export class SummaryExtractor {
     ...
     */
    async extractSummary(filePath, index) {
     ...
     */
    async extractSummaryWithTokenLimit(fileName, index, maxTokens) {
     ...
     */
    extractResearchQuestions(content) {
        const questions = [];
     ...
     */
    generateSummaryText(content, maxTokens) {
        ...
        const meaningfulParagraphs = paragraphs.filter(p => p.length > 100 &&
            !p.startsWith('|') &&
            !p.match(/^\s*[-*]\s/) &&
            !p.match(/^\d+\.\s/));
        let summary = '';
    ...
    generateExpectedFiles() {
        return Array.from({ length: SummaryExtractor.EXPECTED_FILE_COUNT }, (_, i) => `${String(i).padStart(2, '0')}-*.md`);
    }
    /**
     * Create a failed summary for files that couldn't be read
     */
    createFailedSummary(index, fileName, filePath) {
        return {
            index,
            fileName,
            filePath,
