...
export class FinalStageOrchestrator {
    ...
    summaryExtractor;
    semanticMapper;
    chapterWriter = null; // Created lazily with style profile
    paperCombiner;
    styleApplier;
     ...
     */
    constructor(basePath, slug) {
        // SECURITY (S002): Validate slug before use in paths
        const slugValidation = this.validateSlug(slug);
        if (!slugValidation.valid) {
            throw new FinalStageError(`Invalid research slug: ${slugValidation.error}`, 'SECURITY_VIOLATION', false, FinalStageError.getExitCode('SECURITY_VIOLATION'));
        }
    ...
    async execute(options = {}) {
        ...
        try {
            ...
            const chapters = await this.writeChapters(structure, mapping, summaries, style, options.sequential ?? false);
            ...
            return this.createSuccessResult(finalPaperPath, chapters, mapping, scanResult, allWarnings);
        }
        catch (error) {
    ...
    setState(newState) {
        ...
        if (!validTransitions.includes(newState)) {
            throw new FinalStageError(`Invalid state transition: ${this.state} -> ${newState}. Valid: ${validTransitions.join(', ')}`, 'CONSTITUTION_VIOLATION', false, FinalStageError.getExitCode('CONSTITUTION_VIOLATION'));
        }
        const previousState = this.state;
        this.state = newState;
        this.reportProgress(`State transition: ${previousState} -> ${newState}`, -1, -1);
    }
    /**
    ...
    async safeExecutePhase(phase, operation, errorCode, fallbackAction) {
        ...
        try {
            ...
            this.setState('FAILED');
            throw new FinalStageError(errorMessage, errorCode, false, FinalStageError.getExitCode(errorCode));
        }
    }
    /**
    ...
    async initialize(options) {
        ...
        if (!await this.directoryExists(this.researchDir)) {
            throw new FinalStageError(`Research directory not found: ${this.researchDir}`, 'NO_RESEARCH_DIR', false, FinalStageError.getExitCode('NO_RESEARCH_DIR'));
        }
        ...
        if (outputExists) {
            if (options.force) {
                ...
                throw new FinalStageError(`Output directory exists: ${this.outputDir}. Use --force to overwrite.`, 'OUTPUT_EXISTS', true, FinalStageError.getExitCode('OUTPUT_EXISTS'));
            }
        }
     ...
     */
    async pruneArchives(archiveDir, keepCount) {
        try {
     ...
     */
    updateTokenBudget(phase, tokens) {
        ...
        // Update totals
        const totalUsed = this.tokenBudget.phases.summaryExtraction.used +
        ...
        if (totalUsed > this.tokenBudget.limit) {
            throw new FinalStageError(`Token budget exceeded: ${totalUsed} > ${this.tokenBudget.limit}`, 'TOKEN_OVERFLOW', false, FinalStageError.getExitCode('TOKEN_OVERFLOW'));
        }
    }
    // ============================================
        ...
        if (jsonMatch && jsonMatch[1]) {
            try {
                ...
                if (parsed.researchTitle) {
                    this._researchQuery = parsed.researchTitle;
                }
                return {
                    locked: parsed.locked,
                    generatedAt: parsed.generatedAt || parsed.dateLocked || new Date().toISOString(),
                    totalChapters: chapters.length,
                    estimatedTotalWords: chapters.reduce((sum, ch) => sum + ch.wordTarget, 0),
                    chapters,
                    writerMapping: parsed.writerMapping || {},
                    researchTitle: parsed.researchTitle || this.slug
                };
            }
            catch (e) {
                if (e instanceof FinalStageError)
                    throw e;
                throw new FinalStageError(`Failed to parse chapter structure JSON: ${e instanceof Error ? e.message : String(e)}`, 'NO_SOURCES', false, FinalStageError.getExitCode('NO_SOURCES'));
            }
        }
     ...
     */
    extractKeywordsFromChapter(ch) {
        const keywords = [];
        const title = ch.title || '';
        const purpose = ch.purpose || '';
        // Extract from title (split on spaces, filter stopwords)
        const titleWords = title.toLowerCase().split(/\s+/).filter(w => w.length > 3 && !['the', 'and', 'for', 'with', 'from'].includes(w));
        keywords.push(...titleWords);
        // Extract from purpose
        const purposeWords = purpose.toLowerCase().split(/\s+/).filter(w => w.length > 4 && !['the', 'and', 'for', 'with', 'from', 'this', 'that'].includes(w)).slice(0, 10);
     ...
     */
    parseChapterStructureFromMarkdown(content) {
        const chapters = [];
     ...
     */
    async extractSummaries(scanResult) {
     ...
     */
    calculateSummaryTokens(summaries) {
     ...
     */
    async mapOutputsToChapters(chapters, summaries, threshold) {
        // Use SemanticMapper to compute chapter-to-source mappings
        const mappingResult = await this.semanticMapper.mapOutputsToChapters(chapters, summaries, {
            threshold,
     ...
     */
    async writeChapters(structure, mapping, summaries, style, sequential) {
        const chapters = [];
        ...
        for (let i = 0; i < totalChapters; i++) {
            ...
            const styledOutput = await this.styleApplier.applyStyle({
                ...
                style,
                chapterNumber: chapterDef.number
            });
            ...
            if (styledOutput.validation.violations.length > 0) {
                ...
                chapterOutput.warnings.push(...styledOutput.validation.violations.map(v => `Style: ${v.message}`));
            }
     ...
     */
    async combineChapters(chapters, structure) {
     ...
     */
    estimateCombinationTokens(chapters) {
     ...
     */
    async validateOutputs(chapters) {
        const warnings = [];
        for (const chapter of chapters) {
            // QA-001: Word count compliance must be 70%-130%
            const compliance = chapter.qualityMetrics.wordCountCompliance;
            if (compliance < 0.70) {
                warnings.push(`Chapter ${chapter.chapterNumber}: Word count ${(compliance * 100).toFixed(0)}% is below 70% of target`);
            }
            else if (compliance > 1.30) {
                warnings.push(`Chapter ${chapter.chapterNumber}: Word count ${(compliance * 100).toFixed(0)}% exceeds 130% of target`);
            }
            // Check citation count
            if (chapter.citations.length === 0) {
                warnings.push(`Chapter ${chapter.chapterNumber}: No citations found`);
            }
            // Check for style violations
            if (chapter.qualityMetrics.styleViolations > 0) {
                warnings.push(`Chapter ${chapter.chapterNumber}: ${chapter.qualityMetrics.styleViolations} style violations`);
            }
            // Check generation status
            if (chapter.generationStatus === 'failed') {
                warnings.push(`Chapter ${chapter.chapterNumber}: Generation failed`);
            }
            // Aggregate chapter warnings
            warnings.push(...chapter.warnings.map(w => `Chapter ${chapter.chapterNumber}: ${w}`));
        }
        // Cross-chapter validation
        const totalWords = chapters.reduce((sum, ch) => sum + ch.wordCount, 0);
        if (totalWords < 20000) {
            warnings.push(`Total word count (${totalWords}) is below minimum threshold of 20,000`);
        }
        return warnings;
    }
    // ============================================
     ...
     */
    createDryRunResult(_structure, scanResult, mapping, warnings) {
        return {
            success: true,
            dryRun: true,
            outputPath: null,
     ...
     */
    createSuccessResult(outputPath, chapters, mapping, scanResult, warnings) {
        const totalWords = chapters.reduce((sum, ch) => sum + ch.wordCount, 0);
        const totalCitations = chapters.reduce((sum, ch) => sum + ch.citations.length, 0);
        return {
            success: true,
     ...
     */
    createFailureResult(error, errors, warnings) {
        return {
            success: false,
            dryRun: false,
            outputPath: null,
