...
export class ProgressLogger {
    ...
    emitProgress(report) {
        ...
        this.currentPhase = report.phase;
        ...
        if (this.logFileHandle) {
            this.writeToLogFile('INFO', report.phase, report.message);
        }
    }
    /**
     ...
     */
    report(phase, message, current = -1, total = -1) {
        const report = {
            phase,
            message,
            current,
     ...
     */
    estimateRemaining(phaseProgress = this.lastPhaseProgress) {
        if (phaseProgress <= 0)
            return -1;
     ...
     */
    calculateOverallProgress(phaseProgress = this.lastPhaseProgress) {
        const milestone = PROGRESS_MILESTONES[this.currentPhase];
        if (!milestone)
            return 0;
        const phaseRange = milestone.end - milestone.start;
     ...
     */
    async initFileLogging(outputDir) {
        try {
     ...
     */
    createLogEntry(level, phase, message, data) {
        return {
     ...
     */
    async writeToLogFile(level, phase, message, data) {
        if (!this.logFileHandle)
            return;
        try {
            const entry = this.createLogEntry(level, phase, message, data);
