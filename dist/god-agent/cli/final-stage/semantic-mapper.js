...
export class SemanticMapper {
     ...
     */
    async mapOutputsToChapters(chapters, summaries, input) {
        ...
        const similarityMatrix = this.computeSimilarityMatrix(chapters, summaries, chapterKeywords, summaryKeywords);
        // Assign sources to chapters based on thresholds
        const mappings = this.assignSourcesToChapters(chapters, summaries, similarityMatrix, threshold, primaryThreshold);
        ...
        if (orphanedSources.length > 0) {
            fallbacksApplied += this.reassignOrphanedSources(mappings, orphanedSources, summaries, similarityMatrix, threshold);
        }
     ...
     */
    calculateSimilarity(chapter, summary, chapterKeywords, summaryKeywords) {
        // Compute keyword vectors if not provided
        const chapterKw = chapterKeywords ?? new Set(chapter.keywords.flatMap(kw => this.tokenize(kw)));
        const summaryKw = summaryKeywords ?? new Set(summary.keyTerms.flatMap(term => this.tokenize(term)));
        ...
        // Weighted fusion
        return (SemanticMapper.WEIGHTS.KEYWORD * keywordScore +
     ...
     */
    applyHeuristicFallback(chapterNumber) {
     ...
     */
    buildChapterKeywordVectors(chapters) {
     ...
     */
    buildSummaryKeywordVectors(summaries) {
        const vectors = new Map();
     ...
     */
    computeSimilarityMatrix(chapters, summaries, chapterKeywords, summaryKeywords) {
        const matrix = [];
        for (const chapter of chapters) {
            const row = [];
    ...
    questionMatch(chapter, summary) {
        ...
        const normalizeQ = (q) => q.toLowerCase().replace(/rq/i, 'q');
     ...
     */
    phaseHeuristic(chapter, summary) {
     ...
     */
    assignSourcesToChapters(chapters, summaries, matrix, threshold, primaryThreshold) {
        const mappings = [];
     ...
     */
    applyFallbackHeuristics(mappings, summaries) {
        let fallbacksApplied = 0;
     ...
     */
    findOrphanedSources(mappings, totalSources) {
     ...
     */
    reassignOrphanedSources(mappings, orphanedSources, summaries, matrix, _threshold) {
        let reassigned = 0;
     ...
     */
    computeQualityMetrics(mappings, _matrix) {
