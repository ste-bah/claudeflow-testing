 ...
 */
const UK_TO_US_SPELLINGS = Object.fromEntries(Object.entries(US_TO_UK_SPELLINGS).map(([us, uk]) => [uk, us]));
// ============================================
...
export class StyleApplier {
     ...
     */
    applyRegionalSpelling(content, region) {
     ...
     * @returns Transformed content
     */
    applyCitationFormat(content, format) {
        let result = content;
        if (format === 'APA') {
            // Normalize to APA format: (Author, Year) or (Author & Author, Year)
            // Convert (Smith 2020) to (Smith, 2020)
            result = result.replace(/\(([A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\s+(\d{4})\)/g, '($1, $2)');
            // Convert (Smith and Jones, 2020) to (Smith & Jones, 2020)
            result = result.replace(/\(([A-Z][a-z]+)\s+and\s+([A-Z][a-z]+),?\s*(\d{4})\)/gi, '($1 & $2, $3)');
            // Convert 3+ authors to et al.
            result = result.replace(/\(([A-Z][a-z]+)(?:,\s+[A-Z][a-z]+){2,},?\s*(\d{4})\)/g, '($1 et al., $2)');
        }
        else if (format === 'MLA') {
            // Normalize to MLA format: (Author page)
            // Convert (Smith, 2020, p. 45) to (Smith 45)
            result = result.replace(/\(([A-Z][a-z]+),?\s*\d{4},?\s*p\.\s*(\d+(?:-\d+)?)\)/g, '($1 $2)');
            // Convert (Smith, 2020) to (Smith) - MLA doesn't use year
            result = result.replace(/\(([A-Z][a-z]+),?\s*\d{4}\)/g, '($1)');
        }
            ...
            result = result.replace(/\(([A-Z][a-z]+(?:\s+et\s+al\.)?(?:\s*[&,]\s*[A-Z][a-z]+)*),?\s*(\d{4})\)/g, () => `[${citationNumber++}]`);
        }
        return result;
    }
    /**
     ...
     */
    validateCitationFormat(content, format) {
        const violations = [];
        if (format === 'APA') {
     ...
     * @returns Transformed content
     */
    applyHeadingStyle(content, style) {
     ...
     * @returns Transformed content
     */
    applyPunctuationStyle(content, preferences) {
        let result = content;
        ...
        if (preferences.oxfordComma) {
            ...
            result = result.replace(/(\w+),\s+(\w+)\s+(and|or)\s+(\w+)/gi, '$1, $2, $3 $4');
        }
        return result;
    }
    // ============================================
     ...
     */
    protectTechnicalContent(content) {
     ...
     */
    restoreTechnicalContent(content, placeholders) {
...
//# sourceMappingURL=style-applier.js.map