...
export class StyleProfileManager {
     ...
     */
    async createProfile(name, textSamples, options = {}) {
     ...
     */
    async updateProfile(profileId, additionalSamples) {
     ...
     */
    getSampleTexts(profileId, count = 3) {
        const profile = profileId
            ? this.store.profiles.get(profileId)
            : this.getActiveProfile();
        if (!profile) {
    ...
    extractRepresentativeSamples(texts, count) {
...
export function getStyleProfileManager(basePath) {
    if (!defaultManager) {
