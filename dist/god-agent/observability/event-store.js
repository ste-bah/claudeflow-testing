...
export class EventStore {
     ...
     */
    constructor(dbPath = '.god-agent/events.db', maxSize = BUFFER_LIMITS.EVENT_STORE) {
        ...
        try {
            ...
            const transaction = this.db.transaction((events) => {
                for (const event of events) {
                    this.insertStmt.run(event.id, event.timestamp, event.component, event.operation, event.status, event.durationMs ?? null, JSON.stringify(event.metadata), event.traceId ?? null, event.spanId ?? null);
                }
            });
            transaction(batch);
        }
     ...
     * @returns Promise resolving to events array
     */
    async queryDatabase(criteria, limit) {
