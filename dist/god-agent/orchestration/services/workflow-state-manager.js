...
export class WorkflowStateManager {
    ...
    async persistWorkflowState(workflowId, state) {
        try {
            ...
            if (this.verbose) {
                console.log(`[WorkflowStateManager] Persisted workflow ${workflowId} to ${finalPath}`);
            }
            ...
            throw new Error(`Failed to persist workflow state ${workflowId}: ${error.message}`);
        }
    }
    /**
     ...
     */
    async restoreWorkflowState(workflowId) {
        try {
            ...
            this.validateState(state);
            if (this.verbose) {
                console.log(`[WorkflowStateManager] Restored workflow ${workflowId}`);
            }
            return state;
        }
        catch (error) {
            ...
            console.error(`[WorkflowStateManager] Corrupted workflow state ${workflowId}: ${error.message}`);
            return null;
        }
    }
    /**
     ...
     * @private
     */
    async archiveCorruptedFile(workflowId) {
        try {
            const timestamp = Date.now();
            const sourcePath = path.join(this.storageDir, `${workflowId}.json`);
            const corruptedPath = path.join(this.storageDir, 'corrupted', `${workflowId}-${timestamp}.json`);
            await this.ensureDirectories();
            try {
                await fs.rename(sourcePath, corruptedPath);
                if (this.verbose) {
                    console.log(`[WorkflowStateManager] Archived corrupted file to ${corruptedPath}`);
                }
            }
            catch {
            ...
            console.error(`[WorkflowStateManager] Failed to archive corrupted file: ${error.message}`);
        }
    }
    /**
    ...
    async deleteWorkflowState(workflowId) {
        try {
            ...
            throw new Error(`Failed to delete workflow state ${workflowId}: ${error.message}`);
        }
    }
}
