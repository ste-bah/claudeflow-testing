...
import { PHASE_AGENT_MAPPING, PHASE_KEYWORDS, PHASE_CONFIDENCE_THRESHOLD } from '../constants.js';
/**
...
export class AgentRouter {
     ...
     */
    routeToAgent(taskDescription, options) {
        ...
        if (confidence < PHASE_CONFIDENCE_THRESHOLD) {
            console.warn(`[AgentRouter] Low confidence phase detection (${confidence.toFixed(2)}). ` +
     ...
     * @private
     */
    createFallbackRouting(taskDescription, timestamp, confidence) {
        return {
            taskDescription,
     ...
     * @private
     */
    async storeRoutingAmbiguity(taskDescription, detectedPhase, confidence) {
        ...
        console.warn(`[AgentRouter] Routing ambiguity: phase=${detectedPhase}, confidence=${confidence.toFixed(2)}`);
    }
    /**
