...
export class OrchestrationMemoryManager {
     ...
     */
    async wrapTask(taskFn, prompt, agentType, options) {
        ...
        try {
            ...
            if (this.config.enableAutoMemory) {
                ...
                const tags = []; // No specific tags - get all workflow knowledge
     ...
     */
    async storeTaskOutput(output, metadata) {
        try {
     ...
     */
    generateTags(findings, agentType, category) {
        const tags = new Set();
     ...
     */
    async injectContext(prompt, workflowDomain, tags) {
        try {
     ...
     */
    async submitFeedback(output, metadata) {
     ...
     */
    async logFeedbackFailure(entry) {
        try {
            const fs = await import('fs/promises');
            const path = await import('path');
...
//# sourceMappingURL=orchestration-memory-manager.js.map