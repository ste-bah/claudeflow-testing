...
import { GraphDimensionMismatchError, ZeroVectorError, InvalidVectorValueError, NotNormalizedError } from './errors.js';
/**
 ...
 */
export function isL2Normalized(vector, epsilon = L2_NORM_TOLERANCE) {
 ...
 */
export function validateFiniteValues(vector) {
    for (let i = 0; i < vector.length; i++) {
 ...
 */
export function normL2(vector, inPlace = false) {
    const norm = calculateNorm(vector);
    if (norm === 0) {
 ...
 */
export function assertDimensions(vector, expected = VECTOR_DIM, context = 'Vector validation') {
    ...
    if (!finiteCheck.valid) {
        throw new InvalidVectorValueError(finiteCheck.invalidPosition, finiteCheck.invalidValue, context);
    }
 ...
 * @throws InvalidVectorValueError if vector contains NaN or Infinity
 */
export function assertDimensionsOnly(vector, expected = VECTOR_DIM, context = 'Vector dimension check') {
    if (vector.length !== expected) {
        throw new GraphDimensionMismatchError(expected, vector.length, context);
    }
    const finiteCheck = validateFiniteValues(vector);
 ...
 */
export function createValidatedVector(values, context = 'Vector creation') {
...
//# sourceMappingURL=vector-validation.js.map