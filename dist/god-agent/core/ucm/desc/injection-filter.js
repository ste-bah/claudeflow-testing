...
export class InjectionFilter {
     ...
     */
    detectWorkflowCategory(taskContext) {
     ...
     */
    applyRecencyDecay(episode, category) {
     ...
     */
    isContentTypeMatch(episodeMetadata, taskContext) {
        const category = this.detectWorkflowCategory(taskContext);
        const episodeContentType = episodeMetadata.contentType;
     ...
     */
    isFileContextRelevant(episodeMetadata, taskContext) {
     ...
     */
    shouldInject(episode, similarity, taskContext) {
        const category = this.detectWorkflowCategory(taskContext);
...
export class EnhancedInjectionFilter extends InjectionFilter {
    ...
    constructor(outcomeTracker, confidenceCalculator, negativeExampleProvider) {
        super();
        this.outcomeTracker = outcomeTracker;
        this.confidenceCalculator = confidenceCalculator;
        this.negativeExampleProvider = negativeExampleProvider;
    }
    /**
     ...
     */
    async shouldInjectEnhanced(episode, similarity, taskContext) {
        ...
        const confidence = this.confidenceCalculator.calculate(similarity, stats.successRate, stats.outcomeCount, episode.createdAt, baseDecision.category);
        ...
        if (enhancedDecision.inject) {
            console.error(`[EnhancedInjectionFilter] Episode ${episode.episodeId}: inject=true, ` +
     ...
     */
    async shouldInjectBatch(episodes, taskContext) {
        ...
        const decisions = await Promise.all(episodes.map(async ({ episode, similarity }) => {
 ...
 */
export function createEnhancedInjectionFilter(outcomeTracker, confidenceCalculator, negativeExampleProvider) {
    return new EnhancedInjectionFilter(outcomeTracker, confidenceCalculator, negativeExampleProvider);
}
// ============================================================================
 ...
 */
export function toLegacyDecision(enhanced) {
    return {
 ...
 * }
 */
export function isEnhancedDecision(decision) {
    return 'confidence' in decision;
}
//# sourceMappingURL=injection-filter.js.map