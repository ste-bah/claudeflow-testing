...
export class TraceSummarizer {
     ...
     */
    async summarize(fullTrace, maxTokens) {
     ...
     * Implements: TASK-IDESC-RB-003 requirement
     */
    async extractKeyInsights(trace) {
        ...
        for (const line of lines) {
            ...
            const hasMarker = decisionMarkers.some(marker => trimmedLine.toLowerCase().includes(marker.toLowerCase()));
            if (hasMarker) {
     ...
     */
    async formatForInjection(trace, episodeContext) {
        // Graceful degradation
        if (!trace || trace.trim().length === 0) {
            return '';
        }
 ...
 */
export function createTraceSummarizer(config = {}) {
    return new TraceSummarizer(config);
}
//# sourceMappingURL=trace-summarizer.js.map