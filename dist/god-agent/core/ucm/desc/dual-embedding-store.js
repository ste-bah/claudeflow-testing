...
import { getDatabaseConnection, closeDatabaseConnection, LRUCache, createEpisodeSizeCalculator } from '../../database/index.js';
 ...
 */
export class DualEmbeddingStore {
     ...
     */
    async storeEpisode(input, queryEmbeddings, answerEmbeddings) {
        ...
        try {
            ...
            if (this.dao.exists(episodeId)) {
                throw new DESCStorageError(`Episode ${episodeId} already exists`, { episodeId });
            }
            ...
            throw new DESCStorageError(`Failed to store episode: ${error instanceof Error ? error.message : String(error)}`, { originalError: error });
        }
    }
    /**
    ...
    async getEpisode(episodeId) {
        ...
        try {
            ...
            throw new DESCStorageError(`Failed to retrieve episode: ${error instanceof Error ? error.message : String(error)}`, { episodeId, originalError: error });
        }
    }
    /**
    ...
    async getAllEpisodes() {
        ...
        try {
            ...
            throw new DESCStorageError(`Failed to retrieve all episodes: ${error instanceof Error ? error.message : String(error)}`, { originalError: error });
        }
    }
    /**
     ...
     */
    async deleteEpisode(_episodeId) {
        throw new DESCStorageError('RULE-016 VIOLATION: Episodes are append-only. DELETE operations are FORBIDDEN. ' +
            'Exception: Compaction with explicit human approval requires separate implementation.', { rule: 'RULE-016' });
    }
    /**
     ...
     */
    async clear() {
        throw new DESCStorageError('RULE-016 VIOLATION: Episodes are append-only. CLEAR operations are FORBIDDEN. ' +
    ...
    flush() {
        ...
        try {
            ...
            throw new DESCStorageError(`Failed to flush database: ${error instanceof Error ? error.message : String(error)}`, { originalError: error });
        }
    }
    /**
    ...
    close() {
        ...
        try {
            ...
            throw new DESCStorageError(`Failed to close store: ${error instanceof Error ? error.message : String(error)}`, { originalError: error });
        }
    }
    /**
...
//# sourceMappingURL=dual-embedding-store.js.map