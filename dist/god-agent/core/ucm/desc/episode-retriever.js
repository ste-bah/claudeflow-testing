...
export class EpisodeRetriever {
    ...
    options;
    filter;
    constructor(store, options, filter) {
     ...
     */
    async retrieve(queryChunks, queryEmbeddings, options, taskContext) {
        try {
            // Validate input
            ...
            const episodeScores = this.computeEpisodeScores(queryEmbeddings, episodes, retrievalOptions.threshold);
            ...
            throw new DESCRetrievalError(`Retrieval failed: ${error instanceof Error ? error.message : String(error)}`, { originalError: error });
        }
    }
    /**
     * Compute episode scores using RULE-069: all-to-all comparison
     */
    computeEpisodeScores(queryEmbeddings, episodes, threshold) {
        const results = [];
    ...
    cosineSimilarity(a, b) {
        if (a.length !== b.length) {
            throw new DESCRetrievalError('Embedding dimensions must match', { dimA: a.length, dimB: b.length });
        }
     ...
     */
    validateRetrievalInput(queryChunks, queryEmbeddings) {
        ...
        if (queryChunks.length !== queryEmbeddings.length) {
            throw new DESCRetrievalError('Query chunks and embeddings length must match', {
        ...
        for (let i = 0; i < queryEmbeddings.length; i++) {
            ...
            if (!(embedding instanceof Float32Array) || embedding.length === 0) {
                throw new DESCRetrievalError(`Invalid embedding at index ${i}`, { index: i });
            }
            // Check for invalid values
            if (embedding.some(v => !Number.isFinite(v))) {
                throw new DESCRetrievalError(`Embedding at index ${i} contains non-finite values`, { index: i });
            }
        }
    }
    /**
...
//# sourceMappingURL=episode-retriever.js.map