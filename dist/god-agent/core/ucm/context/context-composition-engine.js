...
export class ContextCompositionEngine {
    ...
    dependencyTracker;
    pinningManager;
    descPriorCache = new Map();
    archivedAgents = new Map();
    constructor(initialPhase = 'research', maxPinnedTokens = 2000) {
     ...
     */
    compose(options) {
        const { targetAgent, contextWindow, phase, includeDependencies = true, maxDescPrior = 2, priorityWeights = {
            pinned: 10,
            descPrior: 8,
            active: 5,
            dependency: 6,
        ...
        const activeTier = this.composeActiveTier(targetAgent, includeDependencies, remainingBudget, priorityWeights);
     ...
     */
    getContextForAgent(agentId, contextWindow, phase) {
        return this.compose({
            targetAgent: agentId,
            contextWindow,
            phase,
     ...
     */
    composeDescPriorTier(targetAgent, maxPrior, budget) {
        if (!targetAgent) {
            return {
                tier: 'desc-prior',
                agents: [],
     ...
     * @returns Active tier
     */
    composeActiveTier(targetAgent, includeDependencies, budget, weights) {
        ...
        for (const agentId of orderedIds) {
            ...
            if (!entry)
                continue;
            if (totalTokens + entry.tokenCount > budget)
                break;
     ...
     */
    getOrderedWithDependencies(targetAgent, availableAgents) {
        const dependencies = this.dependencyTracker.getTransitiveDependencies(targetAgent);
        const relevant = availableAgents.filter(id => id === targetAgent || dependencies.has(id));
     ...
     */
    addToWindow(agentId, content, tokenCount) {
    ...
    addDependency(dependent, dependency) {
        ...
        if (windowEntry) {
            this.pinningManager.recordCrossReference(dependency, windowEntry.content, windowEntry.tokenCount);
        }
    }
    /**
     ...
     */
    pin(agentId, content, tokenCount, reason = PinReason.Manual) {
...
//# sourceMappingURL=context-composition-engine.js.map