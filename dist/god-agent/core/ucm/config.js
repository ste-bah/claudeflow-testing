...
// ============================================================================
export const DEFAULT_BREAK_PATTERNS = [
...
export const DEFAULT_TOKEN_DEFAULTS = {
 ...
 */
export const DEFAULT_UCM_CONFIG = {
    daemon: {
...
export function validateConfig(config) {
    ...
    if (config.daemon.startupTimeout < 0) {
        throw new InvalidConfigError('daemon.startupTimeout', config.daemon.startupTimeout, 'positive number');
    }
    // Validate DESC config
    if (config.desc.threshold < 0 || config.desc.threshold > 1) {
        throw new InvalidConfigError('desc.threshold', config.desc.threshold, 'number between 0 and 1');
    }
    if (config.desc.maxEpisodes < 1) {
        throw new InvalidConfigError('desc.maxEpisodes', config.desc.maxEpisodes, 'positive integer');
    }
    // Validate embedding config
    if (config.embedding.mode !== 'http-proxy') {
        throw new InvalidConfigError('embedding.mode', config.embedding.mode, 'http-proxy');
    }
    if (!config.embedding.httpEndpoint) {
        throw new MissingConfigError('embedding.httpEndpoint');
    }
    if (config.embedding.dimension < 1) {
        throw new InvalidConfigError('embedding.dimension', config.embedding.dimension, 'positive integer');
    }
    // Validate token management
    if (config.tokenManagement.defaults.tokensPerWord < 1) {
        throw new InvalidConfigError('tokenManagement.defaults.tokensPerWord', config.tokenManagement.defaults.tokensPerWord, 'number >= 1');
    }
    // Validate summarization budget allocation
    const sumBudget = config.tokenManagement.summarization;
    const totalBudget = sumBudget.citationsBudget +
        sumBudget.keyFindingsBudget +
        sumBudget.methodologyBudget +
        sumBudget.resultsBudget;
    if (Math.abs(totalBudget - 1.0) > 0.001) {
        throw new InvalidConfigError('tokenManagement.summarization budget allocation', totalBudget, 'sum to 1.0 (100%)');
    }
}
/**
 * Load and validate configuration
 */
export function loadAndValidateConfig(overrides) {
