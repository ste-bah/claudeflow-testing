...
export class SummarizationTrigger {
    config;
    summaryConfig;
    constructor(config = DEFAULT_UCM_CONFIG.tokenManagement.defaults, summaryConfig = DEFAULT_UCM_CONFIG.tokenManagement.summarization) {
        this.config = config;
     ...
     */
    calculateUrgency(percentUsed, severity) {
     ...
     */
    getRecommendedAction(urgency, context) {
        switch (urgency) {
            case 'critical':
     ...
     */
    isEmergency(currentUsage, budgetAllocated) {
     ...
     * Between 70-80%
     */
    isProactiveRecommended(currentUsage, budgetAllocated) {
        const percentUsed = (currentUsage / budgetAllocated) * 100;
     ...
     */
    getNextThreshold(currentUsage, budgetAllocated) {
        const percentUsed = (currentUsage / budgetAllocated) * 100;
        // Find next threshold
        const thresholds = [
            { percent: this.config.summarizationThreshold ?? 0.7, severity: 'normal' },
            { percent: 80, severity: 'high' },
...
//# sourceMappingURL=summarization-trigger.js.map