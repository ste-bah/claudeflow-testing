 ...
 */
export function calculatePhaseLScore(phase, phaseResult, config) {
 ...
 */
export async function validatePhaseWithSherlock(validator, phase, phaseResult, retryCount = 0, config) {
    if (!validator) {
        if (config?.verbose) {
    ...
    if (!phaseNumber) {
        throw new SherlockValidatorError('PHASE_INVALID', `Invalid phase: ${phase}`, phase);
    }
    ...
    if (config?.verbose) {
        process.stderr.write(`[SHERLOCK-VALIDATOR] Validating phase ${phase} (${phaseNumber}) ` +
    ...
    if (config?.verbose) {
        process.stderr.write(`[SHERLOCK-VALIDATOR] Phase ${phase} validation result: ` +
 ...
 */
export function handleSherlockGuiltyVerdict(validationResult, phase, config) {
    const remediations = [];
    // Add phase context
    ...
    if (config?.verbose) {
        process.stderr.write(`[SHERLOCK-VALIDATOR] GUILTY verdict for phase ${phase}: ` +
 ...
 */
export function getPhaseThreshold(phase) {
...
//# sourceMappingURL=coding-pipeline-sherlock-validator.js.map