 ...
 */
import { PipelineEventType, DEFAULT_STEP_TIMEOUT, DEFAULT_PIPELINE_TIMEOUT, DEFAULT_MIN_QUALITY, generatePipelineId, generatePipelineTrajectoryId, generateStepTrajectoryId, calculateOverallQuality, } from './dai-002-types.js';
...
import { PipelineExecutionError, PipelineTimeoutError, QualityGateError, AgentSelectionError, } from './pipeline-errors.js';
...
export class PipelineExecutor {
    ...
    async execute(pipeline, options = {}) {
        ...
        try {
            ...
            if (this.config.enableLearning && this.dependencies.reasoningBank) {
                await this.providePipelineFeedback(trajectoryId, overallQuality, 'completed', pipeline.name);
            }
            ...
            this.log(`[Pipeline ${pipelineId}] Completed '${pipeline.name}' in ${totalDuration}ms ` +
            ...
            if (this.config.enableLearning && this.dependencies.reasoningBank) {
                await this.providePipelineFeedback(trajectoryId, overallQuality, 'failed', pipeline.name, currentError.message);
            }
            ...
            this.log(`[Pipeline ${pipelineId}] FAILED '${pipeline.name}' after ${totalDuration}ms: ${currentError.message}`);
            return {
                pipelineId,
                pipelineName: pipeline.name,
                status: 'failed',
    ...
    async executeStep(params) {
        const { step, stepIndex, pipeline, pipelineId, trajectoryId, defaultMinQuality, initialInput, remainingTimeout, } = params;
        ...
        if (step.agentKey) {
            ...
            try {
                ...
                this.log(`[Pipeline ${pipelineId}] Step ${stepIndex}: DAI-001 selected '${agentKey}' ` +
                    `for: "${step.taskDescription.substring(0, 50)}..."`);
            }
            catch (error) {
                throw new AgentSelectionError('No suitable agent found for task', {
                    pipelineId,
                    stepIndex,
                    taskDescription: step.taskDescription,
            ...
            throw new PipelineExecutionError(`Step ${stepIndex} has neither agentKey nor taskDescription`, {
                pipelineId,
                pipelineName: pipeline.name,
                stepIndex,
                agentKey: 'unknown',
        ...
        this.log(`[Pipeline ${pipelineId}] Step ${stepIndex}: Starting '${agentKey}' ` +
            `(${stepIndex + 1}/${pipeline.agents.length})`);
        // 3. Calculate step timeout
        const stepTimeout = Math.min(step.timeout ?? DEFAULT_STEP_TIMEOUT, remainingTimeout);
        // 4. Execute with timeout
        let executionResult;
        try {
            executionResult = await this.executeWithTimeout(agentKey, builtPrompt.prompt, stepTimeout, pipelineId, stepIndex);
        }
        catch (error) {
            if (error instanceof PipelineTimeoutError) {
                throw error;
            }
            throw new PipelineExecutionError(`Step ${stepIndex} (${agentKey}) failed: ${error.message}`, {
                pipelineId,
                pipelineName: pipeline.name,
                stepIndex,
                agentKey,
        ...
        const storeResult = this.memoryCoordinator.storeStepOutput({ ...step, agentKey }, stepIndex, pipelineId, executionResult.output, agentKey);
        this.emitEvent({
        ...
        this.log(`[Pipeline ${pipelineId}] Step ${stepIndex}: Completed '${agentKey}' ` +
     ...
     */
    async executeWithTimeout(agentKey, prompt, timeout, pipelineId, stepIndex) {
     ...
     */
    createTimeoutPromise(timeout, pipelineId, stepIndex, agentKey) {
     ...
     */
    assessQuality(output, providedQuality) {
    ...
    async provideStepFeedback(trajectoryId, quality, agentKey, stepIndex) {
        ...
        try {
            ...
            this.log(`[Feedback] Warning: Failed to provide step feedback: ${error.message}`);
        }
    }
    /**
    ...
    async providePipelineFeedback(trajectoryId, quality, status, pipelineName, errorMessage) {
        ...
        try {
            ...
            this.log(`[Feedback] Warning: Failed to provide pipeline feedback: ${error.message}`);
        }
    }
    /**
    ...
    emitEvent(event) {
        if (this.config.onEvent) {
            try {
                ...
                this.log(`[Event] Warning: Event handler error: ${error.message}`);
            }
        }
    }
    /**
 ...
 */
export function createPipelineExecutor(dependencies, config) {
