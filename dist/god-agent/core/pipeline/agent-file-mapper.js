...
import { PHASE_ORDER, SHERLOCK_AGENTS, CRITICAL_AGENTS, } from './types.js';
import { CODING_PIPELINE_MAPPINGS, buildPipelineDAG, getAgentByKey, getPhaseExecutionOrder, } from './coding-pipeline-dag-builder.js';
// ═══════════════════════════════════════════════════════════════════════════
// PATH RESOLUTION
// ═══════════════════════════════════════════════════════════════════════════
/**
 ...
 */
export function getAgentFilePath(agentKey) {
 ...
 */
export function agentFileExists(agentKey) {
 ...
 */
export async function getAgentPromptContent(agentKey) {
 ...
 */
export function extractAgentInstructions(content) {
...
export function buildPipelineExecutionPlan() {
    ...
    for (let phaseIdx = 0; phaseIdx < PHASE_ORDER.length; phaseIdx++) {
        ...
        for (const agentKey of phaseAgents) {
            ...
            if (!mapping)
                continue;
            const isSherlockReviewer = SHERLOCK_AGENTS.includes(agentKey);
 ...
 */
export async function generateTaskPrompt(agentKey, taskDescription, memoryContext = {}, stepInfo) {
    const mapping = getAgentByKey(agentKey);
    if (!mapping) {
 ...
 */
export async function generateSherlockPrompt(reviewerKey, phase, phaseNumber, taskDescription) {
...
//# sourceMappingURL=agent-file-mapper.js.map