 ...
 */
const PHASE_QUALITY_REQUIREMENTS = {
...
export class PhDPipelineBridge extends PipelineBridge {
     ...
     */
    buildQualityGate(agentConfig, loadedDef) {
        const gates = [];
        // Output requirements
    ...
    getPhDStatistics() {
        ...
        const writingAgents = this.config.agents.filter(a => a.phase === WRITING_PHASE_ID).length;
        const qaAgents = this.config.agents.filter(a => a.phase === QA_PHASE_ID).length;
        // Estimate total citations needed
        let estimatedCitations = 0;
        for (const agent of this.config.agents) {
            const req = CITATION_REQUIREMENTS[agent.phase];
            if (req)
                estimatedCitations += req;
        }
        return {
            ...base,
            writingAgents,
 ...
 */
export function createPhDPipelineBridge(config, registry = null) {
...
//# sourceMappingURL=phd-pipeline-bridge.js.map