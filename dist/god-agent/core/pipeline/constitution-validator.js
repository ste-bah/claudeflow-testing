 ...
 */
import { SherlockVerdict } from './types.js';
...
export class ConstitutionValidator {
    ...
    validateFileLength(content, maxLines) {
    ...
    validateFunctionLength(content) {
        ...
        return this.createBlockViolationResult(violations, 'CONST-FUNC-001', 'Function Length Limit', this.maxLinesPerFunction, 'function');
    }
    /** Validate class lengths against constitution limit */
    validateClassLength(content) {
        const violations = this.findBraceBlockViolations(content, [/^\s*(?:export\s+)?(?:abstract\s+)?class\s+(\w+)/], this.maxLinesPerClass);
        return this.createBlockViolationResult(violations, 'CONST-CLASS-001', 'Class Length Limit', this.maxLinesPerClass, 'class');
    }
    findBraceBlockViolations(content, patterns, maxLines) {
    ...
    createBlockViolationResult(violations, ruleId, ruleName, limit, type) {
    ...
    validateParameterCount(content) {
        const violations = [];
    ...
    validate(content, filePath) {
        ...
        const evidence = validations.filter(v => !v.passed).map(v => ({
...
export function createConstitutionValidator() {
