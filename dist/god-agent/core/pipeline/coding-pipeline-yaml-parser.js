 ...
 */
export function collectMultilineValue(ctx) {
...
export function parseYAML(yaml) {
    ...
    for (let idx = 0; idx < lines.length; idx++) {
        ...
        if (currentArray !== null && line.match(/^\s+-\s+(.+)$/)) {
            ...
            if (m)
                currentArray.push(m[1].replace(/^["']|["']$/g, ''));
            continue;
        }
        ...
        if (currentObject && objectKey && line.match(/^\s+\w+:\s*\|$/)) {
            ...
            if (m)
                currentObject[m[1]] = collectMultilineValue({ lines, startIndex: idx + 1 });
            continue;
        }
        // Nested object value
        if (currentObject && objectKey && line.match(/^\s+\w+:\s*.+$/)) {
            const m = line.match(/^\s+(\w+):\s*(.+)$/);
            if (m)
                currentObject[m[1]] = m[2].replace(/^["']|["']$/g, '');
            continue;
        }
        // End array
...
//# sourceMappingURL=coding-pipeline-yaml-parser.js.map