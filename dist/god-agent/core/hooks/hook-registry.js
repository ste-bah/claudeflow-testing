 ...
 */
import { REQUIRED_HOOKS, DEFAULT_PRIORITIES, HookError, HookErrorCode } from './types.js';
...
export class HookRegistry {
    ...
    preToolUseHooks = new Map();
    /** Map of postToolUse hooks by ID */
    postToolUseHooks = new Map();
    /**
    ...
    register(hook) {
        ...
        if (this.initialized) {
            throw new HookError(HookErrorCode.REGISTRATION_AFTER_INIT, `Cannot register hook '${hook.id}' after initialization. ` +
                `Hooks must be registered at daemon startup per CONSTITUTION RULE-032.`, { hookId: hook.id, hookType: hook.type });
        }
        ...
        if (targetMap.has(hook.id) || this.getOtherMap(hook.type).has(hook.id)) {
            throw new HookError(HookErrorCode.DUPLICATE_HOOK_ID, `Hook with ID '${hook.id}' is already registered.`, { hookId: hook.id, hookType: hook.type });
        }
    ...
    unregister(hookId) {
        if (this.initialized) {
            throw new HookError(HookErrorCode.REGISTRATION_AFTER_INIT, `Cannot unregister hook '${hookId}' after initialization.`, { hookId });
        }
    ...
    initialize() {
        ...
        for (const requiredId of this.requiredHooks) {
            const found = this.preToolUseHooks.has(requiredId) ||
        ...
        if (missingHooks.length > 0) {
            throw new HookError(HookErrorCode.REQUIRED_HOOK_MISSING, `Required hooks not registered: [${missingHooks.join(', ')}]. ` +
                `All required hooks must be registered at daemon startup per CONSTITUTION RULE-032.`, { missingHooks, requiredHooks: [...this.requiredHooks] });
        }
        // Mark as initialized
     ...
     */
    getHooksForTool(hooks, toolName) {
        return Array.from(hooks.values())
            ...
            if (hook.toolName && toolName && hook.toolName !== toolName)
                return false;
            return true;
        })
 ...
 */
export function _resetHookRegistryForTesting() {
    if (instance) {
