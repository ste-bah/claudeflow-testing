 ...
 */
import { CompressionTier, TIER_CONFIGS, TIER_HIERARCHY, DEFAULT_COMPRESSION_CONFIG, CompressionError, TierTransitionError, getTierForHeatScore, isValidTransition, } from './compression-types.js';
import { encodeFloat16, decodeFloat16, encodePQ8, decodePQ8, encodePQ4, decodePQ4, encodeBinary, decodeBinary, trainPQCodebook, trainBinaryThresholds, uint16ToUint8, uint8ToUint16, calculateReconstructionError, } from './compression-utils.js';
/**
...
export class CompressionManager {
    ...
    originalVectors = new Map(); // For error measurement
     ...
     */
    store(vectorId, vector) {
        if (vector.length !== this.config.dimension) {
            throw new CompressionError(`Dimension mismatch: expected ${this.config.dimension}, got ${vector.length}`, 'DIMENSION_MISMATCH');
        }
    ...
    decompress(embedding) {
        switch (embedding.tier) {
            case CompressionTier.HOT:
                ...
                return new Float32Array(embedding.data.buffer.slice(embedding.data.byteOffset, embedding.data.byteOffset + embedding.data.byteLength));
            case CompressionTier.WARM:
     ...
     */
    transitionTier(vectorId, targetTier) {
        const embedding = this.embeddings.get(vectorId);
        if (!embedding) {
     ...
     */
    measureCompressionError(vectorId) {
        const embedding = this.embeddings.get(vectorId);
