 ...
 */
export async function launchDaemon(agentDbPath = '.agentdb', options = {}) {
 ...
 */
export async function stopDaemon(agentDbPath = '.agentdb') {
    const fullAgentDbPath = path.isAbsolute(agentDbPath)
        ? agentDbPath
        : path.join(process.cwd(), agentDbPath);
    const pidFile = await readPidFile(fullAgentDbPath);
 ...
 * Get daemon status
 */
export async function getDaemonStatus(agentDbPath = '.agentdb') {
    const fullAgentDbPath = path.isAbsolute(agentDbPath)
        ? agentDbPath
        : path.join(process.cwd(), agentDbPath);
    const pidFile = await readPidFile(fullAgentDbPath);
 ...
 */
export async function ensureDaemonRunning(agentDbPath = '.agentdb', options = {}) {
...
// ==================== Helper Functions ====================
async function findDaemonScript() {
...
async function readPidFile(agentDbPath) {
...
async function cleanupPidFile(agentDbPath) {
    const pidPath = path.join(agentDbPath, 'memory-server.pid');
    try {
