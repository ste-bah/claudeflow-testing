...
export class WeightManager {
     ...
     */
    constructor(basePath = '.agentdb/gnn/weights', checkpointConfig, autoLoad = true) {
     ...
     */
    initializeWeights(layerId, config) {
    ...
    async saveWeights(layerId) {
        ...
        withRetrySync(() => writeFileSync(filePath, Buffer.from(buffer)), { operationName: 'WeightManager.saveWeights.binary' });
        // Write metadata with retry (RULE-072)
        withRetrySync(() => writeFileSync(metadataPath, JSON.stringify(metadata, null, 2)), { operationName: 'WeightManager.saveWeights.metadata' });
     ...
     */
    async saveWeightsAtomic(layerId) {
        const weights = this.weights.get(layerId);
        if (!weights || weights.length === 0) {
            throw new Error(`No weights found for layer: ${layerId}`);
        }
     ...
     */
    async verifyWeightFile(filePath, expectedChecksum) {
        try {
     ...
     */
    validateWeights(weights, expectedRows, expectedCols) {
        const errors = [];
        const warnings = [];
     ...
     */
    async loadWeights(layerId, validate = true) {
        const filePath = join(this.persistPath, `${layerId}.weights.bin`);
        const metadataPath = join(this.persistPath, `${layerId}.weights.meta.json`);
     ...
     */
    async createCheckpoint(layerId) {
        const weights = this.weights.get(layerId);
        if (!weights || weights.length === 0) {
            throw new Error(`No weights found for layer: ${layerId}`);
        }
        ...
        withRetrySync(() => writeFileSync(checkpointPath, Buffer.from(buffer)), { operationName: 'WeightManager.createCheckpoint.binary' });
        // Write metadata
        const metadata = {
            version: MODEL_VERSION,
        ...
        withRetrySync(() => writeFileSync(metadataPath, JSON.stringify(metadata, null, 2)), { operationName: 'WeightManager.createCheckpoint.metadata' });
     ...
     */
    async restoreFromCheckpoint(layerId) {
     ...
     * Implements: TASK-GNN-003 AC-004
     */
    async cleanupOldCheckpoints(layerId) {
        const checkpoints = this.listCheckpoints(layerId);
    ...
    setWeights(layerId, weights) {
        ...
        if (weights.length !== config.outputDim) {
            throw new Error(`setWeights: Row count mismatch for '${layerId}': ` +
                `expected ${config.outputDim}, got ${weights.length}`);
        }
        if (weights[0]?.length !== config.inputDim) {
            throw new Error(`setWeights: Column count mismatch for '${layerId}': ` +
     ...
     */
    updateWeights(layerId, delta, scale = 1.0) {
        ...
        if (delta.length !== weights.length) {
            throw new Error(`updateWeights: Row count mismatch for '${layerId}': ` +
                `weights have ${weights.length} rows, delta has ${delta.length}`);
        }
        if (delta[0]?.length !== weights[0]?.length) {
            throw new Error(`updateWeights: Column count mismatch for '${layerId}': ` +
        ...
        if (!validation.valid) {
            ...
            throw new Error(`updateWeights: Update produced invalid weights for '${layerId}': ` +
     ...
     */
    async deletePersistedWeights(layerId) {
        const filePath = join(this.persistPath, `${layerId}.weights.bin`);
        const metadataPath = join(this.persistPath, `${layerId}.weights.meta.json`);
        let deleted = false;
        try {
     ...
     */
    hasPersistedWeights(layerId) {
        const filePath = join(this.persistPath, `${layerId}.weights.bin`);
        return existsSync(filePath);
    }
}
//# sourceMappingURL=weight-manager.js.map