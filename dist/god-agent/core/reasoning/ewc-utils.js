...
export class EWCRegularizer {
     ...
     */
    computePenalty(currentWeights, optimalWeights, fisherDiagonal) {
     ...
     */
    computeGradients(currentWeights, optimalWeights, fisherDiagonal) {
        const startTime = performance.now();
     ...
     */
    updateFisher(existingFisher, taskGradients, numSamples) {
        const startTime = performance.now();
     ...
     */
    static unflattenWeights(flat, numRows, numCols) {
        const weights = [];
     ...
     */
    async saveFisher(fisher, filename = 'fisher.json') {
        ...
        withRetrySync(() => writeFileSync(filePath, JSON.stringify(serialized, null, 2)), { operationName: 'EWCRegularizer.saveFisher' });
     ...
     */
    async loadFisher(filename = 'fisher.json') {
        const filePath = join(this.config.persistPath, filename);
     ...
     */
    applyEWCToUpdate(weightUpdate, currentWeights, optimalWeights, fisherDiagonal) {
        // Get EWC gradients
 ...
 */
export function createEWCRegularizer(config) {
 ...
 */
export function computeImportanceScores(fisher) {
 ...
 */
export function getTopImportantParams(fisher, topK = 100) {
    const allParams = [];
    for (const [layerId, fisherLayer] of fisher) {
        for (let i = 0; i < fisherLayer.length; i++) {
 ...
 */
export function computeFisherOverlap(fisher1, fisher2, threshold = 1e-6) {
    let fisher1Only = 0;
    let fisher2Only = 0;
    let both = 0;
    // Collect all layer IDs
    const allLayerIds = new Set();
    for (const layerId of fisher1.keys())
        allLayerIds.add(layerId);
    for (const layerId of fisher2.keys())
        allLayerIds.add(layerId);
    for (const layerId of allLayerIds) {
...
//# sourceMappingURL=ewc-utils.js.map