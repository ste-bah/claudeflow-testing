...
export class CausalTraversal {
     ...
     */
    traverseForward(graph, startNodes, options = {}) {
     ...
     */
    traverseBackward(graph, effectNodes, options = {}) {
        const startTime = performance.now();
        // Set defaults
        const maxDepth = options.maxDepth ?? 5;
        const minConfidence = options.minConfidence ?? 0.0;
     ...
     */
    buildChainExplanation(graph, chain) {
     ...
     */
    wouldCreateCycle(chain, newLink) {
     ...
     */
    addChain(state, path, startNodes, endNodes) {
        const chain = {
            path,
...
//# sourceMappingURL=causal-traversal.js.map