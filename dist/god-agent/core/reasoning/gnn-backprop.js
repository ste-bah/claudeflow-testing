 ...
 */
export function clipGradient(gradient, maxNorm = DEFAULT_GRADIENT_CONFIG.maxGradientNorm) {
    // Compute L2 norm
    let norm = 0;
 ...
 */
function validateGradientDim(gradient, expectedDim, name) {
    if (gradient.length !== expectedDim) {
        throw new Error(`${name} dimension mismatch: expected ${expectedDim}, got ${gradient.length}`);
    }
}
/**
 ...
 */
export function isGradientValid(gradient) {
    for (let i = 0; i < gradient.length; i++) {
 ...
 */
function createZeroGradient(dim) {
 ...
 */
export function project_backward(gradient, W, x, inputDim = VECTOR_DIM, outputDim = VECTOR_DIM, config = {}) {
 ...
 */
export function softmax_backward(gradient, softmaxOutput, config = {}) {
    const cfg = { ...DEFAULT_GRADIENT_CONFIG, ...config };
    // Handle edge cases
 ...
 */
export function attention_backward(gradient, Q, K, V, attentionWeights, scale, config = {}) {
    const cfg = { ...DEFAULT_GRADIENT_CONFIG, ...config };
 ...
 */
export function aggregate_backward(gradient, neighborContributions, weights, config = {}) {
    const cfg = { ...DEFAULT_GRADIENT_CONFIG, ...config };
 ...
 */
export function relu_backward(gradient, input) {
 ...
 * @returns Gradient with respect to input
 */
export function leaky_relu_backward(gradient, input, alpha = 0.01) {
    const dim = Math.min(gradient.length, input.length);
    const dx = new Float32Array(dim);
    for (let i = 0; i < dim; i++) {
 ...
 * @returns Gradient with respect to input
 */
export function tanh_backward(gradient, output) {
 ...
 * @returns Gradient with respect to input
 */
export function sigmoid_backward(gradient, output) {
    const dim = Math.min(gradient.length, output.length);
    const dx = new Float32Array(dim);
    for (let i = 0; i < dim; i++) {
 ...
 */
export function activation_backward(gradient, inputOrOutput, activation) {
    switch (activation) {
        case 'relu':
 ...
 */
export function layer_backward(gradient, input, weights, preActivation, postActivation, activation = 'relu', useResidual = true, config = {}) {
    ...
    const activationInput = activation === 'relu' || activation === 'leaky_relu'
        ? preActivation
        : postActivation;
    const postActivationGradient = activation_backward(currentGradient, activationInput, activation);
    // Step 3: Backward through projection
    const projResult = project_backward(postActivationGradient, weights, input, input.length, preActivation.length, cfg);
 ...
 */
export function accumulateGradient(accumulated, newGradient, batchSize = 1) {
 ...
 * @param batchSize - Total batch size for averaging
 */
export function accumulateWeightGradients(accumulated, newGradients, batchSize = 1) {
 ...
 */
export function createWeightGradientAccumulator(numRows, numCols) {
    const accumulator = [];
    for (let i = 0; i < numRows; i++) {
...
//# sourceMappingURL=gnn-backprop.js.map