 ...
 */
export function generateProvenanceID() {
    const timestamp = Date.now();
    const random = randomBytes(4).toString('hex');
 ...
 */
export function isValidProvenanceID(id) {
...
export function validateSourceInput(input) {
    ...
    if (input.relevanceScore < 0.0 || input.relevanceScore > 1.0) {
        throw new ProvenanceValidationError(`relevanceScore ${input.relevanceScore} out of range [0.0, 1.0]`);
    }
}
/**
 ...
 * @throws ProvenanceValidationError if validation fails
 */
export function validateDerivationStep(step) {
    ...
    if (step.confidence < 0.0 || step.confidence > 1.0) {
        throw new ProvenanceValidationError(`Derivation step confidence ${step.confidence} out of range [0.0, 1.0]`);
    }
}
// ==================== L-Score Calculation ====================
/**
 ...
 */
export function calculateLScore(confidences, relevances, depth, domain) {
 ...
 */
export function getThresholdForDomain(domain) {
    if (!domain) {
 ...
 */
export function validateLScore(lScore, domain) {
    const threshold = getThresholdForDomain(domain);
 ...
 * @throws ProvenanceValidationError if validation fails
 */
export function validateEmbedding(embedding, context) {
    // Check dimensions
    if (embedding.length !== 1536) {
        throw new ProvenanceValidationError(`${context}: Expected 1536D embedding, got ${embedding.length}D`);
    }
    ...
    if (Math.abs(magnitude - 1.0) > 0.01 && magnitude > 0.01) {
        throw new ProvenanceValidationError(`${context}: Embedding not L2-normalized, magnitude = ${magnitude.toFixed(4)}`);
    }
}
//# sourceMappingURL=provenance-utils.js.map