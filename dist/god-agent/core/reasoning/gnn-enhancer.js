...
import { addVectors, normalize, zeroPad, applyActivation, project, softmax, attentionScore, weightedAggregate, } from './gnn-math.js';
...
export class GNNEnhancer {
     ...
     */
    constructor(config, cacheConfig, weightSeed, checkpointConfig, autoLoad = true) {
    ...
    initializeLayerWeights() {
        ...
        const initialization = activation === 'relu' || activation === 'gelu' ? 'he' : 'xavier';
     ...
     */
    async enhance(embedding, graphOrContext, hyperedges, collectActivations = false) {
        const startTime = performance.now();
        this.totalEnhancements++;
        const original = new Float32Array(embedding);
        const activationCache = [];
     ...
     */
    applyLayer(input, outputDim, layerNum) {
     ...
     */
    applyLayerWithCache(input, outputDim, layerNum) {
        // Get layer ID for weight lookup
        const layerId = `layer${layerNum}`;
        // Get learned weights for this layer
        const weights = this.weightManager.getWeights(layerId, {
        ...
        const postActivation = applyActivation(preActivation, this.config.activation);
     ...
     */
    aggregateNeighborhood(center, features, adjacency) {
    ...
    validateAllWeights() {
        ...
        for (const layerId of layerIds) {
            ...
            if (weights.length > 0) {
                ...
                results.set(layerId, this.weightManager.validateWeights(weights, config?.outputDim, config?.inputDim));
            }
        }
        return results;
    }
     ...
     * Implements: TASK-GNN-003
     */
    async deleteAllPersistedWeights() {
        const layerIds = ['input_projection', 'layer1', 'layer2', 'layer3', 'feature_projection'];
        let deletedCount = 0;
        for (const layerId of layerIds) {
