...
export class ProvenanceStore {
    ...
    async createProvenance(input) {
        ...
        const derivationPath = input.derivationPath.map((step) => {
        ...
        for (const step of derivationPath) {
            for (const sourceId of step.sourceIds) {
                if (!this.sources.has(sourceId)) {
                    throw new ProvenanceValidationError(`Derivation step references unknown source: ${sourceId}`);
                }
            }
        }
        ...
        if (this.config.enforceLScore) {
            ...
            if (!lScoreResult.meetsThreshold) {
                throw new LScoreRejectionError(lScoreResult.score, lScoreResult.threshold, input.domain);
            }
        }
     ...
     */
    async traverseCitationGraph(provenanceId, options = {}) {
        const startTime = performance.now();
        ...
        const visitedProvenances = new Set();
        visitedProvenances.add(provenanceId);
        if (includeAncestors && provenance.parentProvenanceId) {
            let currentProvenanceId = provenance.parentProvenanceId;
            let currentDepth = 1;
     ...
     */
    async calculateLScore(provenanceId) {
        const provenance = this.provenances.get(provenanceId);
        if (!provenance) {
            throw new ProvenanceValidationError(`Provenance ${provenanceId} not found`);
        }
     ...
     */
    async findSimilarSources(embedding, k = 5) {
     ...
     */
    getChildProvenances(provenanceId) {
        return Array.from(this.provenances.values()).filter(p => p.parentProvenanceId === provenanceId);
    }
    /**
    ...
    toJSON() {
        ...
        const provenances = Array.from(this.provenances.values()).map(p => ({
            id: p.id,
