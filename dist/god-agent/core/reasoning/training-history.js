 ...
 */
import { withRetrySync } from '../validation/retry.js';
/**
...
export class TrainingHistoryManager {
    ...
    selectByIdStmt = null;
    selectAllStmt = null;
    selectByEpochRangeStmt = null;
    selectLatestLossStmt = null;
    selectRecentLossesStmt = null;
    countStmt = null;
    deleteOlderThanStmt = null;
    /**
    ...
    async recordBatch(record) {
        ...
        withRetrySync(() => this.insertStmt.run(params), { operationName: 'TrainingHistoryManager.recordBatch' });
    }
    /**
     ...
     */
    async getLatestLoss() {
        const result = this.selectLatestLossStmt.get();
     ...
     */
    async getLossTrend(windowSize) {
        const rows = this.selectRecentLossesStmt.all(windowSize);
    ...
    async findById(recordId) {
        const row = this.selectByIdStmt.get(recordId);
        if (!row)
            return null;
    ...
    `);
        ...
        const latestLr = latestLrQuery.get();
        return {
     ...
     */
    static generateRecordId(epoch, batchIndex) {
...
//# sourceMappingURL=training-history.js.map