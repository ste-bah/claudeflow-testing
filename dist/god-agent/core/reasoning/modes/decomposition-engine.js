...
export class DecompositionEngine {
    ...
    async reason(request, config) {
        ...
        const queryText = request.metadata?.queryText || 'decomposition query';
        // Apply defaults
        ...
        const subproblems = await this.identifySubproblems(queryText, strategy, effectiveConfig.maxSubproblems, effectiveConfig.minComplexity);
        ...
        const parallelizationGroups = this.identifyParallelGroups(subproblems, dependencies);
        // Calculate total complexity
        const totalComplexity = subproblems.reduce((sum, sp) => sum + sp.estimatedComplexity, 0) / Math.max(subproblems.length, 1);
     ...
     */
    selectBestStrategy(query, complexity) {
        const lowerQuery = query.toLowerCase();
     ...
     */
    async identifySubproblems(query, strategy, maxSubproblems, minComplexity) {
     ...
     */
    decomposeHierarchical(segments, maxSubproblems) {
        const subproblems = [];
        ...
        for (const taskType of hierarchyOrder) {
            ...
            id++;
            if (subproblems.length >= maxSubproblems)
                break;
        }
     ...
     * Sequential decomposition: by execution order
     */
    decomposeSequential(segments, maxSubproblems) {
        const subproblems = [];
        const dependencies = [];
     ...
     * Parallel decomposition: independent subproblems
     */
    decomposeParallel(segments, maxSubproblems) {
        const subproblems = [];
     ...
     */
    formatSubproblemDescription(taskType, text) {
        const taskLabels = {
     ...
     * Analyze dependencies between subproblems
     */
    analyzeDependencies(subproblems) {
        const edges = [];
     ...
     */
    determineExecutionOrder(subproblems, dependencies) {
     ...
     */
    identifyParallelGroups(subproblems, dependencies) {
     ...
     */
    calculatePlanConfidence(plan, queryComplexity) {
        const factors = [];
     ...
     */
    generateReasoning(plan, strategy) {
        return [
 ...
 */
export function createDecompositionEngine(deps) {
