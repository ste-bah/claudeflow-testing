...
export class AnalogicalEngine {
    ...
    async reason(request, config) {
        ...
        const sourcePatterns = await this.findSourcePatterns(effectiveConfig.sourceDomain);
        // Generate analogical mappings
        const mappings = await this.generateMappings(sourcePatterns, sourceKnowledge, targetKnowledge, effectiveConfig);
        // Filter by threshold
        const filteredMappings = mappings.filter(m => m.structuralSimilarity >= effectiveConfig.structuralMappingThreshold);
        ...
        const result = {
            ...
            reasoningSteps: this.generateReasoning(sortedMappings, effectiveConfig.sourceDomain, effectiveConfig.targetDomain),
            latencyMs,
            confidence,
     ...
     */
    extractConceptsFromDomain(domain) {
     ...
     */
    async findSourcePatterns(sourceDomain) {
     ...
     * Generate synthetic patterns from domain knowledge
     */
    async generateSyntheticPatterns(domain) {
        const patterns = [];
        const textsToEmbed = [];
        const conceptList = [];
     ...
     */
    async generateMappings(sourcePatterns, sourceKnowledge, targetKnowledge, config) {
        const mappings = [];
        // Check for predefined cross-domain analogies
        const predefinedMappings = this.findPredefinedMappings(sourceKnowledge.domain, targetKnowledge.domain);
        // Generate mappings for each source pattern
        for (const sourcePattern of sourcePatterns) {
            const mapping = await this.createMapping(sourcePattern, sourceKnowledge, targetKnowledge, predefinedMappings, config.abstractionLevel);
            if (mapping) {
                mappings.push(mapping);
            }
        }
        // Add structure-based mappings
        const structuralMappings = this.generateStructuralMappings(sourceKnowledge, targetKnowledge, predefinedMappings);
     ...
     */
    findPredefinedMappings(sourceDomain, targetDomain) {
        for (const analogy of CROSS_DOMAIN_ANALOGIES) {
            const [d1, d2] = analogy.domains;
            if ((d1 === sourceDomain && d2 === targetDomain) ||
     ...
     */
    async createMapping(sourcePattern, sourceKnowledge, targetKnowledge, predefinedMappings, abstractionLevel) {
        const nodeMappings = [];
        ...
        for (const sourceConcept of sourceConcepts) {
            const targetMapping = this.findBestTargetMapping(sourceConcept, targetKnowledge, predefinedMappings, abstractionLevel);
            if (targetMapping) {
                nodeMappings.push({
        ...
        const structuralSimilarity = await this.calculateStructuralSimilarity(nodeMappings, sourceKnowledge, targetKnowledge);
        // Calculate transferability
        const transferability = this.calculateTransferability(nodeMappings, sourcePattern, abstractionLevel);
        return {
            sourcePattern,
            targetDomain: targetKnowledge.domain,
     ...
     */
    extractPatternConcepts(pattern, domainKnowledge) {
        const concepts = [];
        ...
        if (pattern.metadata?.concept) {
            concepts.push(pattern.metadata.concept);
        }
        // Deduplicate
     ...
     */
    findBestTargetMapping(sourceConcept, targetKnowledge, predefinedMappings, abstractionLevel) {
        ...
        for (const targetConcept of targetKnowledge.concepts) {
            const score = this.computeConceptSimilarity(sourceConcept, targetConcept, abstractionLevel);
     ...
     */
    computeConceptSimilarity(source, target, abstractionLevel) {
        // Exact match
        if (source === target) {
            return 0.95;
        }
     ...
     */
    generateStructuralMappings(sourceKnowledge, targetKnowledge, predefinedMappings) {
        const candidates = [];
        ...
        for (const sourceRel of sourceKnowledge.relations) {
            for (const targetRel of targetKnowledge.relations) {
                if (sourceRel.type === targetRel.type) {
                    ...
                    const fromMapping = predefinedMappings.find(m => m.source === sourceRel.from && m.target === targetRel.from);
                    const toMapping = predefinedMappings.find(m => m.source === sourceRel.to && m.target === targetRel.to);
     ...
     */
    async calculateStructuralSimilarity(nodeMappings, sourceKnowledge, targetKnowledge) {
        if (nodeMappings.length === 0) {
            return 0;
        }
        ...
        for (const mapping of nodeMappings) {
            ...
            const sourceRelations = sourceKnowledge.relations.filter(r => r.from === mapping.sourceNode || r.to === mapping.sourceNode);
            for (const rel of sourceRelations) {
                // Find corresponding target relation
                const hasTargetRelation = targetKnowledge.relations.some(tr => (tr.from === mapping.targetNode || tr.to === mapping.targetNode) &&
                    tr.type === rel.type);
     ...
     */
    calculateTransferability(nodeMappings, sourcePattern, abstractionLevel) {
    ...
    calculateConfidence(mappings) {
        ...
        const avgTransferability = topMappings.reduce((s, m) => s + m.transferability, 0) / topMappings.length;
     ...
     */
    generateReasoning(mappings, sourceDomain, targetDomain) {
        const reasoning = [
 ...
 */
export function createAnalogicalEngine(deps) {
...
//# sourceMappingURL=analogical-engine.js.map