...
export class AbductiveEngine {
    ...
    async reason(request, config) {
        ...
        const observedEffects = await this.extractEffects(request.query, config.observedEffects.length || 5);
        ...
        const hypotheses = this.generateHypotheses(allChains, Array.from(allCauses), hypothesisLimit);
        // 4. Score hypotheses
        const scoredHypotheses = hypotheses.map(h => this.scoreHypothesis(h, effectNodes, occamWeight));
        // Sort by score descending
        scoredHypotheses.sort((a, b) => b.score - a.score);
        // Filter by minimum plausibility
        const plausibleHypotheses = scoredHypotheses.filter(sh => sh.score >= minPlausibility);
        // 5. Convert to explanations
        const explanations = plausibleHypotheses.map((sh, index) => this.toExplanation(sh, plausibleHypotheses.length, index === 0));
     ...
     */
    async extractEffects(embedding, maxEffects) {
        try {
     ...
     */
    generateHypotheses(chains, allCauses, limit) {
        const hypotheses = [];
        const causeToChains = new Map();
     ...
     */
    scoreHypothesis(hypothesis, observedEffects, occamWeight) {
        // 1. Calculate coverage: what % of observed effects are explained
        const explainedEffects = new Set();
     ...
     */
    toExplanation(scoredHypothesis, totalAlternatives, _isBest) {
     ...
     */
    buildHypothesisDescription(causes) {
        if (causes.length === 1) {
     ...
     */
    calculateCombinedLScore(explanations) {
     ...
     */
    createEmptyResult(_request, processingTimeMs) {
        const latencyMs = Math.round(processingTimeMs);
        return {
            // IAdvancedReasoningResult fields
            mode: AdvancedReasoningMode.ABDUCTIVE,
     ...
     */
    generateReasoningSteps(explanations, allChains) {
        return [
...
//# sourceMappingURL=abductive-engine.js.map