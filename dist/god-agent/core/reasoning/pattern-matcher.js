...
import { rankPatterns, filterPatterns, createPatternResult } from './confidence-scorer.js';
...
export class PatternMatcher {
     ...
     */
    constructor(vectorDB, memoryEngine) {
    ...
    async findPatterns(query) {
        ...
        for (const result of searchResults) {
            ...
            const patternResult = createPatternResult(pattern, result.similarity, 0, // Rank will be set later
            true // Calibrate confidence
            );
            patternResults.push(patternResult);
        }
        // Filter by thresholds
        const filtered = filterPatterns(patternResults, minConfidence, minSuccessRate, minSonaWeight);
        // Rank patterns
     ...
     */
    async createPattern(params) {
        this.ensureInitialized();
        // Validate success rate threshold
        if (params.successRate < 0.8) {
            throw new Error(`Cannot create pattern with successRate ${params.successRate} below threshold 0.8`);
        }
     ...
     */
    async updatePatternSuccess(params) {
        this.ensureInitialized();
     ...
     */
    async updatePatternSonaWeight(patternId, weight) {
        this.ensureInitialized();
     ...
     */
    getPatternsByTaskType(taskType) {
        this.ensureInitialized();
     ...
     */
    async pruneLowQualityPatterns(params) {
        this.ensureInitialized();
