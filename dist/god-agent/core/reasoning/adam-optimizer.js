...
export class AdamOptimizer {
     ...
     */
    stepWithStats(weights, gradients) {
 ...
 */
export function createAdamOptimizer(config) {
 ...
 */
export function unflattenWeights(flat, numRows, numCols) {
    const weights = [];
    let offset = 0;
    for (let r = 0; r < numRows; r++) {
 ...
 */
export function applyAdamTo2DWeights(optimizer, weights, gradients) {
    // Flatten all weights and gradients
    const flatWeights = new Map();
    const flatGradients = new Map();
...
//# sourceMappingURL=adam-optimizer.js.map