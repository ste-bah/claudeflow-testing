 ...
 */
export function calculateConfidence(similarity, successRate, sonaWeight) {
 ...
 */
export function calibrateConfidence(rawConfidence, steepness = 10) {
 ...
 */
export function filterPatterns(patterns, minConfidence, minSuccessRate, minSonaWeight) {
 ...
 */
export function batchCalculateConfidence(patterns, similarities, calibrate = true) {
    if (patterns.length !== similarities.length) {
        throw new Error('Patterns and similarities arrays must have same length');
    }
    const confidences = new Array(patterns.length);
    for (let i = 0; i < patterns.length; i++) {
        const pattern = patterns[i];
        const similarity = similarities[i];
        // Calculate raw confidence
        const raw = calculateConfidence(similarity, pattern.successRate, pattern.sonaWeight);
 ...
 */
export function createPatternResult(pattern, similarity, rank = 0, calibrate = true) {
    const raw = calculateConfidence(similarity, pattern.successRate, pattern.sonaWeight);
