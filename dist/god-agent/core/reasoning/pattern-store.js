...
export class PatternStore {
    ...
    constructor(vectorDB, memoryEngine) {
        ...
        for (const taskType of Object.values(TaskType)) {
            this.indexByTaskType.set(taskType, new Set());
        }
    }
    /**
    ...
    async initialize() {
        ...
        try {
            ...
            const result = await this.memoryEngine.retrieve(PATTERNS_STORAGE_KEY, { namespace: PATTERNS_NAMESPACE });
            if (result) {
    ...
    async addPattern(params) {
        ...
        await withRetry(() => this.vectorDB.insertWithId(pattern.id, pattern.embedding), { operationName: 'PatternStore.addPattern.vectorInsert' });
        // Persist changes
        await this.persist();
        return pattern;
    }
     ...
     */
    async findDuplicate(taskType, embedding) {
