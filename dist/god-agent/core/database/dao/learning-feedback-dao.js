...
export class LearningFeedbackDAO {
    ...
    insertStmt = null;
    selectByIdStmt = null;
    selectByTrajectoryIdStmt = null;
    selectUnprocessedStmt = null;
    countStmt = null;
    countUnprocessedStmt = null;
    markProcessedStmt = null;
    ...
    validateQuality(quality) {
        if (typeof quality !== 'number' || !Number.isFinite(quality)) {
            throw new Error(`RULE-088 VIOLATION: Quality must be a finite number, got: ${typeof quality}`);
        }
        if (quality < 0.0 || quality > 1.0) {
            throw new Error(`RULE-088 VIOLATION: Quality must be in range [0.0, 1.0], got: ${quality}`);
        }
    }
    /**
    ...
    insert(feedback) {
        ...
        withRetrySync(() => this.insertStmt.run(params), { operationName: 'LearningFeedbackDAO.insert' });
    }
    /**
    ...
    markProcessed(id) {
        ...
        withRetrySync(() => this.markProcessedStmt.run(id), { operationName: 'LearningFeedbackDAO.markProcessed' });
    }
    /**
     * Find feedback by ID
     *
     ...
     */
    findByTrajectoryId(trajectoryId) {
        const rows = this.selectByTrajectoryIdStmt.all(trajectoryId);
    ...
    findUnprocessed(limit = 100) {
        const rows = this.selectUnprocessedStmt.all(limit);
        return rows.map(row => this.rowToFeedback(row));
    }
    /**
     ...
     */
    countUnprocessed() {
        const result = this.countUnprocessedStmt.get();
        return result.count;
    }
    /**
     ...
     */
    delete(_id) {
        throw new Error('RULE-018 VIOLATION: Learning feedback is append-only. DELETE operations are FORBIDDEN. ' +
     ...
     */
    clear() {
        throw new Error('RULE-018 VIOLATION: Learning feedback is append-only. CLEAR operations are FORBIDDEN. ' +
            'Only markProcessed() updates are permitted.');
    }
    /**
...
//# sourceMappingURL=learning-feedback-dao.js.map