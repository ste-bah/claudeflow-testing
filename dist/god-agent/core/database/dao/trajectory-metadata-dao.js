...
export class TrajectoryMetadataDAO {
    ...
    updateQualityStmt = null;
    selectByIdStmt = null;
    selectByStatusStmt = null;
    selectAllStmt = null;
    countStmt = null;
    ...
    insert(metadata) {
        ...
        withRetrySync(() => this.insertStmt.run(params), { operationName: 'TrajectoryMetadataDAO.insert' });
    }
    /**
    ...
    updateStatus(id, status, completedAt) {
        ...
        // RULE-072: Database operations MUST retry on failure (max 3 attempts)
        withRetrySync(() => this.updateStatusStmt.run(params), { operationName: 'TrajectoryMetadataDAO.updateStatus' });
    }
    /**
     ...
     */
    updateQuality(id, qualityScore) {
        // Validate quality score bounds per schema CHECK constraint
        if (qualityScore < 0.0 || qualityScore > 1.0) {
            throw new Error(`Invalid quality score: ${qualityScore}. Must be between 0.0 and 1.0.`);
        }
        const params = {
            id,
            qualityScore
        ...
        // RULE-072: Database operations MUST retry on failure (max 3 attempts)
        withRetrySync(() => this.updateQualityStmt.run(params), { operationName: 'TrajectoryMetadataDAO.updateQuality' });
    }
    /**
     ...
     */
    delete(_id) {
        throw new Error('RULE-016 VIOLATION: Trajectory metadata is append-only. DELETE operations are FORBIDDEN. ' +
     ...
     */
    clear() {
        throw new Error('RULE-016 VIOLATION: Trajectory metadata is append-only. CLEAR operations are FORBIDDEN. ' +
            'Exception: Compaction with explicit human approval requires separate implementation.');
    }
    /**
     ...
     */
    findByRoute(routePattern) {
        const stmt = this.db.prepare(`
      SELECT * FROM trajectory_metadata
      WHERE route = ?
      ORDER BY created_at DESC
    `);
        const rows = stmt.all(routePattern);
        return rows.map(row => this.rowToMetadata(row));
    }
    /**
    ...
    findByMinQuality(minQuality) {
        if (minQuality < 0.0 || minQuality > 1.0) {
            throw new Error(`Invalid minQuality: ${minQuality}. Must be between 0.0 and 1.0.`);
        }
        const stmt = this.db.prepare(`
      SELECT * FROM trajectory_metadata
    ...
    `);
        const rows = stmt.all(minQuality);
        return rows.map(row => this.rowToMetadata(row));
    }
}
//# sourceMappingURL=trajectory-metadata-dao.js.map