...
import { DEFAULT_HNSW_CONFIG } from './hnsw-types.js';
import { getDistanceFunction } from './distance.js';
import { Int8Quantizer, QuantizedVectorStorage } from '../quantization/index.js';
/**
...
class MinHeap {
    ...
    bubbleUp(index) {
        while (index > 0) {
            ...
            if (this.heap[parentIdx].distance <= this.heap[index].distance)
                break;
...
export class HNSWIndex {
    ...
    constructor(dimension, config) {
        ...
        if (this.quantizationEnabled) {
            ...
            this.quantizedStorage = new QuantizedVectorStorage(dimension, config?.quantizationConfig, { cacheFullPrecision: false } // We keep full precision in this.vectors for re-ranking
            );
        }
        else {
            this.quantizer = null;
     ...
     */
    searchLayerGreedy(query, entryId, level) {
     ...
     */
    selectNeighbors(query, candidates, M, _level) {
     ...
     */
    rerankWithFullPrecision(query, candidates, k) {
     ...
     */
    searchWithVectors(query, k, includeVectors = false) {
    ...
    getQuantizedDistance(idA, idB) {
        ...
        return this.quantizer.quantizedDistance(qA.quantized, qA.scale, qA.zeroPoint, qB.quantized, qB.scale, qB.zeroPoint);
    }
    /**
