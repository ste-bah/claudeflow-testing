...
export class QuantizedVectorStorage {
     ...
     */
    constructor(dimension, config, options) {
    ...
    retrieve(id) {
        ...
        return this.quantizer.dequantize(stored.quantized, stored.scale, stored.zeroPoint);
    }
    /**
    ...
    search(query, k) {
        ...
        // Quantize query
        const { quantized: qQuery, scale: qScale, zeroPoint: qZp } = this.quantizer.quantize(query);
        // Compute distances to all vectors
        const results = [];
        for (const stored of this.storage.values()) {
            const distance = this.quantizer.quantizedDistance(qQuery, qScale, qZp, stored.quantized, stored.scale, stored.zeroPoint);
     ...
     */
    searchWithRerank(query, k, efSearch) {
        ...
        const reranked = approxResults.map(result => {
    ...
    getMemoryUsage() {
        ...
        const compressionRatio = float32Baseline > 0
    ...
    *entries() {
        for (const [id, stored] of this.storage) {
            const vector = this.quantizer.dequantize(stored.quantized, stored.scale, stored.zeroPoint);
            yield [id, vector];
        }
    }
    /**
...
//# sourceMappingURL=quantized-storage.js.map