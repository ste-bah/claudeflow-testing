 ...
 */
import { DEFAULT_QUANTIZATION_CONFIG, } from './quantization-types.js';
/**
...
export class Int8Quantizer {
     ...
     */
    quantizeSymmetric(vector, quantized) {
        ...
        for (let i = 0; i < vector.length; i++) {
            ...
            quantized[i] = Math.max(this.INT8_MIN, Math.min(this.INT8_MAX, Math.round(scaled)));
        }
        return {
            quantized,
            scale,
     ...
     */
    quantizeAsymmetric(vector, quantized) {
        // Find min and max
        let min = vector[0];
        let max = vector[0];
        ...
        for (let i = 0; i < vector.length; i++) {
            ...
            quantized[i] = Math.max(this.INT8_MIN, Math.min(this.INT8_MAX, Math.round(shifted)));
        }
        return {
            quantized,
            scale,
    ...
    quantizeBatch(vectors) {
        ...
        const quantizedVectors = new Array(count);
     ...
     */
    quantizedDistance(a, scaleA, zpA, b, scaleB, zpB) {
     ...
     */
    symmetricQuantizedDistance(a, scaleA, b, scaleB) {
     ...
     */
    quantizedSquaredEuclidean(a, scaleA, zpA, b, scaleB, zpB) {
        let sum = 0;
        const len = a.length;
        if (zpA === 0 && zpB === 0) {
    ...
    measureBatchQuality(vectors) {
        ...
        for (const vector of vectors) {
            ...
            if (metrics.maxError > maxMaxError)
                maxMaxError = metrics.maxError;
...
//# sourceMappingURL=int8-quantizer.js.map