...
export class DatabaseConnection {
    ...
    getStats() {
        try {
            const episodeCount = this.db.prepare('SELECT COUNT(*) as count FROM episodes').get()?.count || 0;
            const patternCount = this.db.prepare('SELECT COUNT(*) as count FROM patterns').get()?.count || 0;
            const outcomeCount = this.db.prepare('SELECT COUNT(*) as count FROM episode_outcomes').get()?.count || 0;
            const feedbackCount = this.db.prepare('SELECT COUNT(*) as count FROM learning_feedback').get()?.count || 0;
