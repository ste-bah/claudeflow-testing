...
import { GraphDB, FallbackGraph } from './graph-db/index.js';
import { UnifiedSearch, } from './search/index.js';
import { MemoryClient } from './memory-server/index.js';
import { MemoryEngine } from './memory/index.js';
import { ReasoningBank, PatternMatcher, CausalMemory, ProvenanceStore, } from './reasoning/index.js';
import { SonaEngine, createProductionSonaEngine } from './learning/index.js';
import { RelayRaceOrchestrator, } from './orchestration/index.js';
import { CompressionManager, } from './compression/index.js';
import { AttentionFactory, } from './attention/index.js';
import { PhdPipelineOrchestrator, PhDPipelineRunner, } from './pipeline/index.js';
import { ClaudeTaskExecutor } from './executor/claude-task-executor.js';
import { MetricsCollector, Logger, DistributedTracer, } from './observability/index.js';
import { RuntimeSelector, } from './portability/index.js';
import { createAgentExecutionService, } from './services/index.js';
...
export class GodAgent {
    ...
    async initializeNativeCore() {
        ...
        this.fallbackGraph = new FallbackGraph(this.config.graphDB?.dataDir ?? '.agentdb/graphs', this.config.graphDB?.lockTimeout ?? 5000, this.config.graphDB?.enablePersistence ?? true);
    ...
    async initializeMemory() {
        ...
        this.memoryEngine = new MemoryEngine(this.vectorDB, this.graphDB, embeddingProvider, 10000);
    ...
    async initializeUnifiedSearch() {
        ...
        this.unifiedSearch = new UnifiedSearch(this.vectorDB.backend_, // IHNSWBackend implements NativeHNSW interface
        this.fallbackGraph, this.memoryClient, this.reasoningBank, undefined, // GNNEnhancer - can be added later if needed
    ...
    initializeObservability() {
        ...
        this.logger = new Logger({
            minLevel: (this.config.verbose ? 'debug' : 'info'),
        });
    ...
    async store(data, options = {}) {
        ...
        try {
            ...
            if (options.trackProvenance !== false &&
                this.provenanceStore &&
                typeof this.provenanceStore.track === 'function') {
                await this.provenanceStore.track({
                    operation: 'store',
                    entityId: id,
                    timestamp: Date.now(),
    ...
    async learn(feedback) {
        ...
        if (this.sonaEngine && typeof this.sonaEngine.recordFeedback === 'function') {
            await this.sonaEngine.recordFeedback({
     ...
     */
    async searchWithEmbedding(query, embedding, options) {
        this.ensureInitialized();
        if (!this.unifiedSearch) {
            throw new Error('UnifiedSearch not initialized');
        }
    ...
    async runPipeline(task) {
        ...
        try {
            ...
            if (!this.agentRegistry) {
                throw new Error('AgentRegistry not initialized. Call initializeAgentExecution() before using runPipeline().');
            }
            ...
            if (this.sonaEngine) {
                orchestrator.setSonaEngine({
                    createTrajectory: (_route, _patterns, _context) => {
                        // Generate trajectory ID
                        return `trj_${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
                    },
                    provideFeedback: async (trajectoryId, quality, options) => {
                        // Record feedback if sonaEngine supports it
                        if (typeof this.sonaEngine.recordFeedback === 'function') {
                            await this.sonaEngine.recordFeedback({
                                trajectoryId,
                                quality,
                                ...options,
     ...
     */
    async buildPipelineDefinition(task) {
     ...
     */
    async storeKnowledgeIfAvailable(data) {
     ...
     */
    async initializePhdPipeline(agentsBasePath = '.claude/agents', options = {}) {
        this.ensureInitialized();
     ...
     */
    async runPhdResearch(problemStatement) {
        if (!this.phdPipelineRunner) {
            throw new Error('PhD Pipeline Runner not initialized. Call initializePhdPipeline() first.');
        }
     ...
     */
    async initializeAgentExecution(agentsBasePath = '.claude/agents') {
        this.ensureInitialized();
        ...
        const memoryEngineAdapter = this.memoryEngine
            ...
            : undefined;
        // Create service
        this.agentExecutionService = createAgentExecutionService(this.agentRegistry, executor, { verbose: this.config.verbose ?? false }, this.sonaEngine, memoryEngineAdapter);
     ...
     * ```
     */
    async runAgent(agentKey, task, options) {
        if (!this.agentExecutionService) {
            throw new Error('Agent Execution Service not initialized. Call initializeAgentExecution() first.');
        }
     ...
     * ```
     */
    async runAgentChain(steps, options) {
        if (!this.agentExecutionService) {
            throw new Error('Agent Execution Service not initialized. Call initializeAgentExecution() first.');
        }
...
//# sourceMappingURL=god-agent.js.map