...
export class CompatibilityMatrix {
     ...
     */
    generateTestCommand(platform, nodeVersion) {
        const flags = [];
...
`;
     ...
     */
    isSupported(os, arch, nodeVersion) {
     ...
     */
    getRecommendations(os, arch, nodeVersion) {
        const recommendations = [];
        const platform = `${os}-${arch}`;
        if (!SUPPORTED_PLATFORMS.has(platform)) {
            recommendations.push(`Platform ${platform} does not have native bindings. ` +
        ...
        if (!SUPPORTED_NODE_VERSIONS.includes(nodeVersion)) {
            ...
            recommendations.push(`Node.js ${nodeVersion} is not officially supported. ` +
