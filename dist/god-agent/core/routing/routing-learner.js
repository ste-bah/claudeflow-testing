...
export class RoutingLearner {
    ...
    async processFeedback(feedback) {
        try {
            ...
            throw new RoutingLearningError(`Failed to process feedback: ${error instanceof Error ? error.message : String(error)}`, 'feedback-processing', this.totalFeedback, feedback.routingId, { agentKey: feedback.selectedAgent }, error instanceof Error ? error : undefined);
        }
    }
    /**
    ...
    updateWeights(agentKey, reward) {
        ...
        const clampedDelta = Math.max(-this.config.maxWeightChange, Math.min(this.config.maxWeightChange, effectiveDelta));
     ...
     */
    updateImportance(agentKey, reward) {
     ...
     * @param feedback - Routing feedback
     */
    updateAccuracyHistory(feedback) {
     ...
     */
    logAgentReliability(agentKey, success) {
        if (this.verbose) {
     ...
     */
    async submitToReasoningBank(feedback, classification) {
        if (!this.reasoningBank) {
            return;
        }
        try {
...
//# sourceMappingURL=routing-learner.js.map