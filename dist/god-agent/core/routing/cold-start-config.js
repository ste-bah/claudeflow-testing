 ...
 */
import { DEFAULT_COLD_START_CONFIG, } from './routing-types.js';
import { RoutingError } from './routing-errors.js';
/**
 * Default cold start configuration (re-export)
 */
export const defaultColdStartConfig = DEFAULT_COLD_START_CONFIG;
/**
 ...
 * ```
 */
export function getColdStartPhase(executionCount, config = DEFAULT_COLD_START_CONFIG) {
    // Validate execution count
    if (executionCount < 0) {
        throw new RoutingError(`Invalid execution count: ${executionCount}. Must be non-negative.`, { confidence: 0 });
    }
 ...
 * ```
 */
export function getColdStartWeights(phase, config = DEFAULT_COLD_START_CONFIG) {
    let keywordWeight;
    switch (phase) {
        case 'keyword-only':
 ...
 * ```
 */
export function formatColdStartIndicator(phase, executionCount) {
    if (phase === 'learned') {
        return '[Learned Mode]';
    }
    else {
...
//# sourceMappingURL=cold-start-config.js.map