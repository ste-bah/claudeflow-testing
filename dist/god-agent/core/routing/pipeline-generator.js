...
export class PipelineGenerator {
    ...
    async generate(task) {
        ...
        if (!task || task.trim().length === 0) {
            throw new PipelineGenerationError('Task description cannot be empty', task, 'validation', 0, 0);
        }
        ...
        try {
            ...
            if (segments.length < 2) {
                throw new PipelineGenerationError('Use routeTask for single-step tasks', normalizedTask, 'validation', 0, 1);
            }
            if (segments.length > this.config.maxPipelineStages) {
                throw new PipelineGenerationError(`Pipeline exceeds maximum of ${this.config.maxPipelineStages} stages`, normalizedTask, 'validation', 0, segments.length);
            }
            if (this.verbose) {
            ...
            const pipeline = this.buildPipelineDefinition(normalizedTask, stages, startTime);
            if (this.verbose) {
                console.log(`[PipelineGenerator] Generated pipeline "${pipeline.name}" in ${pipeline.generationTimeMs.toFixed(2)}ms`);
            }
            return pipeline;
        }
        catch (error) {
            ...
            throw new PipelineGenerationError(`Pipeline generation failed: ${error}`, normalizedTask, 'definition-building', 0, 0, undefined, error);
        }
    }
     ...
     */
    async generateStages(segments, fullTask) {
        const stages = [];
        for (let i = 0; i < segments.length; i++) {
            ...
            try {
                ...
                stages.push(stage);
                if (this.verbose) {
                    console.log(`  [Stage ${i}] ${name} → ${routing.selectedAgentName} (${(routing.confidence * 100).toFixed(0)}%)`);
                }
            }
            catch (error) {
                throw new PipelineGenerationError(`Failed to generate stage ${i}: ${error}`, fullTask, 'routing', i, segments.length, undefined, error);
            }
        }
        return stages;
    }
     ...
     */
    generateOutputDomain(stageIndex, task) {
     ...
     */
    buildPipelineDefinition(task, stages, startTime) {
        ...
        const estimatedDurationMs = stages.reduce((total, stage) => total + stage.estimatedDurationMs, 0);
...
//# sourceMappingURL=pipeline-generator.js.map