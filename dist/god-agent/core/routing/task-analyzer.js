...
export class TaskAnalyzer {
    ...
    async initEmbeddingProvider() {
        ...
        try {
            ...
            throw new TaskAnalysisError('Failed to initialize embedding provider', '', 'embedding', undefined, error);
        }
    }
    /**
    ...
    async analyze(task) {
        ...
        if (!task || task.trim().length === 0) {
            throw new TaskAnalysisError('Task description cannot be empty', task, 'verb-extraction');
        }
        ...
        if (!this.embeddingProvider) {
            throw new TaskAnalysisError('Embedding provider not initialized', task, 'embedding');
        }
        try {
            // Step 1: Extract verbs
            const verbs = this.extractVerbs(normalizedTask);
            if (verbs.length === 0) {
                throw new TaskAnalysisError('No verbs found in task description', task, 'verb-extraction');
            }
            ...
            throw new TaskAnalysisError('Task analysis failed', task, 'domain-detection', undefined, error);
        }
    }
    /**
    ...
    detectDomain(task, verbs) {
        ...
        let maxDomain = 'code'; // Default to code if no matches
        let maxScore = 0;
        // Check domains in order (review first to prioritize over research)
        const domainOrder = ['review', 'testing', 'code', 'writing', 'design', 'research'];
     ...
     */
    extractCapabilities(task, domain, verbs) {
...
export function buildDomainPatterns() {
    const patterns = {};
    for (const [domain, verbs] of Object.entries(DOMAIN_PATTERNS)) {
 ...
 * @deprecated Not currently used, kept for potential optimization
 */
export function buildVerbPatterns() {
    const verbs = new Set();
    for (const domainVerbs of Object.values(DOMAIN_PATTERNS)) {
        for (const verb of domainVerbs) {
            verbs.add(verb);
