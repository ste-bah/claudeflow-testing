...
import { getColdStartPhase, getColdStartWeights, formatColdStartIndicator, } from './cold-start-config.js';
...
export class RoutingEngine {
    ...
    async route(analysis) {
        ...
        try {
            ...
            const { keywordWeight, capabilityWeight } = getColdStartWeights(coldStartPhase, this.config.coldStart);
            // Search capability index by embedding
            const matches = await this.capabilityIndex.search(analysis.embedding, 10);
            if (matches.length === 0) {
                throw new RoutingError('No matching agents found for task', { confidence: 0 });
            }
            // Calculate scores for each match
            const scoredMatches = this.scoreAgents(matches, analysis, keywordWeight, capabilityWeight);
            ...
            // Build factors array
            const factors = this.buildFactors(selectedMatch, keywordWeight, capabilityWeight, analysis);
            // Get primary factor (highest weighted score)
            const primaryFactor = this.getPrimaryFactor(factors);
            // Build explanation
            const explanation = this.buildExplanation(selectedMatch, confidence, primaryFactor, coldStartPhase, analysis);
            // Build alternatives (top 3 excluding selected)
            const alternatives = this.buildAlternatives(scoredMatches.slice(1, 4));
            // Determine confirmation level
            const { requiresConfirmation, confirmationLevel } = this.determineConfirmationLevel(confidence);
            ...
            if (this.verbose) {
                console.log(`[RoutingEngine] Routed to ${selectedMatch.name} (confidence: ${confidence.toFixed(2)}) in ${routingTimeMs.toFixed(2)}ms`);
            }
            ...
            throw new RoutingError(`Routing failed: ${error}`, { confidence: 0 }, error);
        }
    }
    /**
     ...
     */
    createPreferenceResult(analysis, startTime) {
        const coldStartPhase = this.getColdStartPhase();
        const isColdStart = this.executionCount < this.config.coldStart.learnedThreshold;
        ...
        if (this.verbose) {
            console.log(`[RoutingEngine] Using preferred agent ${agentName} in ${routingTimeMs.toFixed(2)}ms`);
        }
        return result;
    }
    /**
     ...
     */
    scoreAgents(matches, analysis, keywordWeight, capabilityWeight) {
        return matches.map(match => {
            // Calculate keyword score
            const keywordScore = this.calculateKeywordScore(match.capability.keywords, analysis.verbs, analysis.task);
            // Check domain match
            const domainMatch = match.capability.domains.includes(analysis.domain);
            // Calculate combined score
            let combinedScore = capabilityWeight * match.similarityScore + keywordWeight * keywordScore;
     ...
     */
    calculateKeywordScore(agentKeywords, taskVerbs, taskText) {
     ...
     */
    buildFactors(match, keywordWeight, capabilityWeight, analysis) {
        const factors = [
            {
     ...
     */
    buildExplanation(match, confidence, primaryFactor, coldStartPhase, analysis) {
        const parts = [];
        // Main routing decision
        parts.push(`Selected ${match.name} with ${(confidence * 100).toFixed(0)}% confidence for ${analysis.domain} task.`);
        // Primary factor
        parts.push(`Primary factor: ${primaryFactor.description} (score: ${(primaryFactor.score * 100).toFixed(0)}%).`);
        // Domain match
        if (match.domainMatch) {
        ...
        if (coldStartPhase !== 'learned') {
            if (coldStartPhase === 'keyword-only') {
                parts.push(`Using keyword-only matching (execution ${this.executionCount}/100 - learning phase).`);
            }
            else {
                parts.push(`Blending keyword and capability matching (execution ${this.executionCount}/100 - learning phase).`);
            }
        }
        return parts.join(' ');
    }
     ...
     */
    determineConfirmationLevel(confidence) {
