...
export class ConfirmationHandler {
     ...
     */
    async requestConfirmation(routing) {
     ...
     * Public for testing
     */
    createConfirmationRequest(routing) {
        const options = [];
     ...
     */
    async handleConfirmLevel(request) {
        if (this.config.verbose) {
     ...
     */
    async handleSelectLevel(request) {
        if (this.config.verbose) {
     ...
     */
    formatConfirmationMessage(routing) {
        const lines = [];
     ...
     */
    logUserOverride(routingId, originalAgent, selectedAgent) {
     ...
     */
    async processUserResponse(request, selectedKey) {
        // Log override if user selected different agent
        this.logUserOverride(request.routingResult.routingId, request.routingResult.selectedAgent, selectedKey);
        return {
            selectedKey,
            wasTimeout: false,
