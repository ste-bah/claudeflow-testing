...
export class MemoryEngine {
    ...
    cache;
    transactionManager;
    constructor(vectorDB, graphDB, embeddingProvider, cacheCapacity = 1000) {
    ...
    async store(key, value, options) {
        ...
        if (options.linkTo) {
            try {
                ...
                throw new StorageTransactionError(`Link target node ${options.linkTo} not found`, error);
            }
        }
        ...
        try {
            ...
            this.transactionManager.addOperation(txnId, this.transactionManager.createVectorOperation(this.vectorDB, vectorId));
            ...
            this.transactionManager.addOperation(txnId, this.transactionManager.createNodeOperation(this.graphDB, nodeId));
            ...
            if (options.linkTo && nodeId && options.relation) {
                ...
                this.transactionManager.addOperation(txnId, this.transactionManager.createEdgeOperation(this.graphDB, edgeId));
            }
            // PHASE 3: COMMIT
            ...
            throw new StorageTransactionError(`Failed to store memory with key "${key}"`, error);
        }
    }
    /**
    ...
    async retrieve(key, options = {}) {
        try {
            ...
            const fullNode = await this.graphDB.getNode(node.id);
            const encodedValue = fullNode.properties.value;
            ...
            throw new StorageTransactionError(`Failed to retrieve memory with key "${key}"`, error);
        }
    }
    /**
    ...
    async search(query, options = {}) {
        try {
            ...
            throw new StorageTransactionError(`Failed to search memories for query "${query}"`, error);
        }
    }
    /**
        ...
        try {
            ...
            for (const edge of edges) {
                ...
                if (targetNode.type !== 'memory')
                    continue;
                const encodedValue = targetNode.properties.value;
                const decodedValue = decodeValue(encodedValue);
                results.push({
                    key: targetNode.properties.key,
                    value: decodedValue,
            ...
            throw new StorageTransactionError(`Failed to get related memories for node "${nodeId}"`, error);
        }
    }
    /**
    ...
    async close() {
        ...
        if (this.graphDB && typeof this.graphDB.close === 'function') {
    ...
    getCacheStats() {
        return {
            ...
            capacity: this.cache.capacity || 1000
        };
    }
}
