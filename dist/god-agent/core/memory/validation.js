 ...
 */
export const ROOT_NAMESPACES = ['project', 'research', 'patterns'];
/**
 ...
 */
export function isRootNamespace(namespace) {
    return ROOT_NAMESPACES.includes(namespace);
}
/**
...
export function validateNamespace(namespace) {
    if (!namespace) {
        throw new NamespaceValidationError('Namespace cannot be empty', namespace);
    }
    if (!NAMESPACE_REGEX.test(namespace)) {
        throw new NamespaceValidationError(`Invalid namespace format: "${namespace}". Must match pattern: ^[a-z0-9]+(/[a-z0-9-]+)*$`, namespace);
    }
    // Check for trailing slash
    if (namespace.endsWith('/')) {
        throw new NamespaceValidationError(`Namespace cannot end with slash: "${namespace}"`, namespace);
    }
    // Check for leading slash
    if (namespace.startsWith('/')) {
        throw new NamespaceValidationError(`Namespace cannot start with slash: "${namespace}"`, namespace);
    }
}
/**
 ...
 */
export function validateOrphanPrevention(namespace, linkTo) {
    ...
    if (!linkTo) {
        throw new OrphanNodeError(`Non-root namespace "${namespace}" requires linkTo parameter to prevent orphaned nodes. ` +
