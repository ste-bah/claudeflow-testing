...
export class AgentCategoryScanner {
    ...
    async scanCategories(basePath) {
        ...
        if (this.verbose) {
            console.log(`[CategoryScanner] Found ${categories.length} categories with ${categories.reduce((sum, c) => sum + c.agentCount, 0)} total agents`);
        }
        return categories;
    }
    /**
     ...
     */
    async countAgentFiles(directoryPath) {
        try {
     ...
     */
    async getCategoryNames(basePath) {
     ...
     */
    async hasCategory(basePath, categoryName) {
        const categories = await this.scanCategories(basePath);
        return categories.some(c => c.name === categoryName);
    }
}
//# sourceMappingURL=agent-category-scanner.js.map