...
import { DEFAULT_LOADER_OPTIONS, createDefaultFrontmatter, } from './agent-types.js';
// ==================== YAML Parser ====================
/**
 ...
 */
function extractEssentialFields(yamlContent) {
    const result = {};
    // Extract name
    ...
    if (colorMatch)
        result.color = colorMatch[1].trim().replace(/^["']|["']$/g, '');
...
export class AgentDefinitionLoader {
     ...
     */
    async loadAll(categoryPath, category) {
     ...
     */
    async loadOne(filePath, category) {
        try {
     ...
     */
    validateHooks(hooks, agentKey) {
        if (hooks.pre) {
            // Check for common memory commands
            if (!hooks.pre.includes('claude-flow') && !hooks.pre.includes('echo')) {
                if (this.options.verbose) {
                    console.warn(`[AgentLoader] ${agentKey}: pre-hook may be missing claude-flow commands`);
                }
            }
        }
        if (hooks.post) {
            ...
            if (!hooks.post.includes('memory') && !hooks.post.includes('store')) {
                if (this.options.verbose) {
                    console.warn(`[AgentLoader] ${agentKey}: post-hook may be missing memory store command`);
                }
            }
        }
    }
