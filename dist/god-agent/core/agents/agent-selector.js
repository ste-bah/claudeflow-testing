...
export class AgentSelector {
    ...
    selectAgent(task) {
        ...
        if (candidates.length === 0) {
            throw new AgentSelectionError(task, this.registry.getAll().map(a => a.key), {
        ...
        if (this.verbose) {
            console.log(`[AgentSelector] Selected agent: ${selected.key} ` +
     ...
     */
    calculateKeywordScore(agent, keywords) {
    ...
    calculateCapabilityScore(agent, requiredCapabilities) {
        ...
        // Handle capabilities that may be string or array
        const capabilities = agent.frontmatter.capabilities;
        const agentCapabilities = Array.isArray(capabilities)
            ? capabilities
            : typeof capabilities === 'string'
                ? [capabilities]
                : [];
     ...
     */
    calculateTriggerScore(agent, keywords) {
    ...
    detectTaskType(task) {
        ...
        const types = ['design', 'research', 'write', 'code', 'ask'];
