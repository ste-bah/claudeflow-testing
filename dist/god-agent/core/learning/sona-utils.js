 ...
 */
export function isValidTrajectoryID(id) {
 ...
 */
export function isValidCheckpointID(id) {
 ...
 */
export function validateTrajectoryInput(input) {
...
export function validateQuality(quality) {
    ...
    if (quality < 0.0 || quality > 1.0) {
        throw new TrajectoryValidationError(`Quality ${quality} out of range [0.0, 1.0]`);
    }
}
/**
 ...
 * @throws TrajectoryValidationError if invalid
 */
export function validateLearningRate(rate) {
    if (rate < 0.001 || rate > 0.1) {
        throw new TrajectoryValidationError(`Learning rate ${rate} out of range [0.001, 0.1]`);
    }
}
/**
 ...
 * @throws TrajectoryValidationError if invalid
 */
export function validateRegularization(lambda) {
...
export function validateAndApplyConfig(config = {}) {
    ...
    if (validated.learningRate < 0.001 || validated.learningRate > 0.1) {
        throw new TrajectoryValidationError(`Learning rate ${validated.learningRate} out of range [0.001, 0.1]`);
    }
    if (validated.regularization < 0.01 || validated.regularization > 1.0) {
        throw new TrajectoryValidationError(`Regularization ${validated.regularization} out of range [0.01, 1.0]`);
    }
    if (validated.driftAlertThreshold <= 0 || validated.driftAlertThreshold >= 1) {
        throw new TrajectoryValidationError(`Drift alert threshold ${validated.driftAlertThreshold} out of range (0, 1)`);
    }
    if (validated.driftRejectThreshold <= validated.driftAlertThreshold) {
        throw new TrajectoryValidationError(`Drift reject threshold ${validated.driftRejectThreshold} must be greater than alert threshold ${validated.driftAlertThreshold}`);
    }
    return validated;
}
// ==================== Weight Operations ====================
 ...
 */
export function calculateDrift(current, baseline) {
 ...
 */
export function calculateReward(quality, lScore, trajectorySuccessRate) {
 ...
 */
export function calculateGradient(reward, similarity) {
 ...
 */
export function calculateWeightUpdate(gradient, learningRate, regularization, importance) {
 ...
 */
export function updateFisherInformation(currentImportance, gradient, decay = 0.9) {
 ...
 */
export function calculateSuccessRate(qualityScores, defaultRate = 0.5) {
 ...
 * @throws TrajectoryValidationError if invalid
 */
export function validateFeedbackQuality(quality) {
    if (typeof quality !== 'number' || isNaN(quality)) {
        throw new TrajectoryValidationError('Feedback quality must be a valid number');
    }
    if (quality < 0.0 || quality > 1.0) {
        throw new TrajectoryValidationError(`Feedback quality ${quality} out of range [0.0, 1.0]`);
    }
}
/**
...
//# sourceMappingURL=sona-utils.js.map