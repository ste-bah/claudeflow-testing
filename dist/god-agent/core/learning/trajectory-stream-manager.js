...
ERR_ROLLBACK_LOOP, ERR_DELETE_BASELINE, ERR_READ_ONLY, } from '../types/trajectory-streaming-types.js';
/**
...
export class TrajectoryStreamManager {
     ...
     */
    async addTrajectory(trajectory) {
     ...
     */
    async recordRollback(checkpointId) {
        // Check if this is the same checkpoint as last rollback
        if (this.rollbackState.lastRollbackCheckpointId === checkpointId) {
            throw new ERR_ROLLBACK_LOOP(`Rollback loop detected: Attempting to roll back to checkpoint "${checkpointId}" ` +
     ...
     */
    async deleteTrajectory(id, force = false) {
        const result = {
            deletedCount: 0,
            bytesReclaimed: 0,
            compacted: false,
        ...
        if (metadata.isBaseline && !force) {
            throw new ERR_DELETE_BASELINE(`Cannot delete baseline trajectory ${id}. Use force=true to override.`);
        }
     ...
     */
    async pruneTrajectories(filter) {
        const result = {
            deletedCount: 0,
            bytesReclaimed: 0,
            compacted: false,
     ...
     */
    async readTrajectoryFromDisk(metadata) {
     ...
     */
    migrateTrajectories(trajectories, fromVersion, toVersion) {
     ...
     */
    async readDataFile(fileIndex, version) {
     ...
     */
    async writeDataFile(fileIndex, trajectories, version) {
        const filePath = join(this.config.storageDir, `data_${String(fileIndex).padStart(6, '0')}.bin`);
        const tempPath = `${filePath}.tmp`;
...
//# sourceMappingURL=trajectory-stream-manager.js.map