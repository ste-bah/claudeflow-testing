 ...
 */
export const PHASE_MIN_OUTPUT_LENGTH = {
    Foundation: 500,
    Discovery: 1000,
    Architecture: 800,
    Synthesis: 600,
 ...
 */
export const PHD_LEARNING_CONFIG = {
    routePrefix: 'phd/',
...
export class PhDLearningIntegration extends LearningIntegration {
    ...
    criticalAgentResults = new Map();
    /** Track phase completion for phase-level learning */
    phaseCompletionCounts = new Map();
    ...
    calculateQuality(result) {
        ...
        const adjustedQuality = Math.max(0, Math.min(1.0, (baseCalc.quality + outputQualityBonus + citationBonus - criticalPenalty) * phaseWeight));
     ...
     */
    inferPhaseFromAgentNumber(agentNum) {
        if (agentNum <= 4)
            return 'Foundation';
        if (agentNum <= 9)
            return 'Discovery';
     ...
     */
    updatePhaseCompletion(phase, success) {
        const phaseData = this.phaseCompletionCounts.get(phase);
    ...
    getPhdMetrics() {
        ...
        return {
            ...
            criticalAgentSuccessRate: this.criticalAgentResults.size > 0
 ...
 */
export function createPhDLearningIntegration(sonaEngine, reasoningBank = null, config = {}) {
    return new PhDLearningIntegration(sonaEngine, config, reasoningBank);
}
// Constants are already exported at declaration
//# sourceMappingURL=phd-learning-integration.js.map