 ...
 */
import { DEFAULT_AGENT_TIMEOUT, DEFAULT_NAMESPACE, } from '../types/agent-execution-types.js';
// ==================== Agent Execution Service ====================
/**
...
export class AgentExecutionService {
    ...
    memoryEngine;
    config;
    constructor(registry, executor, config = {}, sonaEngine, memoryEngine) {
     ...
     */
    async executeAgent(agentKey, task, options = {}) {
        ...
        if (opts.trackTrajectory && this.sonaEngine) {
            trajectoryId = this.sonaEngine.createTrajectory(`agent/${agentKey}`, [agentKey, loadedDef.category], []);
        }
        try {
     ...
     */
    async executeChain(steps, options = {}) {
        const startTime = Date.now();
        const results = [];
        // Merge options with defaults
        const chainOpts = {
    ...
    listAgents(filter) {
        ...
        if (filter) {
            ...
            if (filter.capability) {
                ...
                agents = agents.filter(a => a.frontmatter.capabilities?.some(c => c.toLowerCase().includes(lowerCap)));
            }
            if (filter.priority) {
            ...
            if (filter.namePattern) {
                ...
                agents = agents.filter(a => a.key.toLowerCase().includes(lowerPattern) ||
     ...
     */
    buildPrompt(task, def, additionalContext, memoryContext) {
        const parts = [];
     ...
     */
    buildMemoryKey(agentKey, namespace) {
     ...
     */
    createTimeoutPromise(timeout, agentKey) {
     ...
     * Create error result
     */
    createErrorResult(agentKey, error, startTime, category) {
        return {
            agentKey,
            success: false,
            output: '',
 ...
 */
export function createAgentExecutionService(registry, executor, config, sonaEngine, memoryEngine) {
