 ...
 * ```
 */
export class RealRWKVAttention {
    name = 'rwkv';
    dimension;
    // Time mixing weights
    wReceptance; // [dim × dim]
    ...
    timeDecay; // [dim] - learned w parameter
    timeFirst; // [dim] - learned u parameter
    // Time mixing factors (for token shift)
    timeMix; // [dim] - interpolation weight
    rng;
    /**
     ...
     */
    computeWKV(k, v, stateWkv, // [dim]
    stateNum, // [dim]
    stateMax // [dim] - for numerical stability
    ) {
    ...
    forward(query, _key, _value, _mask, seqLen) {
        ...
        for (let t = 0; t < actualSeqLen; t++) {
            ...
            const wkvResult = this.computeWKV(kProj, vProj, stateWkv, stateNum, stateMax);
     ...
     */
    getParameterCount() {
        return (4 * this.dimension * this.dimension + // 4 projection matrices
            3 * this.dimension // timeDecay + timeFirst + timeMix
        );
    }
}
//# sourceMappingURL=rwkv-attention.js.map