 ...
 * ```
 */
export class RealLinformerAttention {
    name = 'linformer';
    dimension;
    numHeads;
    headDim;
    projectedDim; // k - projection dimension
    maxSeqLen;
    scale;
    ...
    projE; // Key projection
    projF; // Value projection
    rng;
    /**
    ...
    constructor(config) {
        ...
        if (this.projectedDim > this.maxSeqLen) {
            throw new Error(`ANTI-009: projectedDim ${this.projectedDim} cannot exceed maxSeqLen ${this.maxSeqLen}`);
        }
     ...
     */
    projectSequence(input, proj, seqLen, dim) {
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        if (actualSeqLen > this.maxSeqLen) {
            throw new Error(`ANTI-009: Sequence length ${actualSeqLen} exceeds maxSeqLen ${this.maxSeqLen}`);
        }
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadLinformerAttention(qProjected, kProjected, vProjected, h, actualSeqLen);
     ...
     */
    computeHeadLinformerAttention(Q, K, V, head, seqLen) {
...
//# sourceMappingURL=linformer-attention.js.map