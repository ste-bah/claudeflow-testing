 ...
 * ```
 */
export class RealGlobalAttention {
    name = 'global';
    dimension;
    numHeads;
    headDim;
    ...
    constructor(config) {
        ...
        if (this.numGlobalTokens < 1) {
            throw new Error(`numGlobalTokens must be >= 1, got ${this.numGlobalTokens}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        if (query.length !== actualSeqLen * this.dimension) {
            throw new Error(`Query length ${query.length} incompatible with seqLen=${actualSeqLen}, dim=${this.dimension}` +
        ...
        if (query.length !== key.length || query.length !== value.length) {
            throw new Error(`Dimension mismatch: Q=${query.length}, K=${key.length}, V=${value.length}`);
        }
        // Validate mask dimensions if provided
        if (mask && mask.length !== actualSeqLen * actualSeqLen) {
            throw new Error(`Mask length ${mask.length} incompatible with seqLen=${actualSeqLen}` +
                ` (expected ${actualSeqLen * actualSeqLen})`);
        }
        // Validate numGlobalTokens vs seqLen
        if (this.numGlobalTokens >= actualSeqLen) {
            throw new Error(`numGlobalTokens (${this.numGlobalTokens}) must be < seqLen (${actualSeqLen})`);
        }
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadAttention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadAttention(Q, K, V, head, seqLen, mask) {
        // 1. Compute scaled dot-product scores: (Q·K^T) / √d_k
...
//# sourceMappingURL=global-attention.js.map