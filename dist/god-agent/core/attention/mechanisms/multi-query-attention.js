 ...
 * ```
 */
export class RealMultiQueryAttention {
    name = 'multi-query';
    dimension;
    numHeads;
    headDim;
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            ...
            const headOutput = this.computeHeadAttention(qProjected, kProjected, vProjected, actualSeqLen, mask);
     ...
     */
    projectSingleHead(input, weights, seqLen) {
     ...
     */
    computeHeadAttention(Q, K, V, seqLen, mask) {
        const output = new Float32Array(seqLen * this.headDim);
        for (let i = 0; i < seqLen; i++) {
...
//# sourceMappingURL=multi-query-attention.js.map