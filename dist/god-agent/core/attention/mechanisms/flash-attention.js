 ...
 * ```
 */
export class RealFlashAttention {
    name = 'flash';
    dimension;
    numHeads;
    headDim;
    ...
    constructor(config) {
        ...
        if (this.blockSize <= 0 || this.blockSize > this.dimension) {
            throw new Error(`Block size ${this.blockSize} must be between 1 and dimension ${this.dimension}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeFlashHeadAttention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeFlashHeadAttention(Q, K, V, head, seqLen, mask) {
        // Output buffer
