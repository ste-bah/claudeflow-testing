 ...
 * ```
 */
export class RealMambaAttention {
    name = 'mamba';
    dimension;
    stateSize; // N in the paper (state dimension)
    expandFactor; // Expansion factor for inner dimension
    // Projection matrices
    wIn; // Input projection [dim → expand]
    wDelta; // Δ projection [dim → expand]
    wB; // B projection [dim → stateSize]
    wC; // C projection [dim → stateSize]
    wOut; // Output projection [expand → dim]
    // SSM parameters (A is not learned, structured initialization)
    A; // State matrix [stateSize]
    rng;
    /**
    ...
    constructor(config) {
        ...
        if (this.stateSize < 1) {
            throw new Error(`ANTI-009: State size must be positive, got ${this.stateSize}`);
        }
     ...
     * 4. Output: y = C · h
     */
    selectiveScan(x, delta, B, C, seqLen, expandDim) {
     ...
     */
    forward(query, _key, _value, _mask, seqLen) {
     ...
     */
    projectInput(input, weights, seqLen, outDim) {
     ...
     */
    projectOutput(input, seqLen, expandDim) {
    ...
    getParameterCount() {
        const expandDim = this.dimension * this.expandFactor;
        return (this.dimension * expandDim + // wIn
...
//# sourceMappingURL=mamba-attention.js.map