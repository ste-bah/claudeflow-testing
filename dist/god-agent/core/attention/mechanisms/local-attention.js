 ...
 * ```
 */
export class RealLocalAttention {
    name = 'local';
    dimension;
    numHeads;
    headDim;
    scale;
    windowSize; // Total window size (positions attended to)
    halfWindow; // Half window for left/right
    ...
    constructor(config) {
        ...
        if (this.windowSize < 1) {
            throw new Error(`ANTI-009: Window size must be at least 1, got ${this.windowSize}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadLocalAttention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadLocalAttention(Q, K, V, head, seqLen, mask) {
...
//# sourceMappingURL=local-attention.js.map