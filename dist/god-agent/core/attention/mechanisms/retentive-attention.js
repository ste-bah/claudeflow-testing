 ...
 * ```
 */
export class RealRetentiveAttention {
    name = 'retentive';
    dimension;
    numHeads;
    headDim;
    gammas; // Per-head decay rates
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        const headOutputs = [];
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadRetention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadRetention(Q, K, V, head, seqLen, mask) {
...
//# sourceMappingURL=retentive-attention.js.map