 ...
 * ```
 */
export class RealLongformerAttention {
    name = 'longformer';
    dimension;
    numHeads;
    headDim;
    windowSize; // One-sided window size
    globalIndices;
    scale;
    ...
    constructor(config) {
        ...
        if (this.windowSize < 1) {
            throw new Error(`ANTI-009: Window size must be positive, got ${this.windowSize}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        if (query.length !== actualSeqLen * this.dimension) {
            throw new Error(`ANTI-009: Query length ${query.length} incompatible with seqLen=${actualSeqLen}, dim=${this.dimension}`);
        }
        // Validate key/value dimensions match query
        if (query.length !== key.length || query.length !== value.length) {
            throw new Error(`ANTI-009: Dimension mismatch: Q=${query.length}, K=${key.length}, V=${value.length}`);
        }
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadLongformerAttention(qLocal, kLocal, vLocal, qGlobal, kGlobal, vGlobal, h, actualSeqLen, mask);
     ...
     */
    computeHeadLongformerAttention(qLocal, kLocal, vLocal, qGlobal, kGlobal, vGlobal, head, seqLen, mask) {
        ...
        for (let i = 0; i < seqLen; i++) {
            ...
            if (isGlobalToken) {
                ...
                this.computeGlobalAttention(qGlobal, kGlobal, vGlobal, i, head, seqLen, mask, output, outOffset);
            }
            else {
                // Local token: sliding window + global tokens
                this.computeLocalAttention(qLocal, kLocal, vLocal, qGlobal, kGlobal, vGlobal, i, head, seqLen, mask, output, outOffset);
            }
        }
        return output;
    }
     ...
     */
    computeGlobalAttention(qGlobal, kGlobal, vGlobal, position, head, seqLen, mask, output, outOffset) {
     ...
     */
    computeLocalAttention(qLocal, kLocal, vLocal, _qGlobal, kGlobal, vGlobal, position, head, seqLen, mask, output, outOffset) {
        const qOffset = position * this.dimension + head * this.headDim;
...
//# sourceMappingURL=longformer-attention.js.map