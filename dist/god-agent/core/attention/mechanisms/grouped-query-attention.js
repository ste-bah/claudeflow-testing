 ...
 * ```
 */
export class RealGroupedQueryAttention {
    name = 'grouped-query';
    dimension;
    numHeads; // Query heads
    numKVHeads; // Key-Value heads
    headsPerGroup; // Query heads per KV head
    headDim;
    scale;
    ...
    constructor(config) {
        ...
        if (this.numHeads % this.numKVHeads !== 0) {
            throw new Error(`ANTI-009: numHeads ${this.numHeads} must be divisible by numKVHeads ${this.numKVHeads}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            ...
            const headOutput = this.computeHeadAttention(qProjected, kProjected, vProjected, h, kvGroup, actualSeqLen, mask);
     ...
     */
    projectGroupedHeads(input, weights, seqLen, kvDim) {
     ...
     */
    computeHeadAttention(Q, K, V, qHead, kvGroup, seqLen, mask) {
...
//# sourceMappingURL=grouped-query-attention.js.map