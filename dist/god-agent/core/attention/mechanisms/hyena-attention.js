 ...
 * ```
 */
export class RealHyenaAttention {
    name = 'hyena';
    dimension;
    numHeads;
    headDim;
    order; // Hyena recursion depth
    filterSize;
    ...
    filterMLP1; // [headDim × filterSize]
    filterMLP2; // [filterSize × headDim]
     ...
     */
    generateFilter(position, seqLen) {
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadHyena(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadHyena(Q, K, V, head, seqLen, mask) {
...
//# sourceMappingURL=hyena-attention.js.map