...
class MultiheadAttentionBlock {
    ...
    numHeads;
    headDim;
    constructor(dimension, numHeads, rng) {
     ...
     */
    forward(queries, keysValues, seqLenQ, seqLenKV) {
        // Project inputs
    ...
    multiheadAttention(Q, K, V, seqLenQ, seqLenKV) {
 ...
 */
export class RealSetTransformerAttention {
    ...
    constructor(config = {}) {
        ...
        this.inducingPoints = xavierUniform(this.numInducingPoints, this.dimension, rng);
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        if (query.length !== expectedLength) {
            throw new Error(`ANTI-009: query length ${query.length} must equal seqLen × dimension (${expectedLength})`);
        }
        ...
        const H = this.mab1.forward(this.inducingPoints, input, this.numInducingPoints, seqLen);
        // Step 2: Output = MAB(X, H)
        // Input attends to the compressed representation
        const output = this.mab2.forward(input, H, seqLen, this.numInducingPoints);
        // Apply mask if provided
        if (mask) {
            if (mask.length !== seqLen) {
                throw new Error(`ANTI-009: mask length ${mask.length} must equal seqLen (${seqLen})`);
            }
