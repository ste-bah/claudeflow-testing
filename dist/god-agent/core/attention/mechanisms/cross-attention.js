 ...
 * ```
 */
export class RealCrossAttention {
    name = 'cross';
    dimension;
    numHeads;
    headDim;
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        if (query.length !== queryLen * this.dimension) {
            throw new Error(`ANTI-009: Query length ${query.length} incompatible with queryLen=${queryLen}, dim=${this.dimension}`);
        }
        if (key.length !== value.length) {
            throw new Error(`ANTI-009: Key/Value length mismatch: K=${key.length}, V=${value.length}`);
        }
        if (key.length !== contextLen * this.dimension) {
            throw new Error(`ANTI-009: Key length ${key.length} incompatible with contextLen=${contextLen}, dim=${this.dimension}`);
        }
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadCrossAttention(qProjected, kProjected, vProjected, h, queryLen, contextLen, mask);
     ...
     */
    computeHeadCrossAttention(Q, K, V, head, queryLen, contextLen, mask) {
...
//# sourceMappingURL=cross-attention.js.map