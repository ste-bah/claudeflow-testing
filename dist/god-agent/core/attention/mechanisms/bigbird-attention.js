 ...
 * ```
 */
export class RealBigBirdAttention {
    name = 'bigbird';
    dimension;
    numHeads;
    headDim;
    ...
    constructor(config) {
        ...
        if (this.numRandomBlocks < 0) {
            throw new Error(`ANTI-009: Number of random blocks must be non-negative, got ${this.numRandomBlocks}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadBigBirdAttention(qProjected, kProjected, vProjected, h, actualSeqLen, randomPatterns, mask);
     ...
     */
    computeHeadBigBirdAttention(Q, K, V, head, seqLen, randomPatterns, mask) {
        ...
        for (let i = 0; i < seqLen; i++) {
            ...
            if (this.globalIndices.has(i)) {
                ...
                // Random positions
                const randomPos = randomPatterns.get(i) || new Set();
...
//# sourceMappingURL=bigbird-attention.js.map