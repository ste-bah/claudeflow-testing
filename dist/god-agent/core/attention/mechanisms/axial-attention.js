...
export class RealAxialAttention {
    ...
    headDim;
    scale;
    axisSize; // Size of one axis (sqrt of seqLen)
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        const colAttnOutput = this.performAxisAttention(rowAttnOutput, rowAttnOutput, rowAttnOutput, height, width, 'column', mask, actualSeqLen);
        // Project output
     ...
     */
    performAxisAttention(Q, K, V, height, width, axis, mask, originalSeqLen) {
        const gridSize = height * width;
        const output = new Float32Array(gridSize * this.dimension);
        // Temporary buffers for multi-head attention
        const scoresBuffer = new Float32Array(axis === 'row' ? width : height);
     ...
     */
    padSequence(seq, currentLen, targetLen) {
...
//# sourceMappingURL=axial-attention.js.map