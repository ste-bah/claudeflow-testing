 ...
 * ```
 */
export class RealSparseTransformerAttention {
    ...
    constructor(config) {
        ...
        if (this.stride < 1) {
            throw new Error(`ANTI-009: Stride must be positive, got ${this.stride}`);
        }
        if (this.localSize < 1) {
            throw new Error(`ANTI-009: Local size must be positive, got ${this.localSize}`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadSparseAttention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadSparseAttention(Q, K, V, head, seqLen, mask) {
     ...
     */
    getSparsePattern(position, seqLen) {
...
//# sourceMappingURL=sparse-transformer-attention.js.map