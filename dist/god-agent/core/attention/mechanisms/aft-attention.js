...
export class RealAFTAttention {
     ...
     */
    validateInputs(query, key, value, mask, seqLen) {
        ...
        if (query.length % this.dimension !== 0) {
            throw new Error(`ANTI-009: input length (${query.length}) must be divisible by dimension (${this.dimension})`);
        }
        const inferredSeqLen = Math.floor(query.length / this.dimension);
        if (seqLen !== undefined && seqLen !== inferredSeqLen) {
            throw new Error(`ANTI-009: provided seqLen (${seqLen}) does not match inferred seqLen (${inferredSeqLen})`);
        }
        if (mask !== undefined && mask.length !== inferredSeqLen) {
            throw new Error(`ANTI-009: mask length (${mask.length}) must match seqLen (${inferredSeqLen})`);
        }
...
//# sourceMappingURL=aft-attention.js.map