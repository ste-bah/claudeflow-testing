 ...
 * ```
 */
export class RealHyperbolicAttention {
    name = 'hyperbolic';
    dimension;
    numHeads;
    headDim;
    curvature; // Negative for hyperbolic space
    eps = 1e-5;
    ...
    constructor(config) {
        ...
        if (this.curvature >= 0) {
            throw new Error(`ANTI-009: Curvature must be negative for hyperbolic space, got ${this.curvature}`);
        }
     ...
     */
    projectToPoincare(x, offset, length) {
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadHyperbolicAttention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadHyperbolicAttention(Q, K, V, head, seqLen, mask) {
