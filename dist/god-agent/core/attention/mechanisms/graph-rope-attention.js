 ...
 * ```
 */
export class RealGraphRoPeAttention {
    name = 'graph-rope';
    dimension;
    numHeads;
    headDim;
    rotaryDim; // Dimensions to apply RoPE to
    scale;
    baseFreq = 10000;
    ...
    constructor(config) {
        ...
        if (this.rotaryDim > this.headDim) {
            throw new Error(`ANTI-009: rotaryDim ${this.rotaryDim} must be <= headDim ${this.headDim}`);
        }
        if (this.rotaryDim % 2 !== 0) {
            throw new Error(`ANTI-009: rotaryDim ${this.rotaryDim} must be even for rotation pairs`);
        }
    ...
    forward(query, key, value, mask, seqLen) {
        ...
        for (let h = 0; h < this.numHeads; h++) {
            const headOutput = this.computeHeadGraphRoPeAttention(qProjected, kProjected, vProjected, h, actualSeqLen, mask);
     ...
     */
    computeHeadGraphRoPeAttention(Q, K, V, head, seqLen, mask) {
...
//# sourceMappingURL=graph-rope-attention.js.map