 ...
 * ```
 */
export function matmul(vec, weights, dim) {
    // Validate dimensions
    if (vec.length % dim !== 0) {
        throw new Error(`Vector length ${vec.length} not divisible by dimension ${dim}`);
    }
    if (weights.length !== dim * dim) {
        throw new Error(`Weight matrix size ${weights.length} incompatible with dimension ${dim}` +
            ` (expected ${dim * dim})`);
    }
 ...
 * @returns Dot product
 */
export function dotProduct(a, b, offset_a, offset_b, length) {
    let sum = 0;
 ...
 */
export function validateMatmulDims(vecLen, weightLen, dim) {
    return (vecLen % dim === 0 &&
        weightLen === dim * dim);
}
//# sourceMappingURL=matrix-ops.js.map