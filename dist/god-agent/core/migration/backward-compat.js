...
export class BackwardCompatLayer {
    ...
    convert(vector, sourceId) {
        ...
        if (dim !== LEGACY_VECTOR_DIM) {
            throw new Error(`Unexpected vector dimension: ${dim}. Expected ${LEGACY_VECTOR_DIM} or ${VECTOR_DIM}.`);
        }
        // Track conversion
     ...
     */
    maybeWarn(source) {
        if (!this.options.warnOnConversion)
            return;
        if (this.stats.warningsEmitted >= this.options.maxWarnings)
            return;
        ...
        const conversionRate = this.stats.legacyVectorsConverted / this.stats.totalVectorsProcessed;
        if (conversionRate > this.options.conversionThreshold) {
            console.warn(`[BackwardCompat] Converting 768D -> 1536D vectors. ` +
...
Migration Recommended:
  ${this.stats.legacyVectorsConverted} vectors (${(this.stats.legacyVectorsConverted / this.stats.totalVectorsProcessed * 100).toFixed(1)}%) are being converted at runtime.
 ...
 */
export function getBackwardCompatLayer(options) {
    if (!globalInstance) {
 ...
 */
export function resetBackwardCompatLayer() {
    globalInstance = null;
}
// ============================================================
// UTILITY FUNCTIONS
 ...
 */
export function assertLegacyDimension(vector, context) {
    const dim = Array.isArray(vector) ? vector.length : vector.length;
    if (dim !== LEGACY_VECTOR_DIM) {
        throw new Error(`Expected legacy dimension (${LEGACY_VECTOR_DIM}), got ${dim}${context ? ` in ${context}` : ''}`);
    }
}
/**
 * Type guard for current vectors
 */
export function assertCurrentDimension(vector, context) {
    const dim = Array.isArray(vector) ? vector.length : vector.length;
    if (dim !== VECTOR_DIM) {
        throw new Error(`Expected current dimension (${VECTOR_DIM}), got ${dim}${context ? ` in ${context}` : ''}`);
    }
}
/**
 * Type guard for either dimension (valid for migration period)
 */
export function assertValidDimension(vector, context) {
    const dim = Array.isArray(vector) ? vector.length : vector.length;
    if (dim !== LEGACY_VECTOR_DIM && dim !== VECTOR_DIM) {
        throw new Error(`Expected dimension ${LEGACY_VECTOR_DIM} or ${VECTOR_DIM}, got ${dim}${context ? ` in ${context}` : ''}`);
    }
}
