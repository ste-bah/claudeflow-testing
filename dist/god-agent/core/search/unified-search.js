 ...
 */
import { createSearchError, SearchErrorCode, validateOptions, mergeOptions, } from './search-types.js';
import { VectorSourceAdapter, GraphSourceAdapter, MemorySourceAdapter, PatternSourceAdapter, GNNSearchAdapter, } from './adapters/index.js';
...
export class UnifiedSearch {
     ...
     */
    constructor(vectorDb, graphDb, memoryClient, reasoningBank, gnnEnhancer, options, activityStream) {
    ...
    async search(query, embedding, options) {
        ...
        try {
            ...
            if (!query || query.trim().length === 0) {
                throw createSearchError(SearchErrorCode.INVALID_QUERY, 'Query cannot be empty', { query });
            }
            ...
            const aggregated = await this.executeParallelSearch(query, enhancedEmbedding, queryOptions);
            // Check if all sources failed
            const allFailed = Object.values(aggregated.sourceOutcomes).every((outcome) => outcome.status !== 'success' || outcome.results.length === 0);
            if (allFailed && aggregated.rawResults.length === 0) {
                throw createSearchError(SearchErrorCode.ALL_SOURCES_FAILED, 'All search sources failed or returned no results', {
                    sourceOutcomes: Object.fromEntries(Object.entries(aggregated.sourceOutcomes).map(([k, v]) => [
                        k,
            ...
            const metadata = this.buildMetadata(aggregated, fusedResults.length, totalDurationMs, correlationId);
     ...
     * Execute parallel search across all sources
     */
    async executeParallelSearch(query, embedding, options) {
        // Execute all sources in parallel with Promise.allSettled
        const [vectorResult, graphResult, memoryResult, patternResult] = await Promise.allSettled([
            this.vectorAdapter.search(embedding, options.topK, options.sourceTimeoutMs),
            this.graphAdapter.search(query, options.graphDepth, options.sourceTimeoutMs),
            this.memoryAdapter.search(query, options.memoryNamespace, options.sourceTimeoutMs),
            this.patternAdapter.search(embedding || query, options.minPatternConfidence, options.sourceTimeoutMs),
        ]);
     ...
     */
    settledToOutcome(settled, source) {
     ...
     */
    buildMetadata(aggregated, dedupedCount, totalDurationMs, correlationId) {
     ...
     */
    buildSourceStats(aggregated) {
        const buildStat = (outcome) => ({
            responded: outcome.status === 'success',
            durationMs: outcome.durationMs,
            resultCount: outcome.status === 'success' ? outcome.results.length : 0,
     ...
     */
    async emitSearchEvent(type, payload, status = 'success') {
