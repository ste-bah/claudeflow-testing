 ...
 */
export function isValidPipelineID(id) {
...
export function validatePipelineDefinition(pipeline) {
    ...
    if (pipeline.agents.length > MAX_PIPELINE_AGENTS) {
        throw new PipelineValidationError(`Pipeline cannot exceed ${MAX_PIPELINE_AGENTS} agents`);
    }
 ...
 */
export function validateAgentDefinition(agent, index, isSequential, existingOutputKeys) {
    ...
    if (existingOutputKeys.has(agent.outputKey)) {
        throw new PipelineValidationError(`${position} (${agent.agentName}) has duplicate outputKey: ${agent.outputKey}`);
    }
    ...
    if (isSequential) {
        ...
        if (index === 0 && agent.previousKey) {
            ...
            console.warn(`First agent ${agent.agentName} has previousKey, but no previous agent exists in this pipeline`);
        }
        // Subsequent agents MUST have previousKey
        if (index > 0 && !agent.previousKey) {
            throw new PipelineValidationError(`${agent.agentName} (${agent.position}) missing previousKey. ` +
 ...
 */
export function validateMemoryKeyChain(agents) {
    for (let i = 1; i < agents.length; i++) {
        ...
        if (currentAgent.previousKey !== previousAgent.outputKey) {
            console.warn(`Memory key chain mismatch: Agent ${currentAgent.agentName} expects previousKey "${currentAgent.previousKey}" ` +
 ...
 */
export function buildAgentPrompt(agent, previousContext, pipelineName) {
    const lines = [];
 ...
 */
export function formatAgentPosition(index, total) {
 ...
 */
export function parseAgentPosition(position) {
 ...
 */
export function validateQualityGate(output, qualityGate) {
