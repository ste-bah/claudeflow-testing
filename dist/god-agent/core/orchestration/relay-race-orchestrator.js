 ...
 * 6. REPEAT
 */
import { AgentExecutionError, MemoryKeyError, QualityGateError, } from './orchestration-types.js';
import { validatePipelineDefinition, generatePipelineID, buildAgentPrompt, validateQualityGate, DEFAULT_NAMESPACE, DEFAULT_AGENT_TIMEOUT, } from './orchestration-utils.js';
// ==================== Mock Agent Executor ====================
/**
...
export class RelayRaceOrchestrator {
    ...
    async runPipeline(pipeline) {
        ...
        if (this.sonaEngine && this.options.trackTrajectories) {
            ...
            trajectoryId = this.sonaEngine.createTrajectory(`orchestration.${pipeline.name}`, agentNames, [pipelineId]);
        }
        try {
            ...
            for (let i = 0; i < pipeline.agents.length; i++) {
                ...
                const result = await this.executeAgentStep(pipelineId, agent, pipeline.name, i, pipeline.agents.length);
                // Record result
                execution.agentResults.set(agent.agentName, result);
                // Quality gate failed
                if (!result.success) {
                    throw new QualityGateError(agent.agentName, agent.qualityGate, result.error || 'Unknown error');
                }
            }
            ...
            this.log('info', `Pipeline ${pipeline.name} completed successfully in ${totalDuration}ms. ` +
     ...
     */
    async executeAgentStep(pipelineId, agent, pipelineName, index, totalAgents) {
        ...
        if (agent.previousKey) {
            ...
            if (this.memoryEngine) {
                ...
                if (!previousContext) {
                    throw new MemoryKeyError(agent.previousKey, 'retrieve', `Previous agent output not found. Ensure the previous agent stored its output.`);
                }
        ...
        try {
            ...
            agentOutput = await this.executeWithTimeout(this.agentExecutor.execute(prompt, agent), this.options.agentTimeout, `Agent ${agent.agentName} timed out after ${this.options.agentTimeout}ms`);
            ...
            if (this.memoryEngine) {
                ...
                if (!outputExists) {
                    throw new MemoryKeyError(agent.outputKey, 'validate', `Agent failed to store output. This violates the Relay Race Protocol.`);
                }
            }
     ...
     */
    async validateOutputKey(outputKey) {
        if (!this.memoryEngine) {
...
//# sourceMappingURL=relay-race-orchestrator.js.map