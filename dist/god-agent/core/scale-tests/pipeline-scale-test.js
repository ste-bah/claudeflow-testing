 ...
 */
import { ConcurrencyTracker } from './utils/concurrency-tracker.js';
/**
 * Default pipeline scale configuration
 */
export const DEFAULT_PIPELINE_SCALE_CONFIG = {
    agentCount: 48,
 ...
 */
export function generatePipelineAgents(count) {
    const phases = [
...
export class PipelineScaleTest {
     ...
     */
    async runPipelineTest(config = {}) {
        const cfg = { ...DEFAULT_PIPELINE_SCALE_CONFIG, ...config };
        const agents = generatePipelineAgents(cfg.agentCount);
        const startTime = Date.now();
        const handoffLatencies = [];
        const agentResults = [];
        // Reset state
        this.concurrencyTracker.reset();
        this.memoryStore.clear();
        // Execute agents with concurrency tracking
        const results = await Promise.allSettled(agents.map(async (agent, index) => {
            ...
            try {
                ...
                throw new Error(`Scale test agent "${agent.name}" (id: ${agent.id}) failed: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
            }
            finally {
                handle.exit();
                agentResults.push({
     ...
     */
    async runContentionTest(concurrentOperations = 48) {
...
//# sourceMappingURL=pipeline-scale-test.js.map