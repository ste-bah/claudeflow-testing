 ...
 */
export async function getDirectContext(store, taskId) {
 ...
 */
export async function getTemporalContext(timeIndex, startTime, endTime) {
 ...
 */
export async function getSemanticContext(store, taskId) {
    ...
    const latestEpisode = directEpisodes.reduce((latest, ep) => ep.createdAt > latest.createdAt ? ep : latest);
 ...
 */
export async function getEpisodesByIds(store, ids) {
    const episodes = [];
    // Retrieve episodes in parallel (batch of 20 at a time to avoid overwhelming)
    const batchSize = 20;
    for (let i = 0; i < ids.length; i += batchSize) {
        const batch = ids.slice(i, i + batchSize);
        const batchEpisodes = await Promise.all(batch.map(id => store.getById(id)));
        episodes.push(...batchEpisodes.filter((ep) => ep !== null));
    }
 ...
 */
export async function getEpisodeContext(store, timeIndex, taskId) {
    const now = Date.now();
...
//# sourceMappingURL=episode-linker-context.js.map