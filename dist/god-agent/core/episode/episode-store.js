...
export class EpisodeStore {
    ...
    async createEpisode(options) {
        ...
        try {
            ...
            const transaction = this.db.transaction(() => {
                ...
                this.insertEpisodeStmt.run(episode.id, episode.taskId, episode.startTime, episode.endTime, JSON.stringify(episode.metadata), episode.createdAt, episode.updatedAt);
            ...
            throw new EpisodeStorageError(`Failed to create episode: ${error instanceof Error ? error.message : String(error)}`, error);
        }
    }
    /**
     ...
     */
    async searchBySimilarity(query) {
        await this.initializeVectorBackend();
    ...
    async update(id, updates) {
        ...
        try {
            ...
            throw new EpisodeStorageError(`Failed to update episode ${id}: ${error instanceof Error ? error.message : String(error)}`, error);
        }
    }
    /**
    ...
    async delete(id) {
        ...
        try {
            ...
            throw new EpisodeStorageError(`Failed to delete episode ${id}: ${error instanceof Error ? error.message : String(error)}`, error);
        }
    }
    /**
     * Get all links for an episode
     */
    async getLinks(episodeId) {
    ...
    async save() {
        ...
        try {
            ...
            throw new EpisodeStorageError(`Failed to save vector index: ${error instanceof Error ? error.message : String(error)}`, error);
        }
    }
    /**
     ...
     */
    getStats() {
        const row = this.db.prepare('SELECT COUNT(*) as count FROM episodes').get();
