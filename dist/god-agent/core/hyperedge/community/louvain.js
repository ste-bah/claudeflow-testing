...
export class LouvainDetector {
    graph;
    originalGraph; // Keep original graph for cohesion calculation
    communities; // nodeId -> communityId
    nodeMapping; // super-node -> original nodes
    stats;
    minCommunitySize = 3;
     ...
     */
    calculateModularityGain(nodeId, fromCommunity, toCommunity) {
        ...
        const gain = (weightTo - weightFrom) / m2 +
     ...
     */
    getEdgeWeightToCommunity(nodeId, communityId) {
        let weight = 0;
        const neighbors = this.graph.edges.get(nodeId);
     ...
     */
    calculateCommunityCohesion(members) {
        if (members.length < 2)
            return 1.0;
        let internalEdges = 0;
