...
export class GraphAnomalyDetector {
     ...
     */
    calculateIsolationScore(nodeId, avgDegree) {
        ...
        if (ratio >= 1)
            return 0; // Well-connected or above average
        return 1 - ratio; // [0, 1] where 1 = isolated
    }
    /**
     * Count cross-community edges for a node
     */
    countCrossCommunityEdges(nodeId, nodesCommunityMap) {
        if (!this.graph)
            return 0;
     ...
     */
    calculateCommunityCohesion(nodeId, nodesCommunityMap) {
        if (!this.graph)
            return 0;
        const neighbors = this.graph.edges.get(nodeId) ?? [];
     ...
     */
    extractNodeFeatures(nodeId, avgDegree, nodesCommunityMap) {
        const degree = this.getNodeDegree(nodeId);
    ...
    calculateAnomalyScore(features) {
        ...
        const primarySignal = signals.reduce((max, s) => s.weight * s.value > max.weight * max.value ? s : max);
        const reasons = {
...
//# sourceMappingURL=graph-anomaly-detector.js.map