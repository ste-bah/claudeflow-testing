...
export class CausalStore {
    ...
    async createChain(nodes) {
        ...
        try {
            ...
            if (!validation.valid) {
                throw new Error(`Causal chain contains ${validation.cycles.length} cycle(s). ` +
            ...
            throw new Error(`Failed to create causal chain with ${nodes.length} nodes: ${error instanceof Error ? error.message : String(error)}`, { cause: error });
        }
    }
    /**
     ...
     */
    async findRootCause(effectId, maxDepth) {
        const startTime = Date.now();
        ...
        try {
            ...
            throw new Error(`Root cause analysis failed for effect "${effectId}" (maxDepth: ${maxDepth ?? this.maxDepth}): ${error instanceof Error ? error.message : String(error)}`, { cause: error });
        }
    }
    /**
    ...
    async detectLoops(chainId) {
        ...
        try {
            ...
            throw new Error(`Loop detection failed for chain "${chainId}": ${error instanceof Error ? error.message : String(error)}`, { cause: error });
        }
    }
    /**
     ...
     */
    calculateRootCauseConfidence(actualDepth, maxDepth) {
        ...
        for (const node of nodes) {
            for (const causeId of node.causes) {
                if (!nodeIds.has(causeId)) {
                    throw new Error(`Node ${node.id} references non-existent cause: ${causeId}`);
                }
            }
            for (const effectId of node.effects) {
                if (!nodeIds.has(effectId)) {
                    throw new Error(`Node ${node.id} references non-existent effect: ${effectId}`);
                }
            }
        }
    }
...
//# sourceMappingURL=causal-store.js.map