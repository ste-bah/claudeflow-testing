 ...
 */
export function detectHeadingLevel(markdownLine) {
 ...
 */
export function extractHeadingText(markdownLine) {
    if (!markdownLine || typeof markdownLine !== 'string') {
        return '';
    }
    const match = markdownLine.match(/^#{1,5}\s+(.+)$/);
 ...
 */
export function formatHeading(text, level) {
 ...
 */
export function formatMarkdownHeading(markdownLine) {
 ...
 */
function generateHeadingMarkdown(text, level, style) {
    if (!text)
        return '';
    let formatted = text;
 ...
 */
function generateHeadingHtml(text, level, style) {
    if (!text)
        return '';
 ...
 */
function generateHeadingLatex(text, level, style) {
    if (!text)
        return '';
 ...
 */
export function processHeadings(markdown) {
 ...
 */
export function extractAllHeadings(markdown) {
    if (!markdown || typeof markdown !== 'string') {
        return [];
    }
    const lines = markdown.split('\n');
 ...
 */
function cleanupEmptySubsections(sections) {
...
export function validateHeadingHierarchy(headings) {
    ...
    for (let i = 0; i < headings.length; i++) {
        ...
        if (i === 0 && heading.level > 2) {
            issues.push(`Document should start with a Level 1 or 2 heading, found Level ${heading.level}`);
        }
        // Check for skipped levels (jumping from 1 to 3, etc.)
        if (previousLevel > 0 && heading.level > previousLevel + 1) {
            issues.push(`Heading "${heading.text}" at Level ${heading.level} skips Level ${previousLevel + 1}`);
        }
 ...
 */
export function getHeadingsCss() {
    return `
 ...
 */
export function getApaHeadingStyle(level) {
 ...
 */
export function describeHeadingLevel(level) {
    const descriptions = {
