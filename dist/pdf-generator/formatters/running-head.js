 ...
 * ```
 */
export function truncateToMax(text, maxLength) {
    if (!text) {
 ...
 * ```
 */
export function generateRunningHead(input) {
 ...
 * ```
 */
export function formatRunningHead(input) {
 ...
 */
function generateLatexHeader(runningHead, includeOnTitlePage = true) {
 ...
 */
function generateHtmlHeader(runningHead) {
 ...
 */
export function getRunningHeadCss() {
 ...
 * ```
 */
export function validateRunningHead(text, maxLength) {
    ...
    if (trimmedText.length > max) {
        errors.push(`Running head exceeds ${max} characters (currently ${trimmedText.length} characters). ` +
    ...
    if (trimmedText !== trimmedText.toUpperCase()) {
        warnings.push('Running head should be in ALL CAPS per APA 7th Edition. ' +
 ...
 * ```
 */
export function abbreviateTitle(title, maxLength = 50) {
    if (!title)
        return '';
 ...
 */
export function shouldDisplayRunningHead(isStudentPaper, hasExplicitRunningHead, pageNumber, includeOnTitlePage = true) {
    // Title page check
...
//# sourceMappingURL=running-head.js.map