 ...
 */
import { APA_REFERENCES, APA_FONTS } from '../constants.js';
// =============================================================================
// REFERENCE PARSING
// =============================================================================
/**
 ...
 */
export function parseReference(rawRef) {
    const ref = {
        raw: rawRef.trim(),
        parsed: false,
    };
    ...
    const doiMatch = rawRef.match(/https?:\/\/doi\.org\/([\w.\-/]+)|doi:\s*([\w.\-/]+)/i);
    if (doiMatch) {
    ...
    const authorMatch = rawRef.match(/^([A-Z][a-zA-Z'\-]+(?:\s+[A-Z][a-zA-Z'\-]+)*)/);
    if (authorMatch) {
 ...
 */
export function formatReferences(input) {
    ...
    const formatted = refs.map((ref) => {
        ...
        if (ref.doi) {
            ...
            text = text.replace(/https?:\/\/(?:dx\.)?doi\.org\/[\w.\-/]+|doi:\s*[\w.\-/]+/gi, formattedDoi);
        }
        return text;
    });
    return {
 ...
 */
export function generateReferencesMarkdown(refs) {
    const lines = [];
    ...
    refs.entries.forEach((entry) => {
        ...
        formattedEntry = formattedEntry.replace(/(https:\/\/doi\.org\/[\w.\-/]+)/g, '\\href{$1}{$1}');
        // Also handle regular URLs
        formattedEntry = formattedEntry.replace(/(https?:\/\/(?!doi\.org)[^\s}]+)/g, '\\href{$1}{$1}');
 ...
 */
export function generateReferencesHtml(refs) {
    const lines = [];
    ...
    refs.entries.forEach((entry) => {
        ...
        formattedEntry = formattedEntry.replace(/(https:\/\/doi\.org\/[\w.\-/]+)/g, '<a href="$1" class="doi-link">$1</a>');
        // Convert other URLs to hyperlinks (that aren't already linked)
        formattedEntry = formattedEntry.replace(/(https?:\/\/(?!doi\.org)[^\s<]+)/g, '<a href="$1" class="url-link">$1</a>');
 ...
 */
export function getReferencesCss() {
    return `
...
export function validateReferences(references) {
    ...
    const validRefs = references.filter((ref) => ref && typeof ref === 'string' && ref.trim().length > 0);
    validRefs.forEach((ref, index) => {
        // Check for DOI without https:// prefix
        if (ref.match(/\bdoi:\s*10\./i) && !ref.match(/https?:\/\/doi\.org/)) {
            warnings.push(`Reference ${index + 1}: DOI should be formatted as https://doi.org/... URL`);
        }
        // Check for missing year
        if (!ref.match(/\(\d{4}[a-z]?\)/) && !ref.match(/\((n\.d\.|in press)\)/i)) {
            warnings.push(`Reference ${index + 1}: Missing or non-standard year format`);
        }
        // Check for very short references (likely incomplete)
        if (ref.trim().length < 30) {
            warnings.push(`Reference ${index + 1}: Entry appears incomplete (very short)`);
        }
    });
    return {
        valid: true,
 ...
 */
export function extractDois(references) {
 ...
 */
export function countUniqueAuthors(references) {
    if (!references || !Array.isArray(references)) {
        return 0;
    }
