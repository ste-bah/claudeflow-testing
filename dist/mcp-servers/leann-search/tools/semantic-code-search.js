 ...
 */
function matchesFilter(metadata, condition) {
 ...
 */
function matchesFilterGroup(metadata, filterGroup) {
    const results = filterGroup.conditions.map((condition) => {
        if ('logic' in condition) {
            return matchesFilterGroup(metadata, condition);
        }
        return matchesFilter(metadata, condition);
    });
 ...
 */
function applyFilters(metadata, filters) {
    if (!filters)
        return true;
 ...
 */
function getDeduplicationKey(result) {
 ...
 */
export async function semanticCodeSearch(input, context) {
    ...
    const searchResults = context.backend.search(queryEmbedding, searchLimit, false // Don't include vectors in results
    );
    ...
    for (const result of searchResults) {
        ...
        if (input.languages && input.languages.length > 0) {
            if (!input.languages.includes(metadata.language))
                continue;
        }
        // Apply symbol type filter
        if (input.symbolTypes && input.symbolTypes.length > 0) {
            if (!input.symbolTypes.includes(metadata.symbolType))
                continue;
        }
        // Apply file pattern filter
        if (input.filePattern) {
            if (!matchesGlobPattern(metadata.filePath, input.filePattern))
                continue;
        }
        // Apply repository filter
        if (input.repository) {
            if (metadata.repository !== input.repository)
                continue;
        }
        // Apply custom filters
        if (!applyFilters(metadata, input.filters))
            continue;
...
//# sourceMappingURL=semantic-code-search.js.map