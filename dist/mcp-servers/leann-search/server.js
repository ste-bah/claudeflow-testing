...
import { semanticCodeSearch, SEMANTIC_CODE_SEARCH_DEFINITION, } from './tools/semantic-code-search.js';
import { indexRepository, indexCode, INDEX_REPOSITORY_DEFINITION, INDEX_CODE_DEFINITION, } from './tools/index-repository.js';
import { findSimilarCode, FIND_SIMILAR_CODE_DEFINITION, } from './tools/find-similar-code.js';
import { getIndexStats, GET_INDEX_STATS_DEFINITION, } from './tools/get-index-stats.js';
// ============================================================================
// Constants
// ============================================================================
...
export class LEANNMCPServer {
    ...
    constructor(config = {}) {
        ...
        this.logger = new Logger(this.config.logLevel ?? 'info', this.config.enableLogging ?? true);
        // Initialize state
        this.state = {
            initialized: false,
            backendLoaded: false,
    ...
    setupHandlers() {
        ...
        this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
            ...
            if (!this.backend || !this.embeddingProvider) {
                throw new McpError(ErrorCode.InternalError, 'Server not initialized. Call initialize() first.');
            }
    ...
    async executeTool(name, args, context) {
        switch (name) {
            case 'search_code':
                return semanticCodeSearch(args, context);
            case 'index_repository':
                return indexRepository(args, context);
            case 'index_code':
                return indexCode(args, context);
            case 'find_similar_code':
                return findSimilarCode(args, context);
            case 'get_stats':
                return getIndexStats(args, context);
            default:
    ...
    async initialize() {
        ...
        try {
            ...
            this.backend = new LEANNBackend(VECTOR_DIM, DistanceMetric.COSINE, this.config.leannConfig);
            ...
            this.state.indexedCount = this.backend.count();
            this.logger.info(`LEANN MCP Server initialized with ${this.state.indexedCount} indexed items`);
        }
        catch (error) {
...
export async function main() {
    const server = new LEANNMCPServer({
        ...
        logLevel: process.env.LEANN_LOG_LEVEL ?? 'info',
    });
