...
// Query Param Parsing
// =============================================================================
export function parseGraphQueryParams(query) {
    return {
...
function parseTrajectoryMode(value) {
...
// SQL Query Builders
// =============================================================================
export function buildTrajectoryQuery(params) {
    let sql = `
...
export function buildTokenUsageQuery(params) {
    let sql = `
...
export function createTokenUsageNode(usage) {
    return {
...
export function createFeedbackNode(feedback) {
    return {
...
export function createSessionNode(sessionId, stats) {
    return {
...
// =============================================================================
export function getFilterOptions(learningDb, eventsDb) {
    const options = {
        taskTypes: [],
        agents: [],
        components: [],
    ...
    if (learningDb) {
      ...
      `).get();
            ...
            // Counts
            const trajCount = learningDb.prepare(`SELECT COUNT(*) as cnt FROM trajectory_metadata`).get();
            options.counts.trajectories = trajCount.cnt;
            const tokenCount = learningDb.prepare(`SELECT COUNT(*) as cnt FROM token_usage`).get();
            options.counts.tokenUsage = tokenCount.cnt;
            const feedbackCount = learningDb.prepare(`SELECT COUNT(*) as cnt FROM learning_feedback`).get();
            options.counts.feedback = feedbackCount.cnt;
            const patternCount = learningDb.prepare(`SELECT COUNT(*) as cnt FROM patterns WHERE deprecated = 0`).get();
    ...
    if (eventsDb) {
      ...
      `).all();
            ...
            // Events count
            const eventsCount = eventsDb.prepare(`SELECT COUNT(*) as cnt FROM events`).get();
